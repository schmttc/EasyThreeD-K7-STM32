...
//  void __attribute__((optimize("O0"))) gcode_G29() {
      ...
      switch (phase_value) {
        ...
        case 1:
          ...
          if (g29_verbose_level > 1) {
			...
			SERIAL_PROTOCOLPGM("Probing Mesh Points Closest to (");
			SERIAL_PROTOCOL(x_pos);
            SERIAL_PROTOCOLCHAR(',');
        ...
        for (uint8_t x = 0; x < GRID_MAX_POINTS_X; x++)
          for (uint8_t y = 0;  y < GRID_MAX_POINTS_Y; y++)
            if (!isnan(ubl.z_values[x][y])) {
              ...
              SERIAL_ECHOPAIR(" ; X ", LOGICAL_X_POSITION(ubl.mesh_index_to_xpos[x]));
              SERIAL_ECHOPAIR(", Y ", LOGICAL_Y_POSITION(ubl.mesh_index_to_ypos[y]));
              SERIAL_EOL;
            }
        return;
      }
  ...
  void g29_what_command() {
    ...
    { SERIAL_PROTOCOLPGM("No Mesh Loaded.");}
	  ...
	  SERIAL_PROTOCOL(ubl.state.eeprom_storage_slot);
    ...
    SERIAL_PROTOCOLLNPAIR("ubl.eeprom_start=", hex_address(ubl.eeprom_start));
	...
	SERIAL_PROTOCOLPGM("Kill pin on :");
	SERIAL_PROTOCOL(KILL_PIN);
    ...
    SERIAL_EOL;
    safe_delay(50);
    SERIAL_PROTOCOLLNPAIR("Free EEPROM space starts at: ", hex_address(ubl.eeprom_start));

    SERIAL_PROTOCOLLNPAIR("end of EEPROM              : ", hex_address(E2END));
    safe_delay(50);
    ...
    SERIAL_PROTOCOLLNPAIR("EEPROM free for UBL: ", hex_address(k));
    safe_delay(50);

//    SERIAL_PROTOCOLPAIR("EEPROM can hold ", k / sizeof(ubl.z_values));
	SERIAL_PROTOCOLPGM("EEPROM can hold ");
	SERIAL_PROTOCOL(k / sizeof(ubl.z_values));
...
//    SERIAL_PROTOCOLPAIR("sizeof(ubl.state) : ", (int)sizeof(ubl.state));
	...
	SERIAL_PROTOCOL((int)sizeof(ubl.state));


    //SERIAL_PROTOCOLPAIR("\nGRID_MAX_POINTS_X  ", GRID_MAX_POINTS_X);
	SERIAL_PROTOCOLPGM("\nGRID_MAX_POINTS_X  ");
	SERIAL_PROTOCOL(GRID_MAX_POINTS_X);
	
    //SERIAL_PROTOCOLPAIR("\nGRID_MAX_POINTS_Y  ", GRID_MAX_POINTS_Y);
	SERIAL_PROTOCOLPGM("\nGRID_MAX_POINTS_Y  ");
	SERIAL_PROTOCOL(GRID_MAX_POINTS_Y);
	
    safe_delay(50);
    //SERIAL_PROTOCOLPAIR("\nUBL_MESH_MIN_X         ", UBL_MESH_MIN_X);
	SERIAL_PROTOCOLPGM("\nUBL_MESH_MIN_X         ");
	SERIAL_PROTOCOL(UBL_MESH_MIN_X);
	
   // SERIAL_PROTOCOLPAIR("\nUBL_MESH_MIN_Y         ", UBL_MESH_MIN_Y);
	SERIAL_PROTOCOLPGM("\nUBL_MESH_MIN_Y         ");
	SERIAL_PROTOCOL(UBL_MESH_MIN_Y);
	
    safe_delay(50);
    //SERIAL_PROTOCOLPAIR("\nUBL_MESH_MAX_X         ", UBL_MESH_MAX_X);
	SERIAL_PROTOCOLPGM("\nUBL_MESH_MAX_X         ");
	SERIAL_PROTOCOL(UBL_MESH_MAX_X);

   // SERIAL_PROTOCOLPAIR("\nUBL_MESH_MAX_Y         ", UBL_MESH_MAX_Y);
	SERIAL_PROTOCOLPGM("\nUBL_MESH_MAX_Y         ");
	SERIAL_PROTOCOL(UBL_MESH_MAX_Y);
  ...
  void g29_compare_current_mesh_to_stored_mesh() {
    ...
    float tmp_z_values[GRID_MAX_POINTS_X_MAX][GRID_MAX_POINTS_Y_MAX];
    ...
    SERIAL_PROTOCOLLNPAIR(" loaded from EEPROM address ", hex_address(j)); // Soon, we can remove the extra clutter of printing
            ...
            for (uint8_t k = 0; k < GRID_MAX_POINTS_X; k++) {
              for (uint8_t l = 0; l < GRID_MAX_POINTS_Y; l++) {
                if (!isnan(ubl.z_values[k][l])) {
                  ...
                  */
                  distance += (i - k)* (i - k)* (MESH_X_DIST) * .05
                            + (j - l) * (j - l) *(MESH_Y_DIST) * .05;
