...
//#if ENABLED(AUTO_BED_LEVELING_UBL)
   ...
   * in the future.
   */
  void bit_clear(unsigned int bits[16], uint8_t x, uint8_t y) { CBI(bits[y], x); }
  void bit_set(unsigned int bits[16], uint8_t x, uint8_t y) { SBI(bits[y], x); }
  bool is_bit_set(unsigned int bits[16], uint8_t x, uint8_t y) { return TEST(bits[y], x); }
...
#else
float unified_bed_leveling::z_values[GRID_MAX_POINTS_X_MAX][GRID_MAX_POINTS_Y_MAX],
	  unified_bed_leveling::last_specified_z;
#endif
  ...
  void unified_bed_leveling::load_mesh(const int16_t slot) {
	...
	uint16_t j = (UBL_LAST_EEPROM_INDEX - eeprom_start) / sizeof(z_values);
	...
	SERIAL_PROTOCOLLNPAIR(" at offset ", hex_address(j));
  }
