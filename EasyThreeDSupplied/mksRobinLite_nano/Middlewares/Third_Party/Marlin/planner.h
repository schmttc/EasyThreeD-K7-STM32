...
class Planner {
  ...
  public:
    ...
    static float max_feedrate_mm_s[XYZE_N],     // Max speeds in mm per second
                 ...
                 steps_to_mm[XYZE_N];
    //static uint32_t max_acceleration_steps_per_s2[XYZE_N],    /*--mks--*/
...
//    #if HAS_ABL
	...
	static uint16_t mksBugPrintPauseCnt;
#endif
  ...
  public:
   ...
   // #if PLANNER_LEVELING
     ...
     *  extruder     - target extruder
     */
    static FORCE_INLINE void buffer_line(float lx, float ly, float lz, const float &e, const float &fr_mm_s, const uint8_t extruder) {
      //#if PLANNER_LEVELING && IS_CARTESIAN //mks_delta
    ...
    /*
    static FORCE_INLINE void buffer_line(const float &lx, const float &ly, const float &lz, const float &e, const float &fr_mm_s, const uint8_t extruder) {
      //#if PLANNER_LEVELING && IS_CARTESIAN //mks_delta
      if((BED_LEVELING_METHOD&PLANNER_LEVELING) &&  (MACHINETPYE == Cartesian))
        apply_leveling(lx, ly, lz);
      //#endif
...
*/
	...
	*/
     ...
     */
    static FORCE_INLINE void set_position_mm(float lx, float ly, float lz, const float &e) {
      //#if PLANNER_LEVELING && IS_CARTESIAN	//mks_delta
	  if((BED_LEVELING_METHOD&PLANNER_LEVELING) &&  (MACHINETPYE == Cartesian))
        apply_leveling(lx, ly, lz);
      //#endif
...
/*    
    static FORCE_INLINE void set_position_mm(const float &lx, const float &ly, const float &lz, const float &e) {
      //#if PLANNER_LEVELING && IS_CARTESIAN	//mks_delta
	  if((BED_LEVELING_METHOD&PLANNER_LEVELING) &&  (MACHINETPYE == Cartesian))
        apply_leveling(lx, ly, lz);
      //#endif
