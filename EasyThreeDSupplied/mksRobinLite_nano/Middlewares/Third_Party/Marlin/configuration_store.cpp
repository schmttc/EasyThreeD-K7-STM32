...
//#if ENABLED(MESH_BED_LEVELING)	/*--mks cfg-- MESH_BED_LEVELING */
#if 1
...
//  #if ENABLED(AUTO_BED_LEVELING_BILINEAR)
  ...
  void MarlinSettings::write_data(int &pos, const uint8_t* value, uint16_t size) {
    ...
    while (size--) {
      //uint8_t * const p = (uint8_t * const)pos;       /*--mks--*/
  ...
  void MarlinSettings::read_data(int &pos, uint8_t* value, uint16_t size) {
    do {
      //uint8_t c = eeprom_read_byte((unsigned char*)pos);	/*--mks--*/
      uint8_t c;
	...
	if(BED_LEVELING_METHOD == MESH_BED_LEVELING)
      ...
      static_assert(
        //sizeof(mbl.z_values) == (GRID_MAX_POINTS_X) * (GRID_MAX_POINTS_Y) * sizeof(mbl.z_values[0][0]),
         sizeof(mbl.z_values) == (GRID_MAX_POINTS_X_MAX) * (GRID_MAX_POINTS_Y_MAX) * sizeof(mbl.z_values[0][0]),
...
/*
      ...
      const int bilinear_start[2] = { 0 }, bilinear_grid_spacing[2] = { 0 };	 //int to int16_t
      dummy = 0.0f;
...
*/
	...
	if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_BILINEAR)
		...
		static_assert(
		  //sizeof(z_values) == (GRID_MAX_POINTS_X) * (GRID_MAX_POINTS_Y) * sizeof(z_values[0][0]),
                    sizeof(z_values) == (GRID_MAX_POINTS_X_MAX) * (GRID_MAX_POINTS_Y_MAX) * sizeof(z_values[0][0]),
		  "Bilinear Z array is the wrong size."
		);
		 grid_max_x = GRID_MAX_POINTS_X;
		...
		EEPROM_WRITE(z_values); 			 // 9-256 floats
			...
			//const int bilinear_start[2] = { 0 }, bilinear_grid_spacing[2] = { 0 };	  /*---mks---*/ //int to int16_t
    ...
    if (!eeprom_write_error) {
	  ...
	  if(eeprom_size> (MKS_EEPROM_OFFSET-1))
	  	{
	  	SERIAL_ECHOLNPGM("Error!!! EEPROM OVERFLOW!");
  ...
  bool MarlinSettings::load() {
    ...
    /*---mks---*/
      ...
      float *tmp2 = NULL;
      uint32_t *tmp3 = NULL;
      tmp1 = (float *)malloc((XYZ + esteppers) * sizeof(float));
      tmp2 = (float *)malloc((XYZ + esteppers) * sizeof(float));
      tmp3 = (uint32_t *)malloc((XYZ + esteppers) * sizeof(uint32_t));
      read_data(eeprom_index, (uint8_t *)tmp1, (XYZ + esteppers) * sizeof(float));
      read_data(eeprom_index, (uint8_t *)tmp2, (XYZ + esteppers) * sizeof(float));
      read_data(eeprom_index, (uint8_t *)tmp3, (XYZ + esteppers) * sizeof(uint32_t));
...
//#elif HAS_ABL
	else if(BED_LEVELING_METHOD&HAS_ABL)	{
      if (!forReplay) {
        CONFIG_ECHO_START;
        SERIAL_ECHOLNPGM("Auto Bed Leveling:");
      }
