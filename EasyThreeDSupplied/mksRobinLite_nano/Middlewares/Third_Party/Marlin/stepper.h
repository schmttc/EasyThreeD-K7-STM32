...
#define ENABLE_STEPPER_DRIVER_INTERRUPT()   HAL_TIM_Base_Start_IT(&htim2)    //?a?¡ì?¨¨?D?¡ì?¡ì?¡ì??a?a???¨¬?¡ìo?¡§¡è????D??
#define DISABLE_STEPPER_DRIVER_INTERRUPT() 	HAL_TIM_Base_Stop_IT(&htim2)		//?a?¡ì?¨¨?D?¡ì?¡ì?¡ì??a1????¨¬?¡ìo?¡§¡è????D??
...
class Stepper {
  ...
  private:
    ...
    static volatile uint32_t step_events_completed; // The number of step events executed in the current block
	  ...
	  #define _NEXT_ISR(T)  do{OCR1A = T; if(TCNT1>(T-16)) TCNT1=T-16;}while(0)
...
if (step_rate >= (2048)) { // higher step rate
  ...
  unsigned short table_address = &speed_lookuptable_slow[0][0];
  table_address += ((step_rate) >> 1) & 0xFFFC;
  timer = (unsigned short)pgm_read_word_near(table_address);
  timer -= (((unsigned short)pgm_read_word_near(table_address + 2) * (unsigned char)(step_rate & 0x0007)) >> 3);
}
...
/*-------------TEST-----------------*/
      ...
      timer = (uint16_t)(F_CPU() / (step_rate * (TICK_TIMER_PRESCALER)));		/*--mks--*/ /*???*/
