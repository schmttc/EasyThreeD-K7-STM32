...
/*--mks cfg--begin MESH_BED_LEVELING */
/*
#if HAS_ABL
  #include "vector_3.h"
...
static char command_queue_bak[8][MAX_CMD_SIZE];
static char command_queue_bak_end[MAX_CMD_SIZE];
...
#if FAN_COUNT > 0
  //int16_t fanSpeeds[FAN_COUNT] = { 0 };  /*--mks--*/     
...
/*--mks cfg--begin AUTO_BED_LEVELING_BILINEAR */
...
/*
  ...
  #endif
*/
void  ADJUST_DELTA(const float logical[XYZ])
{
	float zadj;
...
//#elif IS_KINEMATIC	//mks_delta
  //#define ADJUST_DELTA(V) NOOP
#endif
/*--mks cfg--end AUTO_BED_LEVELING_BILINEAR */
...
//#if ENABLED(AUTO_BED_LEVELING_BILINEAR)
  ...
  int bilinear_grid_spacing[2], bilinear_start[2];		/*---mks---*/ //int to int16_t
  float bilinear_grid_factor[2],
        //z_values[GRID_MAX_POINTS_X][GRID_MAX_POINTS_Y];
        z_values[GRID_MAX_POINTS_X_MAX][GRID_MAX_POINTS_Y_MAX];
#endif

//#if IS_SCARA	//mks_delta
#if 1
...
#define XYZ_CONSTS_FROM_CONFIG(type, array, CONFIG) \
  type array##_P[XYZ] = { X_##CONFIG, Y_##CONFIG, Z_##CONFIG }; \
  static inline type array(AxisEnum axis) { return (array##_P[axis]); } 	
  //typedef void __void_##CONFIG##__

XYZ_CONSTS_FROM_CONFIG(float, base_min_pos,   MIN_POS);			/*--mks cfg--*/
XYZ_CONSTS_FROM_CONFIG(float, base_max_pos,   MAX_POS);			/*--mks cfg--*/
XYZ_CONSTS_FROM_CONFIG(float, base_home_pos,  HOME_POS);		/*--mks cfg--*/ 
XYZ_CONSTS_FROM_CONFIG(float, max_length,     MAX_LENGTH);		/*--mks cfg--*/	
XYZ_CONSTS_FROM_CONFIG(float, home_bump_mm,   HOME_BUMP_MM);
XYZ_CONSTS_FROM_CONFIG(signed char, home_dir, HOME_DIR);		/*--mks cfg--*/
//signed char home_dir_P[XYZ] = {-1, Y_HOME_DIR, Z_HOME_DIR }; 
//static inline signed char home_dir(AxisEnum axis) { return (home_dir_P[axis]); }	  
...
*/
	int freeMemory() { return 4396; }	//No free ram management
...
static bool drain_injected_commands_P() {
  if (injected_commands_P != NULL) {
    ...
    strncpy(cmd, injected_commands_P, sizeof(cmd) - 1);	/*--mks--*/
...
static millis_t last_wait_time = 0;
  ...
  // send "wait" to indicate Marlin is still waiting.
	...
	/*5s?¨¬2a2?¦Ì?¡ä??¨²¨ºy?Y?¨°¨¨??a??¨®D¨¢a?¨²????¡ê??¨¦¨º1¨®?wifi*/
    ...
    while (commands_in_queue < BUFSIZE && !card_eof && !stop_buffering) {
      ...
      if (card_eof || n == -1
        ...
        if (!sd_comment_mode) 
			...
			if(sd_char == 0x5a ) 
				{
				mksReprint.sdpos = card.getsdpos()-3;	//"·Ö²ãÎ»ÖÃ"
...
static void set_axis_is_at_home(AxisEnum axis) {
   ...
   */
  //#if HAS_BED_PROBE && Z_HOME_DIR < 0	/*--mks cfg--*/
  #if HAS_BED_PROBE
  if(Z_HOME_DIR < 0){
    if (axis == Z_AXIS) {
...
//    #if ENABLED(Z_MIN_PROBE_ENDSTOP)
	...
	if(Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN)
		{
		if (deploy && (READ(Z_MIN_PIN) != Z_MIN_ENDSTOP_INVERTING)) 
		  { 	 // If BL-Touch says it's triggered
		  ...
		  if ((READ(Z_MIN_PIN) != Z_MIN_ENDSTOP_INVERTING)) { 			 // If it still claims to be triggered...
			SERIAL_ERROR_START;
	...
	else if(Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MAX_ENDSTOP_PIN
		{
		if (deploy && (READ(Z_MAX_PIN) != Z_MIN_PROBE_ENDSTOP_INVERTING)) 
		  { 	 // If BL-Touch says it's triggered
		  bltouch_command(BLTOUCH_RESET);	 //  try to reset it.
		  bltouch_command(BLTOUCH_DEPLOY);	 // Also needs to deploy and stow to
		  bltouch_command(BLTOUCH_STOW);	 //  clear the triggered condition.
		  safe_delay(1500); 				 // Wait for internal self-test to complete.
											 //  (Measured completion time was 0.65 seconds
											 //   after reset, deploy, and stow sequence)
		  if ((READ(Z_MAX_PIN) != Z_MIN_PROBE_ENDSTOP_INVERTING)) { 			 // If it still claims to be triggered...
			SERIAL_ERROR_START;
			SERIAL_ERRORLNPGM(MSG_STOP_BLTOUCH);
			stop(); 						 // punt!
		  }
  ...
  bool set_probe_deployed(bool deploy) {
    ...
    // Make room for probe
    //do_probe_raise(_Z_CLEARANCE_DEPLOY_PROBE);	//mks
    do_probe_raise(Z_CLEARANCE_DEPLOY_PROBE>Z_CLEARANCE_BETWEEN_PROBES ? Z_CLEARANCE_DEPLOY_PROBE : Z_CLEARANCE_BETWEEN_PROBES);
    ...
    const float oldXpos = current_position[X_AXIS],
      ...
      bool _triggered_bool =false;
	if(Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN)
		_triggered_bool = (READ(Z_MIN_PIN) != Z_MIN_ENDSTOP_INVERTING);
	else if(Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MAX_ENDSTOP_PIN)
		_triggered_bool = (READ(Z_MAX_PIN) != Z_MIN_PROBE_ENDSTOP_INVERTING);
	
      //if (_TRIGGERED_WHEN_STOWED_TEST == deploy) 
	if (_triggered_bool == deploy) 
	  {     // closed after the probe specific actions.
...
//      if (_TRIGGERED_WHEN_STOWED_TEST == deploy) { // State hasn't changed?
	  if (_triggered_bool == deploy) { // State hasn't changed?
...
//#else
else if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_UBL)
{
	ubl.state.active = enable;
	//set_current_from_steppers_for_axis(Z_AXIS);
...
*/	
		  ...
		  if (!enable)
		  	{
			if(BED_LEVELING_METHOD&ABL_PLANAR)
...
else
{
  ...
  else if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_BILINEAR)
  	{
	  bilinear_start[X_AXIS] = bilinear_start[Y_AXIS] =
	  bilinear_grid_spacing[X_AXIS] = bilinear_grid_spacing[Y_AXIS] = 0;
	  for (uint8_t x = 0; x < GRID_MAX_POINTS_X; x++)
...
//#if ENABLED(AUTO_BED_LEVELING_BILINEAR) || ENABLED(MESH_BED_LEVELING)
#if 1   /*--mks cfg--end AUTO_BED_LEVELING_BILINEAR */
  /**
  ...
  static void print_2d_array(const uint8_t sx, const uint8_t sy, const uint8_t precision, const uint8_t fn) {
    ...
    for (uint8_t y = 0; y < sy; y++) {
      ...
      for (uint8_t x = 0; x < sx; x++) {
        SERIAL_PROTOCOLCHAR(' ');
        //const float offset = fn(x, y); /*--mks cfg-- LCD_ MESH_BED_LEVELING */
        float offset;
		switch(fn)
			{
			case 0:	
...
//#if ENABLED(AUTO_BED_LEVELING_BILINEAR)
  ...
  static void print_bilinear_leveling_grid() {
	...
	*/
    print_2d_array(GRID_MAX_POINTS_X, GRID_MAX_POINTS_Y, 3,0);
  }
    ...
    static void bed_level_virt_print() {
      ...
      print_2d_array(ABL_GRID_POINTS_VIRT_X, ABL_GRID_POINTS_VIRT_Y, 5,1);
    }
...
static void do_homing_move(const AxisEnum axis, float distance, float fr_mm_s=0.0) {
  ...
  if((Z_HOME_DIR < 0) && (Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN)){	/*--mks cfg--*/
...
#if HOMING_Z_WITH_PROBE
	if((Z_HOME_DIR < 0) && (Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN))	/*--mks cfg--*/
		bump = axis_home_dir*((axis == Z_AXIS) ? max(Z_CLEARANCE_BETWEEN_PROBES, home_bump_mm(Z_AXIS)) :home_bump_mm(axis));
	else
	...
	inline void gcode_G0_G1(
	  ...
	  if (IsRunning()) {
		gcode_get_destination(); // For X Y Z E F
		  ...
		  prepare_move_to_destination(); //mks step1  destination[]={1.0,0.0,100.0,0.0}
	  }
	}
...
//#if ENABLED(QUICK_HOME)	//mks_delta
#if 1
...
bool cfg_tmp_bool;    
 if((Z_HOME_DIR < 0) && (Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN))	/*--mks cfg--*/
	{
	#if HOMING_Z_WITH_PROBE
    cfg_tmp_bool = (position_is_reachable(destination , true)); 
	#else
    cfg_tmp_bool = (position_is_reachable(destination)); 
	#endif
	}
else
	cfg_tmp_bool = (position_is_reachable(destination));
...
//  #if HAS_LEVELING
  ...
  if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_UBL)
    bed_leveling_state_at_entry = ubl.state.active;

  if(BED_LEVELING_METHOD&HAS_LEVELING)
  	{
	//#if ENABLED(AUTO_BED_LEVELING_UBL)
	/*if(BED_LEVELING_METHOD&AUTO_BED_LEVELING_UBL)
		{
...
if(Z_HOME_DIR > 0){		/*--mks cfg--*/
   ...
   // #else	/*--mks cfg--*/
    	...
    	{
      if ((home_all_axis || homeX || homeY) && (mksReprint.mks_printer_state != MKS_REPRINTED)) {    // mksReprint.mks_printer_state
...
//    #endif
   ...
   // #if ENABLED(HOME_Y_BEFORE_X)	/*--mks cfg--*/
	if(HOME_Y_BEFORE_X)
      // Home Y
...
//    #if DISABLED(HOME_Y_BEFORE_X)	/*--mks cfg--*/
	if(!HOME_Y_BEFORE_X)
      // Home Y
      if (home_all_axis || homeY) {
        HOMEAXIS(Y);
...
/*--mks cfg-- MESH_BED_LEVELING */
//#if ENABLED(MESH_BED_LEVELING) || ENABLED(PROBE_MANUALLY)
#if 1
  inline void _manual_goto_xy(const float &x, const float &y) {
    const float old_feedrate_mm_s = feedrate_mm_s;

    //#if MANUAL_PROBE_HEIGHT > 0	//mks
    #if 1
...
//#if ENABLED(MESH_BED_LEVELING)
  ...
  void mbl_mesh_report() {
    ...
    print_2d_array(GRID_MAX_POINTS_X, GRID_MAX_POINTS_Y, 5,2);
  }
...
/*--mks cfg-- AUTO_BED_LEVELING_BILINEAR */
...
//  #if ABL_GRID
  ...
  inline void gcode_G29() {
   ...
   // #if ABL_GRID
	//if(BED_LEVELING_METHOD&ABL_GRID)  
		//{
	...
	ABL_VAR int left_probe_bed_position=0, right_probe_bed_position=0, front_probe_bed_position=0, back_probe_bed_position=0;
...
//#if ENABLED(AUTO_BED_LEVELING_LINEAR) || ENABLED(PROBE_MANUALLY)
  //if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_LINEAR)
  	//{
  	int abl2;
	  //#if ABL_PLANAR
	  if(! (BED_LEVELING_METHOD&ABL_PLANAR)) abl2 = ABL_GRID_MAX;
	  //#endif
  	//}
//#endif
	...
	//if(BED_LEVELING_METHOD&AUTO_BED_LEVELING_BILINEAR)
	  ...
	  //else if(BED_LEVELING_METHOD&AUTO_BED_LEVELING_LINEAR)
	  	//{
        ABL_VAR int indexIntoAB[GRID_MAX_POINTS_X_MAX][GRID_MAX_POINTS_Y_MAX];
...
/*
			  ABL_VAR float eqnAMatrix[9 * 3], // "A" matrix of the linear system of equations
						 eqnBVector[9],	   // "B" vector of Z points
						 mean;
*/

                    
		ABL_VAR float *eqnAMatrix=NULL, // "A" matrix of the linear system of equations
			 		 *eqnBVector=NULL,	   // "B" vector of Z points
					 mean;
		
		eqnAMatrix = (float *)malloc((ABL_GRID_MAX * 3 + 1) * sizeof(float));
		if(eqnAMatrix == NULL)
			{
			Running = false;
			kill("malloc eqnAMatrix err!");
			}
		for(int i=0;i<ABL_GRID_MAX * 3 + 1;i++)		*(eqnAMatrix+i) = 0.0;

		
		eqnBVector = (float *)malloc((ABL_GRID_MAX + 1) * sizeof(float));
		if(eqnBVector == NULL)
			{
			Running = false;
			kill("malloc eqnBVector err!");
			}
		for(int i=0;i<ABL_GRID_MAX + 1;i++) 	*(eqnBVector+i) = 0.0;
    ...
    //else if(BED_LEVELING_METHOD&AUTO_BED_LEVELING_3POINT)	
		//{
    ...
    if (!g29_in_progress) {
      ...
      //#if ENABLED(AUTO_BED_LEVELING_BILINEAR) /*--mks cfg-- AUTO_BED_LEVELING_BILINEAR */
	if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_BILINEAR) {
        if (code_seen('W')) {
...
*/
	if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_BILINEAR)
	  ...
	  if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_3POINT)
	  	{
        #if ENABLED(DEBUG_LEVELING_FEATURE)
...
inline void gcode_M42() {
  ...
  //analogWrite(pin_number, pin_status);	/*--mks--*/
  ...
  inline void servo_probe_test() {
      ...
      uint8_t PROBE_TEST_PIN = 0;
	  if(Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN)
	  	{

       // #define PROBE_TEST_PIN Z_MIN_PIN
		PROBE_TEST_PIN = Z_MIN_PIN;
		...
		/*--mks cfg begin-- Z_MIN_ENDSTOP_INVERTING */
		/*
		...
		*/
		if(Z_MIN_ENDSTOP_INVERTING)
			...
			SERIAL_PROTOCOLLNPGM("false");
		/*--mks cfg end-- Z_MIN_ENDSTOP_INVERTING */
...
*/
	bool was_enabled;
	if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_UBL)
		was_enabled = ubl.state.active;
	else if(BED_LEVELING_METHOD == MESH_BED_LEVELING)
		was_enabled = mbl.active();
	else if(BED_LEVELING_METHOD != NULL_BED_LEVELING)
		was_enabled = planner.abl_enabled;

	if(BED_LEVELING_METHOD != NULL_BED_LEVELING)
		set_bed_leveling_enabled(false);
...
//	if (code_value_temp_abs() > thermalManager.degHotend(target_extruder)) LCD_MESSAGEPGM("E1 Heating..."); 	  //mks
	...
	if (code_value_temp_abs() > thermalManager.degHotend(target_extruder))
		...
		switch(target_extruder)
			{
			case 0:
				if(mksCfg.lcd_language == 1) LCD_MESSAGEPGM("E1 \x80\xc4\x9b...");
				else LCD_MESSAGEPGM("E1 Heating...");
				break;
			case 1:
				LCD_MESSAGEPGM("E2 Heating...");
				break;
			case 2:
				LCD_MESSAGEPGM("E3 Heating...");
				break;
			case 3:
				LCD_MESSAGEPGM("E4 Heating...");
				break;
			case 4:
				LCD_MESSAGEPGM("E5 Heating...");
				break;
			default: break;
...
//#if HAS_TEMP_HOTEND || HAS_TEMP_BED	/*--mks cfg-- HAS_TEMP_BED --*/
#if 1
  void print_heaterstates() {
      ...
      SERIAL_PROTOCOL(thermalManager.degTargetHotend(target_extruder));
        ...
        SERIAL_PROTOCOLCHAR(')');
      #endif
       // SERIAL_PROTOCOLLNPGM(".");       //mks add
    #endif
/*--mks cfg begin-- HAS_TEMP_BED */	
/*    #if HAS_TEMP_BED
      SERIAL_PROTOCOLPGM(" B:");
...
/*--mks cfg end-- HAS_TEMP_BED */
   ...
   // #if HAS_TEMP_BED	/*--mks cfg-- HAS_TEMP_BED --*/
	if(HAS_TEMP_BED) {
   	  SERIAL_PROTOCOLPGM(" B@:");
...
/*--mks cfg-- HAS_TEMP_BED --*/
//#if ENABLED(AUTO_REPORT_TEMPERATURES) && (HAS_TEMP_HOTEND || HAS_TEMP_BED)
#if ENABLED(AUTO_REPORT_TEMPERATURES) && (HAS_TEMP_HOTEND )
...
inline void gcode_M109() {
  ...
  if (no_wait_for_cooling || code_seen('R')) {
    ...
    //if (thermalManager.isHeatingHotend(target_extruder))  LCD_MESSAGEPGM("E1 Heating...");        //mks
	if (code_value_temp_abs() > thermalManager.degHotend(target_extruder))
		{
		switch(target_extruder)
			{
			case 0:
				//LCD_MESSAGEPGM("E1 Heating...");
  ...
  do {
    //cancel m109 waiting for narno print_pin 
    //if(cancel_heatup) {
     // cancel_heatup = false;
     // break;
    //}
...
inline void gcode_M115() {
  SERIAL_PROTOCOLLNPGM(MSG_M115_REPORT);
    ...
    // AUTOLEVEL (G29)
    //#if HAS_ABL
	if(BED_LEVELING_METHOD&HAS_ABL)
...
//#if ENABLED(MORGAN_SCARA)	//mks_delta
#if 1
...
//#if ENABLED(MESH_BED_LEVELING)
   ...
   */
//  inline void gcode_M421() {	/*--mks cfg-- MESH_BED_LEVELING */
...
*/			  
	  float diff;
//#if  ENABLED(BABYSTEP_ZPROBE_OFFSET) || ENABLED(DELTA)
	if(MACHINETPYE == DELTA)
...
/*    
    ...
    ;
*/
    float z_lift = code_seen('Z') ? code_value_linear_units() :FILAMENT_CHANGE_Z_ADD;
      ...
      if (tmp_extruder != active_extruder) {
        ...
        set_destination_to_current();
          ...
          if (dual_x_carriage_mode == DXC_AUTO_PARK_MODE
            ...
            //#if ENABLED(MAX_SOFTWARE_ENDSTOPS)	/*--mks cfg--*/
...
//      #if ENABLED(AUTO_BED_LEVELING_UBL) && ENABLED(UBL_G26_MESH_EDITING)
      ...
      //#if HAS_LEVELING
		...
		if(BED_LEVELING_METHOD&HAS_LEVELING)	
			{
			switch(BED_LEVELING_METHOD)		/*--mks cfg-- MESH_BED_LEVELING */ 
				{
				case MESH_BED_LEVELING:
					gcode_G29_MESH_BED_LEVELING();
					(void)settings.save();
					break;
				case AUTO_BED_LEVELING_3POINT:
				case AUTO_BED_LEVELING_LINEAR:
				case AUTO_BED_LEVELING_BILINEAR:
					gcode_G29();
					(void)settings.save();
					break;
				case AUTO_BED_LEVELING_UBL:
					gcode_G29_UBL();
					break;
				default:
					SERIAL_PROTOCOL("bed leveling hasn't defined!");
					SERIAL_EOL;
					break;
				}
			}
...
/*		  
		 ...
		 if(mksReprint.dirname[0] == 0 || tmp_index != NULL)
	  		{
...
*/
	  ...
	  if(tmp_index)
		...
		memset(mksReprint.dirname,0,sizeof(mksReprint.dirname));	//Ä¿Â¼ÃûÇåÁã
	...
	if(HAS_TEMP_BED)	/*--mks cfg-- HAS_TEMP_BED --*/
          ...
          SERIAL_PROTOCOLLNPGM("HAS_NO_TEMP_BED!"); 
        break;
...
void ok_to_send() {
  ...
  void clamp_to_software_endstops(float target[XYZ]) {
    ...
    //#if ENABLED(MIN_SOFTWARE_ENDSTOPS)	/*--mks cfg--*/
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
  ...
  //#if PLANNER_LEVELING
  if(BED_LEVELING_METHOD&PLANNER_LEVELING)
...
//#if ENABLED(MORGAN_SCARA)	//mks_delta
   ...
   * Integrated into Marlin and slightly restructured by Joachim Cerny.
   */
//  void inverse_kinematics(const float logical[XYZ]) {		//mks_delta
  void inverse_kinematics_MORGAN_SCARA(const float logical[XYZ]) {
...
//#if MASK_FOR_SPACE
  ...
  //#if ENABLED(AUTO_REPORT_TEMPERATURES) && (HAS_TEMP_HOTEND || HAS_TEMP_BED)	/*--mks cfg-- HAS_TEMP_BED --*/
  #if ENABLED(AUTO_REPORT_TEMPERATURES) && (HAS_TEMP_HOTEND)	
