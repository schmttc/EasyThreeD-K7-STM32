...
//#define ENABLE_STEPPER_DRIVER_INTERRUPT()   HAL_TIM_Base_Start_IT(&htim2)    //?a¡§¡è?D¡§¡§¡§¡ãa?a??¡ì¡§o?¨¤????D??
//#define DISABLE_STEPPER_DRIVER_INTERRUPT() 	HAL_TIM_Base_Stop_IT(&htim2)		//?a¡§¡è?D¡§¡§¡§¡ãa1???¡ì¡§o?¨¤????D??
...
*/
	/*--mks--*/
//AVR GCC has sei() for __enable_interrupts(), and cli() for __disable_interrupts().  
...
/*
    ...
    sei();
*/	/*--mks--*/
	  HAL_TIM_Base_Stop_IT(&htim4);		//¹Ø±Õ²âÎÂÖÐ¶Ïå
	  DISABLE_STEPPER_DRIVER_INTERRUPT();
	  sei();
  #endif
  ...
  for (uint8_t i = step_loops; i--;) {
    ...
    // which increments every 4Âµs on 16MHz and every 3.2Âµs on 20MHz.
...
//  #if CORE_IS_XY || CORE_IS_XZ || IS_SCARA
  	if(MACHINETPYE&IS_SCARA || MACHINETPYE&CORE_IS_XZ || MACHINETPYE&IS_SCARA)
...
//  #endif
 ...
 // #if CORE_IS_XY || CORE_IS_YZ || IS_SCARA
  if(MACHINETPYE&CORE_IS_XY || MACHINETPYE&CORE_IS_YZ || MACHINETPYE&IS_SCARA)
