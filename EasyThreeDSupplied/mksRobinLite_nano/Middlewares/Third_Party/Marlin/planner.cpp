...
#endif
/*--mks--*/
unsigned long Planner::max_acceleration_steps_per_s2[XYZE_N],
...
//#if ABL_PLANAR
  ...
  bool Planner::autotemp_enabled = false;
	 ...
	 uint16_t Planner::mksBugPrintPauseCnt=0;
#endif
...
void Planner::check_axes_activity() {
  ...
  if (blocks_queued()) {
    ...
    if (!axis_active[X_AXIS]) disable_X(); ABC
  #endif
    ...
    #else
      #if HAS_FAN0
        //analogWrite(FAN_PIN, CALC_FAN_SPEED(0));	/*--mks--*/
      #endif
      #if HAS_FAN1
...
//#if PLANNER_LEVELING
  ...
  void Planner::apply_leveling(float &lx, float &ly, float &lz) {
    ...
    #if ENABLED(ENABLE_LEVELING_FADE_HEIGHT)    //MKS_DEBUG_AUTO_BED_LEVELING_LINEAR
		...
		if((MACHINETPYE != DELTA) && (BED_LEVELING_METHOD&HAS_FADE))	//mks_delta
			{
				...
				{
				lz += mbl.get_z_mks(RAW_X_POSITION(lx), RAW_Y_POSITION(ly));
				}
		}
		}
		else if(BED_LEVELING_METHOD&ABL_PLANAR)
			{
			float dx = RAW_X_POSITION(lx) - (X_TILT_FULCRUM),
				  dy = RAW_Y_POSITION(ly) - (Y_TILT_FULCRUM),
				  dz = RAW_Z_POSITION(lz);
		...
		if (mbl.active()) {
                      if((MACHINETPYE != DELTA) && (BED_LEVELING_METHOD&HAS_FADE))	//mks_delta
                      {
						...
						logical[Z_AXIS] = (z_fade_height * (RAW_Z_POSITION(logical[Z_AXIS]) - c)) / (z_fade_height - c);
					  	...
					  	{
						logical[Z_AXIS] -= mbl.get_z_mks(RAW_X_POSITION(logical[X_AXIS]), RAW_Y_POSITION(logical[Y_AXIS]));
					  	}
		}
...
void Planner::mksBugPrintPause()
{
  if(card.sdprinting && card.getsdpos()>5000)
    mksBugPrintPauseCnt++;
}
#endif
