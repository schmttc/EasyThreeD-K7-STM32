...
//int16_t Temperature::current_temperature_raw[HOTENDS] = { 0 },     /*--mks--*/   
  ...
  void Temperature::PID_autotune(float temp, int hotend, int ncycles, bool set_result/*=false*/) {
...
#endif
	if(PIDTEMP == 0 && PIDTEMPBED == 0) if(hotend >= 0 || hotend < 0) {SERIAL_ECHOLN(MSG_PID_BAD_EXTRUDER_NUM);return;}
	if(PIDTEMP == 0 && PIDTEMPBED == 1) if(hotend >= 0 || hotend < -1) {SERIAL_ECHOLN(MSG_PID_BAD_EXTRUDER_NUM);return;}
	if(PIDTEMP == 1 && PIDTEMPBED == 0) if(hotend >= HOTENDS|| hotend < 0) {SERIAL_ECHOLN(MSG_PID_BAD_EXTRUDER_NUM);return;}
	if(PIDTEMP == 1 && PIDTEMPBED == 1) if(hotend >= HOTENDS|| hotend < -1) {SERIAL_ECHOLN(MSG_PID_BAD_EXTRUDER_NUM);return;}
	...
	if (HAS_PID_FOR_BOTH) {
      if (hotend < 0)
        ...
        soft_pwm[hotend] = bias = d = (PID_MAX) >> 1;}
	else if(PIDTEMP)	soft_pwm[hotend] = bias = d = (PID_MAX) >> 1;
	else soft_pwm_bed = bias = d = (MAX_BED_POWER) >> 1;
    ...
    while (wait_for_heatup) {
      ...
      if (temp_meas_ready) { // temp sample ready
        ...
        ;
#endif
		if(HAS_PID_FOR_BOTH) 	input = hotend < 0 ? current_temperature_bed : current_temperature[hotend];
		else if(PIDTEMP) 		input = current_temperature[hotend];
        ...
        if (heating && input > temp) {
          if (ELAPSED(ms, t2 + 5000UL)) {
            ...
            #endif
#endif
			if(HAS_PID_FOR_BOTH)	{if (hotend < 0) soft_pwm_bed = (bias - d) >> 1; else soft_pwm[hotend] = (bias - d) >> 1;}
			else if(PIDTEMP)		soft_pwm[hotend] = (bias - d) >> 1;
...
#endif
              long max_pow;
              if(HAS_PID_FOR_BOTH)	 max_pow = hotend < 0 ? MAX_BED_POWER : PID_MAX;
              else if(PIDTEMP)	 max_pow = PID_MAX;
              else 			 max_pow = MAX_BED_POWER;
	/*--mks cfg begin--*/
...
#endif	
	if(HAS_PID_FOR_BOTH)	if (set_result) {if (hotend < 0) _SET_BED_PID(); else _SET_EXTRUDER_PID();}
	else if(PIDTEMP)		if (set_result)  _SET_EXTRUDER_PID();
	else 					if (set_result)  _SET_BED_PID();
...
//  #if ENABLED(PIDTEMP) && ENABLED(PID_EXTRUSION_SCALING)	/*--mks cfg end--*/
#if 0
    last_e_position = 0;
  #endif
  ...
  void Temperature::start_watching_heater(uint8_t e) {
    ...
    if (degHotend(HOTEND_INDEX) < degTargetHotend(HOTEND_INDEX) - (WATCH_TEMP_INCREASE + TEMP_HYSTERESIS + 1)) {
		//int mks_temp = WATCH_TEMP_INCREASE;
    	//watch_target_temp[HOTEND_INDEX] = degHotend(HOTEND_INDEX) + mks_temp;	
...
void Temperature::set_current_temp_raw() {
  ...
  void endstop_monitor() {
    ...
    #if HAS_X_MIN	if(mksCfg.use_xmin_plug)
      if (READ(X_MIN_PIN)) SBI(current_endstop_bits_local, X_MIN);
    #endif
    #if HAS_X_MAX	if(mksCfg.use_xmax_plug)
      if (READ(X_MAX_PIN)) SBI(current_endstop_bits_local, X_MAX);
    #endif
    #if HAS_Y_MIN	if(mksCfg.use_ymin_plug)
      if (READ(Y_MIN_PIN)) SBI(current_endstop_bits_local, Y_MIN);
    #endif
    #if HAS_Y_MAX	if(mksCfg.use_ymax_plug)
      if (READ(Y_MAX_PIN)) SBI(current_endstop_bits_local, Y_MAX);
    #endif
    #if HAS_Z_MIN	if(mksCfg.use_zmin_plug)
 ...
 // sei();  /*--mks--*/
  ...
  ISR_STATICS(0);
    ...
    if (pwm_count_tmp >= 127) {
      ...
      //WRITE_HEATER_0(soft_pwm_0 > pwm_mask ? HIGH : LOW);	/*--mks--*/
	  if(soft_pwm_0 > pwm_mask) WRITE_HEATER_0(HIGH);
	  else WRITE_HEATER_0(LOW);
      #if HOTENDS > 1
        ...
        //WRITE_HEATER_BED(soft_pwm_BED > pwm_mask ? HIGH : LOW);	/*--mks--*/
		if(soft_pwm_BED > pwm_mask) WRITE_HEATER_BED(HIGH);
