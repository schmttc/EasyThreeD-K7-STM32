...
void MarlinSettings::postprocess() {
  ...
  calculate_volumetric_multipliers();
    ...
    for (uint8_t i=X_AXIS; i<=Z_AXIS; i++) update_software_endstops((AxisEnum)i);
  ...
  bool MarlinSettings::save() {
    ...
    write_data(eeprom_index, (uint8_t*)&ver, sizeof(ver));     
    eeprom_index += sizeof(eeprom_checksum); 

    eeprom_checksum = 0; 

    const uint8_t esteppers = (sizeof(planner . axis_steps_per_mm)/sizeof(*planner . axis_steps_per_mm)) - 3;
    write_data(eeprom_index, (uint8_t*)&esteppers, sizeof(esteppers));

    write_data(eeprom_index, (uint8_t*)&planner . axis_steps_per_mm, sizeof(planner . axis_steps_per_mm));
    write_data(eeprom_index, (uint8_t*)&planner . max_feedrate_mm_s, sizeof(planner . max_feedrate_mm_s));
    write_data(eeprom_index, (uint8_t*)&planner . max_acceleration_mm_per_s2, sizeof(planner . max_acceleration_mm_per_s2));

    write_data(eeprom_index, (uint8_t*)&planner . acceleration, sizeof(planner . acceleration));
    write_data(eeprom_index, (uint8_t*)&planner . retract_acceleration, sizeof(planner . retract_acceleration));
    write_data(eeprom_index, (uint8_t*)&planner . travel_acceleration, sizeof(planner . travel_acceleration));
    write_data(eeprom_index, (uint8_t*)&planner . min_feedrate_mm_s, sizeof(planner . min_feedrate_mm_s));
    write_data(eeprom_index, (uint8_t*)&planner . min_travel_feedrate_mm_s, sizeof(planner . min_travel_feedrate_mm_s));
    write_data(eeprom_index, (uint8_t*)&planner . min_segment_time, sizeof(planner . min_segment_time));
    write_data(eeprom_index, (uint8_t*)&planner . max_jerk, sizeof(planner . max_jerk));
    
	if(mksCfg . machinetype == 2) {
      dummy = 0.0;
      write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
      write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
      dummy = mksCfg . delta_height + home_offset[Z_AXIS];
      write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy)); }
    
    else
      write_data(eeprom_index, (uint8_t*)&home_offset, sizeof(home_offset));
    ...
    write_data(eeprom_index, (uint8_t*)&zfh, sizeof(zfh));
	...
	if(mksCfg . bed_leveling_method == 32)
      ...
      static_assert(
         ...
         sizeof(mbl.z_values) == (16) * (16) * sizeof(mbl.z_values[0][0]),
        "MBL Z array is the wrong size."
      );
      const bool leveling_is_on = (((mbl . status)&(1<<(MBL_STATUS_HAS_MESH_BIT)))!=0);
      const uint8_t mesh_num_x = mksCfg . grid_max_points_x, mesh_num_y = mksCfg . grid_max_points_y;
      write_data(eeprom_index, (uint8_t*)&leveling_is_on, sizeof(leveling_is_on));
      write_data(eeprom_index, (uint8_t*)&mbl . z_offset, sizeof(mbl . z_offset));
      write_data(eeprom_index, (uint8_t*)&mesh_num_x, sizeof(mesh_num_x));
      write_data(eeprom_index, (uint8_t*)&mesh_num_y, sizeof(mesh_num_y));
      write_data(eeprom_index, (uint8_t*)&mbl . z_values, sizeof(mbl . z_values));
    }else{
      ...
      write_data(eeprom_index, (uint8_t*)&mesh_num_x, sizeof(mesh_num_x));
      write_data(eeprom_index, (uint8_t*)&mesh_num_y, sizeof(mesh_num_y));
      for (uint8_t q = mesh_num_x * mesh_num_y; q--;) write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
    }


    write_data(eeprom_index, (uint8_t*)&zprobe_zoffset, sizeof(zprobe_zoffset));
	...
	if(mksCfg . bed_leveling_method&(4 | 2))
      write_data(eeprom_index, (uint8_t*)&planner . bed_level_matrix, sizeof(planner . bed_level_matrix));
    
	else
        {
      dummy = 0.0;
      for (uint8_t q = 9; q--;) write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
        }
	...
	if(mksCfg . bed_leveling_method == 8)
		...
		static_assert(
                    ...
                    sizeof(z_values) == (16) * (16) * sizeof(z_values[0][0]),
		  "Bilinear Z array is the wrong size."
		);
		 grid_max_x = mksCfg . grid_max_points_x;
		 grid_max_y = mksCfg . grid_max_points_y;
		write_data(eeprom_index, (uint8_t*)&grid_max_x, sizeof(grid_max_x));			 
		write_data(eeprom_index, (uint8_t*)&grid_max_y, sizeof(grid_max_y));			 
		write_data(eeprom_index, (uint8_t*)&bilinear_grid_spacing, sizeof(bilinear_grid_spacing)); 
		write_data(eeprom_index, (uint8_t*)&bilinear_start, sizeof(bilinear_start));		 
		write_data(eeprom_index, (uint8_t*)&z_values, sizeof(z_values)); 			 
		}
		else
			{
			grid_max_x = 3, grid_max_y = 3;
			
			bilinear_start[0] = bilinear_start[1] = 0;
			bilinear_start[0] = bilinear_start[1] =0;
			dummy = 0.0f;
			write_data(eeprom_index, (uint8_t*)&grid_max_x, sizeof(grid_max_x));
			write_data(eeprom_index, (uint8_t*)&grid_max_y, sizeof(grid_max_y));
			write_data(eeprom_index, (uint8_t*)&bilinear_start, sizeof(bilinear_start));
			write_data(eeprom_index, (uint8_t*)&bilinear_grid_spacing, sizeof(bilinear_grid_spacing));
			for (uint16_t q = grid_max_x * grid_max_y; q--;) write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
			}
	...
	if(mksCfg . bed_leveling_method == 16)	{
      write_data(eeprom_index, (uint8_t*)&ubl . state . active, sizeof(ubl . state . active));
      write_data(eeprom_index, (uint8_t*)&ubl . state . z_offset, sizeof(ubl . state . z_offset));
      write_data(eeprom_index, (uint8_t*)&ubl . state . eeprom_storage_slot, sizeof(ubl . state . eeprom_storage_slot));	}
    
	else {
      const bool ubl_active = 0;
      dummy = 0.0f;
      const int8_t eeprom_slot = -1;
      write_data(eeprom_index, (uint8_t*)&ubl_active, sizeof(ubl_active));
      write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
      write_data(eeprom_index, (uint8_t*)&eeprom_slot, sizeof(eeprom_slot));	}
    

    
    
	if(mksCfg . machinetype == 2)  {
      write_data(eeprom_index, (uint8_t*)&endstop_adj, sizeof(endstop_adj));               
      write_data(eeprom_index, (uint8_t*)&delta_radius, sizeof(delta_radius));              
      write_data(eeprom_index, (uint8_t*)&delta_diagonal_rod, sizeof(delta_diagonal_rod));        
      write_data(eeprom_index, (uint8_t*)&delta_segments_per_second, sizeof(delta_segments_per_second)); 
      write_data(eeprom_index, (uint8_t*)&delta_calibration_radius, sizeof(delta_calibration_radius));  
      write_data(eeprom_index, (uint8_t*)&delta_tower_angle_trim, sizeof(delta_tower_angle_trim));    
      dummy = 0.0f;
      for (uint8_t q = 3; q--;) write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy)); }
      ...
      dummy = 0.0f;
      for (uint8_t q = 12; q--;) write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));	}
    


    write_data(eeprom_index, (uint8_t*)&lcd_preheat_hotend_temp, sizeof(lcd_preheat_hotend_temp));
    write_data(eeprom_index, (uint8_t*)&lcd_preheat_bed_temp, sizeof(lcd_preheat_bed_temp));
    write_data(eeprom_index, (uint8_t*)&lcd_preheat_fan_speed, sizeof(lcd_preheat_fan_speed));

    for (uint8_t e = 0; e < 1; e++) {
	if(mksCfg . pidtemp){
		  if (e < 1) {
			write_data(eeprom_index, (uint8_t*)&Temperature::Kp, sizeof(Temperature::Kp));
			write_data(eeprom_index, (uint8_t*)&Temperature::Ki, sizeof(Temperature::Ki));
			write_data(eeprom_index, (uint8_t*)&Temperature::Kd, sizeof(Temperature::Kd));
			  dummy = 1.0f; 
			  write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
		  }
		  else
      ...
      int lpq_len = 20;
    write_data(eeprom_index, (uint8_t*)&lpq_len, sizeof(lpq_len));

	if(mksCfg . pidtempbed == 0)
		{
		dummy = 3000.0f;
		for (uint8_t q = 3; q--;) write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
		}
		...
		{
		write_data(eeprom_index, (uint8_t*)&thermalManager . bedKp, sizeof(thermalManager . bedKp));
		write_data(eeprom_index, (uint8_t*)&thermalManager . bedKi, sizeof(thermalManager . bedKi));
		write_data(eeprom_index, (uint8_t*)&thermalManager . bedKd, sizeof(thermalManager . bedKd));
		}
	 

      const int lcd_contrast = 32;
    write_data(eeprom_index, (uint8_t*)&lcd_contrast, sizeof(lcd_contrast));


    write_data(eeprom_index, (uint8_t*)&volumetric_enabled, sizeof(volumetric_enabled));

    
    for (uint8_t q = 0; q < 1; q++) {
      if (q < (sizeof(filament_size)/sizeof(*filament_size))) dummy = filament_size[q];
      write_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
    }
    ...
    uint16_t val;
      val = 0;
      for (uint8_t q = 0; q < 11; ++q) write_data(eeprom_index, (uint8_t*)&val, sizeof(val));
    ...
    if (!eeprom_write_error) {
      ...
      eeprom_index = 0;
      write_data(eeprom_index, (uint8_t*)&version, sizeof(version));
      write_data(eeprom_index, (uint8_t*)&final_checksum, sizeof(final_checksum));

      
      {serialprintPGM(echomagic);};
      {serial_echopair_P(("Settings Stored ("),(eeprom_size - (0)));};
      {serialprintPGM((" bytes)" "\n"));};
	  if(eeprom_size> (1536-1))
	  	{
	  	{serialprintPGM(("Error!!! EEPROM OVERFLOW!" "\n"));};
  ...
  bool MarlinSettings::load() {
    ...
    char stored_ver[4];
    read_data(eeprom_index, (uint8_t*)&stored_ver, sizeof(stored_ver));

    uint16_t stored_checksum;
    read_data(eeprom_index, (uint8_t*)&stored_checksum, sizeof(stored_checksum));
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      {serialprintPGM(echomagic);};
      {serialprintPGM(("EEPROM version mismatch "));};
      {serial_echopair_P(("(EEPROM="),(stored_ver));};
      {serialprintPGM((" Marlin=" "V37" ")" "\n"));};
      ...
      uint8_t esteppers;
      read_data(eeprom_index, (uint8_t*)&esteppers, sizeof(esteppers));

      
      
      const float def1[] = { 606, 606, 1167, 1040}, def2[] = { 40, 40, 10, 70 };
      const uint32_t def3[] = { 1000, 1000, 100, 1000 };
      ...
      float *tmp2 = 0;
      uint32_t *tmp3 = 0;
      tmp1 = (float *)malloc((3 + esteppers) * sizeof(float));
      tmp2 = (float *)malloc((3 + esteppers) * sizeof(float));
      tmp3 = (uint32_t *)malloc((3 + esteppers) * sizeof(uint32_t));
      read_data(eeprom_index, (uint8_t *)tmp1, (3 + esteppers) * sizeof(float));
      read_data(eeprom_index, (uint8_t *)tmp2, (3 + esteppers) * sizeof(float));
      read_data(eeprom_index, (uint8_t *)tmp3, (3 + esteppers) * sizeof(uint32_t));

      for (uint8_t i=X_AXIS; i<4; i++) {
        planner.axis_steps_per_mm[i]          = i < 3 + esteppers ? *(tmp1+i) : def1[i < (sizeof(def1)/sizeof(*def1)) ? i : (sizeof(def1)/sizeof(*def1)) - 1];
        planner.max_feedrate_mm_s[i]          = i < 3 + esteppers ? *(tmp2+i) : def2[i < (sizeof(def2)/sizeof(*def2)) ? i : (sizeof(def2)/sizeof(*def2)) - 1];
        planner.max_acceleration_mm_per_s2[i] = i < 3 + esteppers ? *(tmp3+i) : def3[i < (sizeof(def3)/sizeof(*def3)) ? i : (sizeof(def3)/sizeof(*def3)) - 1];
      }

      read_data(eeprom_index, (uint8_t*)&planner . acceleration, sizeof(planner . acceleration));
      read_data(eeprom_index, (uint8_t*)&planner . retract_acceleration, sizeof(planner . retract_acceleration));
      read_data(eeprom_index, (uint8_t*)&planner . travel_acceleration, sizeof(planner . travel_acceleration));
      read_data(eeprom_index, (uint8_t*)&planner . min_feedrate_mm_s, sizeof(planner . min_feedrate_mm_s));
      read_data(eeprom_index, (uint8_t*)&planner . min_travel_feedrate_mm_s, sizeof(planner . min_travel_feedrate_mm_s));
      read_data(eeprom_index, (uint8_t*)&planner . min_segment_time, sizeof(planner . min_segment_time));
      read_data(eeprom_index, (uint8_t*)&planner . max_jerk, sizeof(planner . max_jerk));

      read_data(eeprom_index, (uint8_t*)&home_offset, sizeof(home_offset));

     
	  if(mksCfg . machinetype == 2)	{
        home_offset[X_AXIS] = 0.0;
        home_offset[Y_AXIS] = 0.0;
        home_offset[Z_AXIS] -= mksCfg . delta_height;	}
		...
		if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))
        	read_data(eeprom_index, (uint8_t*)&planner . z_fade_height, sizeof(planner . z_fade_height));
		else
			read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
      ...
      read_data(eeprom_index, (uint8_t*)&leveling_is_on, sizeof(leveling_is_on));
      read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
      read_data(eeprom_index, (uint8_t*)&mesh_num_x, sizeof(mesh_num_x));
      read_data(eeprom_index, (uint8_t*)&mesh_num_y, sizeof(mesh_num_y));

	   
      
		if(mksCfg . bed_leveling_method == 32) {
        mbl.status = leveling_is_on ? (1<<(MBL_STATUS_HAS_MESH_BIT)) : 0;
        mbl.z_offset = dummy;
        if (mesh_num_x == mksCfg . grid_max_points_x && mesh_num_y == mksCfg . grid_max_points_y) {
          
          read_data(eeprom_index, (uint8_t*)&mbl . z_values, sizeof(mbl . z_values));
        }
        else {
          
          mbl.reset();
          for (uint16_t q = mesh_num_x * mesh_num_y; q--;) read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
        }
      }else{
      ...
      read_data(eeprom_index, (uint8_t*)&zprobe_zoffset, sizeof(zprobe_zoffset));
	  ...
	  if(mksCfg . bed_leveling_method&(4 | 2))
        read_data(eeprom_index, (uint8_t*)&planner . bed_level_matrix, sizeof(planner . bed_level_matrix));
      
      else
        for (uint8_t q = 9; q--;) read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
      ...
      uint8_t grid_max_x, grid_max_y;
      read_data(eeprom_index, (uint8_t*)&grid_max_x, sizeof(grid_max_x));                       
      read_data(eeprom_index, (uint8_t*)&grid_max_y, sizeof(grid_max_y));                       
	...
	if(mksCfg . bed_leveling_method == 8)
		{
        if (grid_max_x == mksCfg . grid_max_points_x && grid_max_y == mksCfg . grid_max_points_y) {
          set_bed_leveling_enabled(false);
          read_data(eeprom_index, (uint8_t*)&bilinear_grid_spacing, sizeof(bilinear_grid_spacing));        
          read_data(eeprom_index, (uint8_t*)&bilinear_start, sizeof(bilinear_start));               
          read_data(eeprom_index, (uint8_t*)&z_values, sizeof(z_values));                     
        }
        else 
			{
			  
			  read_data(eeprom_index, (uint8_t*)&bgs, sizeof(bgs));
			  read_data(eeprom_index, (uint8_t*)&bs, sizeof(bs));
			  for (uint16_t q = grid_max_x * grid_max_y; q--;) read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
			}
		}
		else
			{
	  ...
	  if(mksCfg . bed_leveling_method == 16)	{
        read_data(eeprom_index, (uint8_t*)&ubl . state . active, sizeof(ubl . state . active));
        read_data(eeprom_index, (uint8_t*)&ubl . state . z_offset, sizeof(ubl . state . z_offset));
        read_data(eeprom_index, (uint8_t*)&ubl . state . eeprom_storage_slot, sizeof(ubl . state . eeprom_storage_slot)); }
      
	  else {
        bool dummyb;
        uint8_t dummyui8;
        read_data(eeprom_index, (uint8_t*)&dummyb, sizeof(dummyb));
        read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
        read_data(eeprom_index, (uint8_t*)&dummyui8, sizeof(dummyui8)); }
     

     
	  if(mksCfg . machinetype == 2)	{
        read_data(eeprom_index, (uint8_t*)&endstop_adj, sizeof(endstop_adj));               
        read_data(eeprom_index, (uint8_t*)&delta_radius, sizeof(delta_radius));              
        read_data(eeprom_index, (uint8_t*)&delta_diagonal_rod, sizeof(delta_diagonal_rod));        
        read_data(eeprom_index, (uint8_t*)&delta_segments_per_second, sizeof(delta_segments_per_second)); 
        read_data(eeprom_index, (uint8_t*)&delta_calibration_radius, sizeof(delta_calibration_radius));  
        read_data(eeprom_index, (uint8_t*)&delta_tower_angle_trim, sizeof(delta_tower_angle_trim));    
        dummy = 0.0f;
        for (uint8_t q=3; q--;) read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));	}
        ...
        dummy = 0.0f;
        for (uint8_t q=12; q--;) read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));	}
      


      read_data(eeprom_index, (uint8_t*)&lcd_preheat_hotend_temp, sizeof(lcd_preheat_hotend_temp));
      read_data(eeprom_index, (uint8_t*)&lcd_preheat_bed_temp, sizeof(lcd_preheat_bed_temp));
      read_data(eeprom_index, (uint8_t*)&lcd_preheat_fan_speed, sizeof(lcd_preheat_fan_speed));
      ...
      if(mksCfg . pidtemp){
        for (uint8_t e = 0; e < 1; e++) {
          read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy)); 
          if (e < 1 && dummy != 3000.0f) {
            
            Temperature::Kp = dummy;
            read_data(eeprom_index, (uint8_t*)&Temperature::Ki, sizeof(Temperature::Ki));
            read_data(eeprom_index, (uint8_t*)&Temperature::Kd, sizeof(Temperature::Kd));
              read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
          }
          else {
            for (uint8_t q=3; q--;) read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy)); 
        ...
        for (uint8_t q = 1 * 4; q--;) read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));  
      }
	 

	
        int lpq_len;
      read_data(eeprom_index, (uint8_t*)&lpq_len, sizeof(lpq_len));
	...
	if(mksCfg . pidtempbed)
        ...
        if (dummy != 3000.0f) {
          ...
          read_data(eeprom_index, (uint8_t*)&thermalManager . bedKi, sizeof(thermalManager . bedKi));
          read_data(eeprom_index, (uint8_t*)&thermalManager . bedKd, sizeof(thermalManager . bedKd));
        	}
		}
	else
		{
        ...
        int lcd_contrast;
      read_data(eeprom_index, (uint8_t*)&lcd_contrast, sizeof(lcd_contrast));


      read_data(eeprom_index, (uint8_t*)&volumetric_enabled, sizeof(volumetric_enabled));

      for (uint8_t q = 0; q < 1; q++) {
        read_data(eeprom_index, (uint8_t*)&dummy, sizeof(dummy));
        if (q < (sizeof(filament_size)/sizeof(*filament_size))) filament_size[q] = dummy;
      }

      uint16_t val;
        for (uint8_t q = 0; q < 11; q++) read_data(eeprom_index, (uint8_t*)&val, sizeof(val));
      ...
      if (eeprom_checksum == stored_checksum) {
        if (eeprom_read_error)
        	{
			   uint8_t eppr_w1 = 0;
			   AT24CXX_Write(0, (uint8_t *)&eppr_w1,1);
        	
                Running = false;
                kill("eeprom_read_error");
          		reset();
        	}
        else {
          postprocess();
          {serialprintPGM(echomagic);};
          {if(!from_wifi_flag){customizedSerial . print(version);}else{serial2 . print(version);}};
          {serial_echopair_P((" stored settings retrieved ("),(eeprom_index - (0)));};
          {serialprintPGM((" bytes)" "\n"));};
        }
      }
      else 
	  	{
		 uint8_t eppr_w2 = 0;
		 AT24CXX_Write(0, (uint8_t *)&eppr_w2,1);
        (serialprintPGM(errormagic));
        {serialprintPGM(("EEPROM checksum mismatch" "\n"));};
        Running = false;
        kill("EEPROM checksum mis!");
        reset();
      }

      
	  if(mksCfg . bed_leveling_method == 16) 	{
        ubl.eeprom_start = (eeprom_index + 32) & 0xFFF8; 
                                                         
                                                         
        {serialprintPGM((" UBL "));};
        if (!ubl.state.active) {if(!from_wifi_flag){customizedSerial . print("not ");}else{serial2 . print("not ");}};
        {serialprintPGM(("active!" "\n"));};

        if (!ubl.sanity_check()) {
          {serialprintPGM(("\nUnified Bed Leveling system initialized.\n" "\n"));};
        }
        else {
          {serialprintPGM(("?Unable to enable Unified Bed Leveling system.\n"));};
          ubl.reset();
        }

        if (ubl.state.eeprom_storage_slot >= 0) {
          ubl.load_mesh(ubl.state.eeprom_storage_slot);
          {serial_echopair_P(("Mesh "),(ubl . state . eeprom_storage_slot));};
          {serialprintPGM((" loaded from storage." "\n"));};
        }
        else {
          ubl.reset();
          {serialprintPGM(("UBL System reset()" "\n"));};
        }
	  	}
...
void MarlinSettings::reset() {
  ...
  const float tmp1[] = { 606, 606, 1167, 1040}, tmp2[] = { 40, 40, 10, 70 };
  const uint32_t tmp3[] = { 1000, 1000, 100, 1000 };
  for (uint8_t i=X_AXIS; i<4; i++) {
    planner.axis_steps_per_mm[i]          = tmp1[i < (sizeof(tmp1)/sizeof(*tmp1)) ? i : (sizeof(tmp1)/sizeof(*tmp1)) - 1];
    planner.max_feedrate_mm_s[i]          = tmp2[i < (sizeof(tmp2)/sizeof(*tmp2)) ? i : (sizeof(tmp2)/sizeof(*tmp2)) - 1];
    planner.max_acceleration_mm_per_s2[i] = tmp3[i < (sizeof(tmp3)/sizeof(*tmp3)) ? i : (sizeof(tmp3)/sizeof(*tmp3)) - 1];
  }

  planner.acceleration = 1000;
  planner.retract_acceleration = 1000;
  planner.travel_acceleration = 1000;
  planner.min_feedrate_mm_s = 0.0;
  planner.min_segment_time = 20000;
  planner.min_travel_feedrate_mm_s = 0.0;
  planner.max_jerk[X_AXIS] = 2.0;
  planner.max_jerk[Y_AXIS] = 2.0;
  planner.max_jerk[Z_AXIS] = 0.4;
  planner.max_jerk[E_AXIS] = 0.5;
  ...
  if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))	
    ...
    memset(home_offset,0,sizeof(home_offset));
  ...
  if(mksCfg . machinetype == 2) {
    const float adj[3] = { 0, 0, 0 },
                dta[3] = { 0, 0, 0 };
    memcpy(endstop_adj,adj,(((sizeof(endstop_adj))<(sizeof(adj)))?(sizeof(endstop_adj)):(sizeof(adj))));
    delta_radius = (0 - 0 - 0);
    delta_diagonal_rod = 0;
    delta_segments_per_second = 0;
    delta_calibration_radius = 0;
    ...
    lcd_preheat_hotend_temp[0] = 180;
    lcd_preheat_hotend_temp[1] = 240;
    lcd_preheat_bed_temp[0] = 70;
    lcd_preheat_bed_temp[1] = 110;
    lcd_preheat_fan_speed[0] = 0;
    lcd_preheat_fan_speed[1] = 0;
    ...
    {
      Temperature::Kp = 22.2;
      Temperature::Ki = ( (1.08) * 0.16384 );
      Temperature::Kd = ( (114) / 0.16384 );
    }
  }

  
  if(mksCfg . pidtempbed) {
    thermalManager.bedKp = 10.00;
    thermalManager.bedKi = ( (.023) * 0.16384 );
    thermalManager.bedKd = ( (305.4) / 0.16384 );
  }
  ...
  volumetric_enabled =
      false
  ;
  for (uint8_t q = 0; q < (sizeof(filament_size)/sizeof(*filament_size)); q++)
    filament_size[q] = 3.00;
  ...
  {serialprintPGM(echomagic);};
  {serialprintPGM(("Hardcoded Default Settings Loaded" "\n"));};
}
  ...
  void MarlinSettings::report(bool forReplay) {
    ...
    do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("  G21 ; Units in mm\n" "\n"));};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

      

      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("  M149 C ; Units in Celsius\n" "\n"));};
      {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    ...
    if (!forReplay) {
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("Filament settings:"));};
      if (volumetric_enabled)
      {  {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};}
      else
        {serialprintPGM((" Disabled" "\n"));};
    }

    do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
    {serial_echopair_P(("  M200 D"),(filament_size[0]));};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

    if (!volumetric_enabled) {
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("  M200 D0" "\n"));};
    }

    if (!forReplay) {
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("Steps per unit:" "\n"));};
    }
    do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
    {serial_echopair_P(("  M92 X"),(planner . axis_steps_per_mm[X_AXIS]));};
    {serial_echopair_P((" Y"),(planner . axis_steps_per_mm[Y_AXIS]));};
    {serial_echopair_P((" Z"),(planner . axis_steps_per_mm[Z_AXIS]));};
      {serial_echopair_P((" E"),(planner . axis_steps_per_mm[E_AXIS]));};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

    if (!forReplay) {
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("Maximum feedrates (units/s):" "\n"));};
    }
    do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
    {serial_echopair_P(("  M203 X"),(planner . max_feedrate_mm_s[X_AXIS]));};
    {serial_echopair_P((" Y"),(planner . max_feedrate_mm_s[Y_AXIS]));};
    {serial_echopair_P((" Z"),(planner . max_feedrate_mm_s[Z_AXIS]));};
      {serial_echopair_P((" E"),(planner . max_feedrate_mm_s[E_AXIS]));};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

    if (!forReplay) {
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("Maximum Acceleration (units/s2):" "\n"));};
    }
    do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
    {serial_echopair_P(("  M201 X"),(planner . max_acceleration_mm_per_s2[X_AXIS]));};
    {serial_echopair_P((" Y"),(planner . max_acceleration_mm_per_s2[Y_AXIS]));};
    {serial_echopair_P((" Z"),(planner . max_acceleration_mm_per_s2[Z_AXIS]));};
      {serial_echopair_P((" E"),(planner . max_acceleration_mm_per_s2[E_AXIS]));};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

    if (!forReplay) {
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("Acceleration (units/s2): P<print_accel> R<retract_accel> T<travel_accel>" "\n"));};
    }
    do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
    {serial_echopair_P(("  M204 P"),(planner . acceleration));};
    {serial_echopair_P((" R"),(planner . retract_acceleration));};
    do{ {serial_echopair_P((" T"),(planner . travel_acceleration));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

    if (!forReplay) {
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serialprintPGM(("Advanced: S<min_feedrate> T<min_travel_feedrate> B<min_segment_time_ms> X<max_xy_jerk> Z<max_z_jerk> E<max_e_jerk>" "\n"));};
    }
    do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
    {serial_echopair_P(("  M205 S"),(planner . min_feedrate_mm_s));};
    {serial_echopair_P((" T"),(planner . min_travel_feedrate_mm_s));};
    {serial_echopair_P((" B"),(planner . min_segment_time));};
    {serial_echopair_P((" X"),(planner . max_jerk[X_AXIS]));};
    {serial_echopair_P((" Y"),(planner . max_jerk[Y_AXIS]));};
    {serial_echopair_P((" Z"),(planner . max_jerk[Z_AXIS]));};
    do{ {serial_echopair_P((" E"),(planner . max_jerk[E_AXIS]));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

    
	if(mksCfg . machinetype != 2){
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Home offset:" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serial_echopair_P(("  M206 X"),(home_offset[X_AXIS]));};
      {serial_echopair_P((" Y"),(home_offset[Y_AXIS]));};
      do{ {serial_echopair_P((" Z"),(home_offset[Z_AXIS]));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    }
      ...
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Mesh Bed Leveling:" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serial_echopair_P(("  M420 S"),(mbl . has_mesh() ? 1 : 0));};
  
  	if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))	
        {serial_echopair_P((" Z"),(planner . z_fade_height));};
  
      {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
      for (uint8_t py = 0; py < mksCfg . grid_max_points_y; py++) {
        for (uint8_t px = 0; px < mksCfg . grid_max_points_x; px++) {
          do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
          {serial_echopair_P(("  G29 S3 X"),((int)px + 1));};
          {serial_echopair_P((" Y"),((int)py + 1));};
          {serialprintPGM((" Z"));};
          {if(!from_wifi_flag){customizedSerial . print(mbl . z_values[px][py],5);}else{serial2 . print(mbl . z_values[px][py],5);}};
          {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
        }
      }
	  	}
      ...
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Unified Bed Leveling:" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serial_echopair_P(("  M420 S"),(ubl . state . active ? 1 : 0));};
      ...
      if (!forReplay) {
        ...
        if(ubl.state.active)
          {if(!from_wifi_flag){customizedSerial . write('A');}else{serial2 . print('A');}}
         else
          {serialprintPGM(("Ina"));};
        do{ {serial_echopair_P(("ctive\n\nActive Mesh Slot: "),(ubl . state . eeprom_storage_slot));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

        {serialprintPGM(("z_offset: "));};
        {if(!from_wifi_flag){customizedSerial . print(ubl . state . z_offset,6);}else{serial2 . print(ubl . state . z_offset,6);}};
        {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

        {serial_echopair_P(("EEPROM can hold "),((int)(((1536-1) - ubl . eeprom_start) / sizeof(ubl . z_values))));};
        {serialprintPGM((" meshes.\n" "\n"));};

        do{ {serial_echopair_P(("GRID_MAX_POINTS_X  "),(mksCfg . grid_max_points_x));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
        do{ {serial_echopair_P(("GRID_MAX_POINTS_Y  "),(mksCfg . grid_max_points_y));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

        {serialprintPGM(("UBL_MESH_MIN_X  " "(mksCfg.x_min_pos + mksCfg.ubl_mesh_inset)"));};
        do{ {serial_echopair_P(("="),((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
        {serialprintPGM(("UBL_MESH_MIN_Y  " "(mksCfg.y_min_pos + mksCfg.ubl_mesh_inset)"));};
        do{ {serial_echopair_P(("="),((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

        {serialprintPGM(("UBL_MESH_MAX_X  " "(mksCfg.x_max_pos - (mksCfg.ubl_mesh_inset))"));};
        do{ {serial_echopair_P(("="),((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset))));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
        {serialprintPGM(("UBL_MESH_MAX_Y  " "(mksCfg.y_max_pos - (mksCfg.ubl_mesh_inset))"));};
        do{ {serial_echopair_P(("="),((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset))));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

        do{ {serial_echopair_P(("MESH_X_DIST  "),((float((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_x - 1))));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
        do{ {serial_echopair_P(("MESH_Y_DIST  "),((float((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_y - 1))));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
        {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
      }
		}

	else if(mksCfg . bed_leveling_method&((4 | 2) | (4 | 8) | 16))	{
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Auto Bed Leveling:" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serial_echopair_P(("  M420 S"),(planner . abl_enabled ? 1 : 0));};
	...
	if(mksCfg . machinetype == 2)	{
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Endstop adjustment:" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serial_echopair_P(("  M666 X"),(endstop_adj[X_AXIS]));};
      {serial_echopair_P((" Y"),(endstop_adj[Y_AXIS]));};
      do{ {serial_echopair_P((" Z"),(endstop_adj[Z_AXIS]));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Delta settings: L<diagonal_rod> R<radius> H<height> S<segments_per_s> B<calibration radius> XYZ<tower angle corrections>" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      {serial_echopair_P(("  M665 L"),(delta_diagonal_rod));};
      {serial_echopair_P((" R"),(delta_radius));};
      {serial_echopair_P((" H"),(mksCfg . delta_height + home_offset[Z_AXIS]));};
      {serial_echopair_P((" S"),(delta_segments_per_second));};
      {serial_echopair_P((" B"),(delta_calibration_radius));};
      {serial_echopair_P((" X"),(delta_tower_angle_trim[A_AXIS]));};
      {serial_echopair_P((" Y"),(delta_tower_angle_trim[B_AXIS]));};
      {serial_echopair_P((" Z"),(0.00));};
      {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};		}
      ...
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Material heatup parameters:" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      for (uint8_t i = 0; i < (sizeof(lcd_preheat_hotend_temp)/sizeof(*lcd_preheat_hotend_temp)); i++) {
        {serial_echopair_P(("  M145 S"),((int)i));};
        {serial_echopair_P((" H"),(lcd_preheat_hotend_temp[i]));};
        {serial_echopair_P((" B"),(lcd_preheat_bed_temp[i]));};
        do{ {serial_echopair_P((" F"),(lcd_preheat_fan_speed[i]));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
      }
...
if(mksCfg . pidtemp){
        ...
        {
          do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
          {serial_echopair_P(("  M301 P"),(Temperature::Kp));}; 
      ...
      if(mksCfg . pidtempbed) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serial_echopair_P(("  M304 P"),(thermalManager . bedKp));};
        {serial_echopair_P((" I"),(( (thermalManager . bedKi) / 0.16384 )));};
        {serial_echopair_P((" D"),(( (thermalManager . bedKd) * 0.16384 )));};
        {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
      }
      ...
      if (!forReplay) {
        do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
        {serialprintPGM(("Z-Probe Offset (mm):" "\n"));};
      }
      do{ if (!forReplay) {serialprintPGM(echomagic);}; }while(0);
      do{ {serial_echopair_P(("  M851 Z"),(zprobe_zoffset));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
