...
const size_t EspFlashBlockSize = 0x0400;			

const uint8_t ESP_IMAGE_MAGIC = 0xe9;
const uint8_t ESP_CHECKSUM_MAGIC = 0xef;

const uint32_t ESP_ERASE_CHIP_ADDR = 0x40004984;	
const uint32_t ESP_SEND_PACKET_ADDR = 0x40003c80;	
const uint32_t ESP_SPI_READ_ADDR = 0x40004b1c;		
const uint32_t ESP_UNKNOWN_ADDR = 0x40001121;		
const uint32_t ESP_USER_DATA_RAM_ADDR = 0x3ffe8000;	
const uint32_t ESP_IRAM_ADDR = 0x40100000;			
const uint32_t ESP_FLASH_ADDR = 0x40200000;			
...
static const unsigned int percentToReportIncrement = 5;	
...
{
	BSP_UartIfTxData(serial2 .getUartStack(), (uint8_t *)buf, len);	
}

char uploadPort_read()
{
	uint8_t retChar;
	serial2 .readNbytes(&retChar, 1);
	return retChar;
...
{
	return serial2 .availableForRead() ;
}
...
EspUploadResult readPacket(uint8_t op, uint32_t *valp, size_t& bodyLen, uint32_t msTimeout)
{
	...
	while (state != done)
	{
		...
		if (HAL_GetTick() - startTime > msTimeout)
		{
			return(timeout);
		}
		...
		case body:		
			{
				...
				else
				{
					...
					if (bodyIdx < sizeof(respBuf) / sizeof((respBuf)[0]))
					{
						respBuf[bodyIdx] = c;
					}
					++bodyIdx;
...
void writePacket(const uint8_t *hdr, size_t hdrLen, const uint8_t *data, size_t dataLen)
{
	...
	WriteByteRaw(0xc0);				
	writePacket(hdr, hdrLen);		
	writePacket(data, dataLen);		
	WriteByteRaw(0xc0);				
}
...
{
	WriteByteRaw(0xc0);				
	writePacketRaw(hdr, hdrLen);	
	writePacketRaw(data, dataLen);	
	WriteByteRaw(0xc0);				
}
...
void upload_spin()
{
	...
	case upload_resetting:
		...
		else
		{
			...
			esp_upload.lastAttemptTime = esp_upload.lastResetTime = HAL_GetTick();
			esp_upload.state = upload_connecting;
		}
		break;

	case upload_connecting:
		if (HAL_GetTick() - esp_upload.lastAttemptTime >= connectAttemptInterval && HAL_GetTick() - esp_upload.lastResetTime >= 500)
		{
			
			EspUploadResult res = Sync(5000);
			esp_upload.lastAttemptTime = HAL_GetTick();
			if (res == success)
			{
	...
	case upload_erasing:
		if (HAL_GetTick() - esp_upload.lastAttemptTime >= blockWriteInterval)
		{
			uint32_t eraseSize;
	...
	case upload_uploading:
		...
		if (HAL_GetTick() - esp_upload.lastAttemptTime >= 15)
		{
...
void ResetWiFiForUpload(int begin_or_end)
{
	...
	GPIO_InitStructure.Speed = ((0x3U << (0U)));
	GPIO_InitStructure.Pin = ((uint16_t)0x0080);
	GPIO_InitStructure.Mode = ((uint32_t)0x00000001);   
	HAL_GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), &GPIO_InitStructure);
	
	
        
	if(begin_or_end == 0)
	{
		HAL_GPIO_WritePin(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), ((uint16_t)0x0080), GPIO_PIN_RESET);
	}
	else
	{
		HAL_GPIO_WritePin(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), ((uint16_t)0x0080), GPIO_PIN_SET);
	...
	if(begin_or_end == 1)
	{
		GPIO_InitStructure.Speed = ((0x3U << (0U)));
		GPIO_InitStructure.Pin = ((uint16_t)0x0080);
		GPIO_InitStructure.Mode = ((uint32_t)0x00000000);   
		HAL_GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), &GPIO_InitStructure);
	}
...
int32_t wifi_upload(int type)
{
	esp_upload.retriesPerBaudRate = 3; 
	...
	if(type == 0) 
	{
		SendUpdateFile("/MksWifi.bin", FirmwareAddress);
	}
	else if(type == 1) 
	{
		SendUpdateFile("/MksWifi_Web.bin", FirmwareAddress);
	}
	else if(type == 2) 
	{
		SendUpdateFile("/MksWifi_WebView.bin", WebFilesAddress);
	}
	else
		return -1;

	lcd_setstatuspgm(("Wifi updating..."));
