...
bool Stopwatch::stop() {
  ...
  if (this->isRunning() || this->isPaused()) {
    this->state = STOPPED;
    this->stopTimestamp = HAL_GetTick();
    return true;
  }
  else return false;
}
...
bool Stopwatch::start() {
  ...
  if (!this->isRunning()) {
    ...
    this->state = RUNNING;
    this->startTimestamp = HAL_GetTick();
    return true;
  }
  else return false;
}
...
millis_t Stopwatch::duration() {
  return (((this->isRunning()) ? HAL_GetTick() : this->stopTimestamp)
          - this->startTimestamp) / 1000UL + this->accumulator;
}


void Stopwatch::getTime(millis_t *accu,millis_t *startTime,millis_t *stopTime)
{
	*accu = accumulator;
...
void Stopwatch::setTime(millis_t accu,millis_t startTime,millis_t stopTime)
{
	accumulator = accu;
