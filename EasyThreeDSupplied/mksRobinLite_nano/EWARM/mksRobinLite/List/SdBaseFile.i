...
int8_t SdBaseFile::lsPrintNext(uint8_t flags, uint8_t indent) {
  ...
  for (uint8_t i = 0; i < indent; i++) customizedSerial .write(' ');

  
  for (uint8_t i = 0; i < 11; i++) {
    if (dir.name[i] == ' ')continue;
    if (i == 8) {
      customizedSerial .write('.');
      w++;
    }
    customizedSerial .write(dir.name[i]);
    w++;
  }
  ...
  if (flags & (LS_DATE | LS_SIZE)) {
    while (w++ < 14) customizedSerial .write(' ');
  }
  ...
  if (!DIR_IS_SUBDIR(&dir) && (flags & LS_SIZE)) {
    customizedSerial .write(' ');
    customizedSerial .print(dir.fileSize);
  }
...
bool SdBaseFile::make83Name(const char* str, uint8_t* name, const char** ptr) {
  ...
  while (*str != '\0' && *str != '/') {
    ...
    if (c == '.') {
      ...
      const char * p = ("|<>^+=?/[];,*\"\\");
      uint8_t b;
      while ((b = (*(const unsigned char *)(p++)))) if (b == c) goto fail;
...
bool SdBaseFile::openParent(SdBaseFile* dir) {
  ...
  do {
    if (file.readDir(&entry, 0) != 32) goto fail;
...
bool SdBaseFile::openRoot(SdVolume* vol) {
  ...
  if (vol->fatType() == 16 || (0 && vol->fatType() == 12)) {
...
static void print2u(uint8_t v) {
  if (v < 10) customizedSerial .write('0');
  customizedSerial .print(v, 10);
}
...
void SdBaseFile::printFatDate(uint16_t fatDate) {
  customizedSerial .print(FAT_YEAR(fatDate));
  customizedSerial .write('-');
...
int16_t SdBaseFile::read(void* buf, uint16_t nbyte) {
  ...
  do{ if (nbyte > fileSize_ - curPosition_) nbyte = fileSize_ - curPosition_; }while(0);
  ...
  while (toRead > 0) {
    ...
    do{ if (n > 512 - offset) n = 512 - offset; }while(0);
...
int8_t SdBaseFile::readDir(dir_t* dir, char* longFilename) {
  ...
  if (longFilename != 0) longFilename[0] = '\0';
  ...
  while (1) {
    ...
    if (longFilename != 0 && DIR_IS_LONG_NAME(dir)) {
      vfat_t* VFAT = (vfat_t*)dir;
      
      if (VFAT->firstClusterLow == 0 && (VFAT->sequenceNumber & 0x1F) > 0 && (VFAT->sequenceNumber & 0x1F) <= (2)) {
        
        n = ((VFAT->sequenceNumber & 0x1F) - 1) * (13);
        for (uint8_t i = 0; i < 13; i++)
          longFilename[n + i] = (i < 5) ? VFAT->name1[i] : (i < 11) ? VFAT->name2[i - 5] : VFAT->name3[i - 11];
        
        if (VFAT->sequenceNumber & 0x40) longFilename[n + 13] = '\0';
      }
    }
...
int16_t SdBaseFile::write(const void* buf, uint16_t nbyte) {
  ...
  while (nToWrite > 0) {
    ...
    do{ if (n > nToWrite) n = nToWrite; }while(0);
