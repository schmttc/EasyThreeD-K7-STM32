...
static uint8_t pg_prepare(pg_struct *pg)
{
  ...
  pg->pge[1].next_idx_fn = pg_inc;
  pg->pge[0].next_idx_fn = pg_dec;
  ...
  pg->pge[1].curr_idx = pg->pge[0].curr_idx;  
  pg_expand_min_y(pg, min_y, 1);
  pg_expand_min_y(pg, min_y, 0);
  
   
  pg->is_min_y_not_flat = 1;
  if ( pg->list[pg->pge[0].curr_idx].x != pg->list[pg->pge[1].curr_idx].x )
  {
...
{
  pg_word_t x1, x2, y;
  x1 = pg->pge[0].current_x;
  x2 = pg->pge[1].current_x;
  y = pg->pge[1].current_y;
...
static void pg_exec(pg_struct *pg, u8g_t *u8g)
{
  ...
  {
    pg_hline(pg, u8g);
    while ( pge_Next(&(pg->pge[0])) == 0 )
    {
      pg_line_init(pg, 0);
    }
    while ( pge_Next(&(pg->pge[1])) == 0 )
    {
      pg_line_init(pg, 1);
    }
    i--;
