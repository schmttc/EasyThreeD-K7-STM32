...
void MX_ADC1_Init(void);
  ...
  static void* heater_ttbl_map[1] = { (void*)temptable_1 };
  static uint8_t heater_ttbllen_map[1] = { (sizeof(temptable_1)/sizeof(*temptable_1)) };

Temperature thermalManager;



float Temperature::current_temperature[1] = { 0.0 },
      Temperature::current_temperature_bed = 0.0;

int Temperature::current_temperature_raw[1] = { 0 },
        Temperature::target_temperature[1] = { 0 },
...
uint8_t Temperature::soft_pwm_bed;
    ...
    float Temperature::Kp = 22.2,
          Temperature::Ki = (1.08) * (0.16384),
          Temperature::Kd = (114) / (0.16384);


  float Temperature::bedKp = 10.00,
        Temperature::bedKi = ((.023) * 0.16384),
        Temperature::bedKd = ((305.4) / 0.16384);


  int Temperature::watch_target_temp[1] = { 0 };
  millis_t Temperature::watch_heater_next_ms[1] = { 0 };

  int Temperature::watch_target_bed_temp = 0;
  millis_t Temperature::watch_bed_next_ms = 0;

  bool Temperature::allow_cold_extrude = false;
  float Temperature::extrude_min_temp = mksCfg . extrude_mintemp;
...
volatile bool Temperature::temp_meas_ready = false;
  ...
  float Temperature::temp_iState[1] = { 0 },
        Temperature::temp_dState[1] = { 0 },
        Temperature::pTerm[1],
        Temperature::iTerm[1],
        Temperature::dTerm[1];


  float Temperature::pid_error[1];
  bool Temperature::pid_reset[1];
...
uint16_t Temperature::raw_temp_value[1] = { 0 },
         Temperature::raw_temp_bed_value = 0;


int16_t Temperature::minttemp_raw[1] = { 16383 },
        Temperature::maxttemp_raw[1] = { 0 },
        Temperature::minttemp[1] = { 0 },
        Temperature::maxttemp[1] = { 16383 };



  int16_t Temperature::bed_minttemp_raw = 16383;

  int16_t Temperature::bed_maxttemp_raw = 0;



uint8_t Temperature::soft_pwm[1];
  ...
  void Temperature::PID_autotune(float temp, int hotend, int ncycles, bool set_result ) {
    ...
    millis_t temp_ms = HAL_GetTick(), t1 = temp_ms, t2 = temp_ms;
	...
	if(mksCfg . pidtemp == 0 && mksCfg . pidtempbed == 0) if(hotend >= 0 || hotend < 0) {{do{ if(!from_wifi_flag){customizedSerial . print("PID Autotune" " failed!" " Bad extruder number");}else{serial2 . print("PID Autotune" " failed!" " Bad extruder number");} {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);};return;}
	if(mksCfg . pidtemp == 0 && mksCfg . pidtempbed == 1) if(hotend >= 0 || hotend < -1) {{do{ if(!from_wifi_flag){customizedSerial . print("PID Autotune" " failed!" " Bad extruder number");}else{serial2 . print("PID Autotune" " failed!" " Bad extruder number");} {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);};return;}
	if(mksCfg . pidtemp == 1 && mksCfg . pidtempbed == 0) if(hotend >= 1|| hotend < 0) {{do{ if(!from_wifi_flag){customizedSerial . print("PID Autotune" " failed!" " Bad extruder number");}else{serial2 . print("PID Autotune" " failed!" " Bad extruder number");} {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);};return;}
	if(mksCfg . pidtemp == 1 && mksCfg . pidtempbed == 1) if(hotend >= 1|| hotend < -1) {{do{ if(!from_wifi_flag){customizedSerial . print("PID Autotune" " failed!" " Bad extruder number");}else{serial2 . print("PID Autotune" " failed!" " Bad extruder number");} {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);};return;}

    {do{ if(!from_wifi_flag){customizedSerial . print("PID Autotune" " start");}else{serial2 . print("PID Autotune" " start");} {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);};

    disable_all_heaters(); 
	if ((mksCfg . pidtemp && mksCfg . pidtempbed)) {
      if (hotend < 0)
        soft_pwm_bed = bias = d = (255) >> 1;
      else
        soft_pwm[hotend] = bias = d = (255) >> 1;}
	else if(mksCfg . pidtemp)	soft_pwm[hotend] = bias = d = (255) >> 1;
	else soft_pwm_bed = bias = d = (255) >> 1;
    ...
    while (wait_for_heatup) {
      ...
      if (temp_meas_ready) { 
		...
		if((mksCfg . pidtemp && mksCfg . pidtempbed)) 	input = hotend < 0 ? current_temperature_bed : current_temperature[hotend];
		else if(mksCfg . pidtemp) 		input = current_temperature[hotend];
		else					input = current_temperature_bed;
		 

        do{ if (max < input) max = input; }while(0);
        do{ if (min > input) min = input; }while(0);


        if (heating && input > temp) {
          if ((!((long)(ms-(t2 + 5000UL))<0))) {
            heating = false;
			if((mksCfg . pidtemp && mksCfg . pidtempbed))	{if (hotend < 0) soft_pwm_bed = (bias - d) >> 1; else soft_pwm[hotend] = (bias - d) >> 1;}
			else if(mksCfg . pidtemp)		soft_pwm[hotend] = (bias - d) >> 1;
        ...
        if (!heating && input < temp) {
          if ((!((long)(ms-(t1 + 5000UL))<0))) {
            heating = true;
            t2 = ms;
            t_low = t2 - t1;
            if (cycles > 0) {
              long max_pow;
              if((mksCfg . pidtemp && mksCfg . pidtempbed))	 max_pow = hotend < 0 ? 255 : 255;
              else if(mksCfg . pidtemp)	 max_pow = 255;
              else 			 max_pow = 255;
	 

              bias += (d * (t_high - t_low)) / (t_low + t_high);
              bias = ((bias) < (20))?(20):(((bias) > (max_pow - 20))? (max_pow - 20) : (bias));
              d = (bias > max_pow / 2) ? max_pow - 1 - bias : bias;

              {serial_echopair_P((" bias: "),(bias));};
              {serial_echopair_P((" d: "),(d));};
              {serial_echopair_P((" min: "),(min));};
              {serial_echopair_P((" max: "),(max));};
              if (cycles > 2) {
                Ku = (4.0 * d) / (3.141592653589793238462643 * (max - min) * 0.5);
                Tu = ((float)(t_low + t_high) * 0.001);
                {serial_echopair_P((" Ku: "),(Ku));};
                {serial_echopair_P((" Tu: "),(Tu));};
                workKp = 0.6 * Ku;
                workKi = 2 * workKp / Tu;
                workKd = workKp * Tu * 0.125;
                {serialprintPGM(("\n" " Classic PID " "\n"));};
                {serial_echopair_P((" Kp: "),(workKp));};
                {serial_echopair_P((" Ki: "),(workKi));};
                do{ {serial_echopair_P((" Kd: "),(workKd));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
      ...
      if (input > temp + 20) {
        {serialprintPGM(("PID Autotune" " failed!" " Temperature too high" "\n"));};
        return;
      }
      
      if ((!((long)(ms-(temp_ms + 2000UL))<0))) {
      ...
      if (((ms - t1) + (ms - t2)) > (10L * 60L * 1000L * 2L)) {
        {serialprintPGM(("PID Autotune" " failed!" " timeout" "\n"));};
        return;
      }
      if (cycles > ncycles) {
        {serialprintPGM(("PID Autotune" " finished! Put the last Kp, Ki and Kd constants from below into Configuration.h" "\n"));};
		
		if((mksCfg . pidtemp && mksCfg . pidtempbed))
			{
			const char* estring = hotend < 0 ? "bed" : "";
			{serial_echopair_P(("#define  DEFAULT_"),(estring));}; {serial_echopair_P(("Kp "),(workKp));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			{serial_echopair_P(("#define  DEFAULT_"),(estring));}; {serial_echopair_P(("Ki "),(workKi));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			{serial_echopair_P(("#define  DEFAULT_"),(estring));}; {serial_echopair_P(("Kd "),(workKd));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			}
		else if(mksCfg . pidtemp)
			{
			{serial_echopair_P(("#define  DEFAULT_Kp "),(workKp));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			{serial_echopair_P(("#define  DEFAULT_Ki "),(workKi));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			{serial_echopair_P(("#define  DEFAULT_Kd "),(workKd));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			}
		else
			{
			{serial_echopair_P(("#define  DEFAULT_bedKp "),(workKp));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			{serial_echopair_P(("#define  DEFAULT_bedKi "),(workKi));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			{serial_echopair_P(("#define  DEFAULT_bedKd "),(workKd));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
			}
	...
	if((mksCfg . pidtemp && mksCfg . pidtempbed))	if (set_result) {if (hotend < 0) do { bedKp = workKp; bedKi = ( (workKi) * 0.16384 ); bedKd = ( (workKd) / 0.16384 ); updatePID(); } while(0); else do { Temperature::Kp = workKp; Temperature::Ki = ( (workKi) * 0.16384 ); Temperature::Kd = ( (workKd) / 0.16384 ); updatePID(); } while(0);}
	else if(mksCfg . pidtemp)		if (set_result)  do { Temperature::Kp = workKp; Temperature::Ki = ( (workKi) * 0.16384 ); Temperature::Kd = ( (workKd) / 0.16384 ); updatePID(); } while(0);
	else 					if (set_result)  do { bedKp = workKp; bedKi = ( (workKi) * 0.16384 ); bedKd = ( (workKd) / 0.16384 ); updatePID(); } while(0);
...
void Temperature::_temp_error(int e, const char* serial_msg, const char* lcd_msg) {
  ...
  if (IsRunning()) {
    ...
    {serialprintPGM((", system stopped! Heater_ID: "));};
    if (e >= 0) {{do{ if(!from_wifi_flag){customizedSerial . print((int)e);}else{serial2 . print((int)e);} {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);};} else {{serialprintPGM(("bed" "\n"));};}
  }
    if (!killed) {
      Running = false;
      killed = true;
...
if(mksCfg . has_temp_bed)
	  _temp_error(e, ("MAXTEMP triggered"), e >= 0 ? ("Err: MAXTEMP") : ("Err: MAXTEMP BED"));
else
{
	_temp_error(0, ("MAXTEMP triggered"), ("Err: MAXTEMP"));
		((void)(e));
}
...
if(mksCfg . has_temp_bed)
		  _temp_error(e, ("MINTEMP triggered"), e >= 0 ? ("Err: MINTEMP") : ("Err: MINTEMP BED"));
else
{
		  _temp_error(0, ("MINTEMP triggered"), ("Err: MINTEMP"));
			((void)(e));
}
...
float Temperature::get_pid_output(int e) {
  ...
  if(mksCfg . pidtemp) {
      pid_error[0] = target_temperature[0] - current_temperature[0];
      dTerm[0] = (1.0-0.95) * Temperature::Kd * (current_temperature[0] - temp_dState[0]) + 0.95 * dTerm[0];
      temp_dState[0] = current_temperature[0];
      if (pid_error[0] > 10) {
        pid_output = 255;
        pid_reset[0] = true;
      }
      else if (pid_error[0] < -(10) || target_temperature[0] == 0) {
        pid_output = 0;
        pid_reset[0] = true;
      }
      else {
        ...
        pTerm[0] = Temperature::Kp * pid_error[0];
        temp_iState[0] += pid_error[0];
        iTerm[0] = Temperature::Ki * temp_iState[0];

        pid_output = pTerm[0] + iTerm[0] - dTerm[0];


        if (pid_output > 255) {
          if (pid_error[0] > 0) temp_iState[0] -= pid_error[0]; 
          pid_output = 255;
        }
    ...
    pid_output = (current_temperature[0] < target_temperature[0]) ? 255 : 0;
  }
  ...
  float Temperature::get_pid_output_bed() {
    float pid_output;
      ...
      dTerm_bed = (1.0-0.95) * bedKd * (current_temperature_bed - temp_dState_bed) + 0.95 * dTerm_bed;
...
void Temperature::manage_heater() {
  ...
  for (int8_t e = 0; e < 1; e ++) {
      ...
      thermal_runaway_protection(&thermal_runaway_state_machine[e], &thermal_runaway_timer[e], current_temperature[e], target_temperature[e], e, mksCfg . thermal_protection_period, mksCfg . thermal_protection_hysteresis);

    float pid_output = get_pid_output(e);

    
    soft_pwm[e] = (current_temperature[e] > minttemp[e] || (false)) && current_temperature[e] < maxttemp[e] ? (int)pid_output >> 1 : 0;

    

      
      if (watch_heater_next_ms[e] && (!((long)(ms-(watch_heater_next_ms[e]))<0))) {
        
        if (degHotend(e) < watch_target_temp[e]) {
          
          _temp_error(e, ("Heating failed"), (MSG_HEATING_FAILED_LCD));
        }
        else {
	...
	if(mksCfg . has_temp_bed) {	 
      ...
      if (watch_bed_next_ms && (!((long)(ms-(watch_bed_next_ms))<0))) {
        
        if (degBed() < watch_target_bed_temp) {
          
          _temp_error(-1, ("Heating failed"), (MSG_HEATING_FAILED_LCD));
        }
        else {
  ...
  if(mksCfg . pidtempbed == 0) {
    if (((long)(ms-(next_bed_check_ms))<0)) return;
    next_bed_check_ms = ms + 5000;
  }


	if(mksCfg . has_temp_bed)  
      thermal_runaway_protection(&thermal_runaway_bed_state_machine, &thermal_runaway_bed_timer, current_temperature_bed, target_temperature_bed, -1, mksCfg . thermal_protection_bed_period, mksCfg . thermal_protection_bed_hysteresis);
	
	if(mksCfg . pidtempbed)
		{
		float pid_output = get_pid_output_bed();
		soft_pwm_bed = ((current_temperature_bed) >= (-5) && (current_temperature_bed) <= (mksCfg . bed_maxtemp)) ? (int)pid_output >> 1 : 0;
		}
	else
		{
		
		if (((current_temperature_bed) >= (-5) && (current_temperature_bed) <= (mksCfg . bed_maxtemp))) {
		  soft_pwm_bed = current_temperature_bed < target_temperature_bed ? 255 >> 1 : 0;
		}
		else {
		  soft_pwm_bed = 0;
		  if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[15],gArrayGpioPin[15],(GPIO_PinState)GPIO_PIN_RESET);
		}
		}
...
float Temperature::analog2temp(int raw, uint8_t e) {
    ...
    {
      (serialprintPGM(errormagic));
      {if(!from_wifi_flag){customizedSerial . print((int)e);}else{serial2 . print((int)e);}};
      {serialprintPGM((" - Invalid extruder number !" "\n"));};
      kill(("KILLED. "));
      return 0.0;
    }
  ...
  if (heater_ttbl_map[e] != 0) {
    ...
    if (i == heater_ttbllen_map[e]) celsius = (short)((*tt)[i - 1][1]);

    return celsius;
  }
  return ((raw * ((5.0 * 100.0) / 1024.0) / 16) * (1.0)) + 0.0;
}
...
float Temperature::analog2tempBed(int raw) {
    ...
    for (i = 1; i < (sizeof(temptable_1)/sizeof(*temptable_1)); i++) {
      if ((short)(temptable_1[i][0]) > raw) {
        celsius  = (short)(temptable_1[i - 1][1]) +
                   (raw - (short)(temptable_1[i - 1][0])) *
                   (float)((short)(temptable_1[i][1]) - (short)(temptable_1[i - 1][1])) /
                   (float)((short)(temptable_1[i][0]) - (short)(temptable_1[i - 1][0]));
        break;
      }
    }

    
    if (i == (sizeof(temptable_1)/sizeof(*temptable_1))) celsius = (short)(temptable_1[i - 1][1]);
...
void Temperature::init() {
  ...
  for (int8_t e = 0; e < 1; e ++) maxttemp[e] = maxttemp[0];
    ...
    minttemp[0] = -5; while(analog2temp(minttemp_raw[0], 0) < -5) { if (16383 < 0) minttemp_raw[0] += 16; else minttemp_raw[0] -= 16; };
    maxttemp[0] = mksCfg . heater_0_maxtemp; while(analog2temp(maxttemp_raw[0], 0) > mksCfg . heater_0_maxtemp) { if (16383 < 0) maxttemp_raw[0] -= 16; else maxttemp_raw[0] += 16; };

    while(analog2tempBed(bed_minttemp_raw) < -5) {
        bed_minttemp_raw -= 16;
    }
    while (analog2tempBed(bed_maxttemp_raw) > mksCfg . bed_maxtemp) {
        bed_maxttemp_raw += 16;
    }
  ...
  void Temperature::start_watching_heater(uint8_t e) {
      ((void)(e));
    if (degHotend(0) < degTargetHotend(0) - (mksCfg . watch_temp_increase + 2 + 1)) {
		
    	
      watch_target_temp[0] = degHotend(0) + mksCfg . watch_temp_increase;
      watch_heater_next_ms[0] = HAL_GetTick() + (mksCfg . watch_temp_period) * 1000UL;
    }
    else
      watch_heater_next_ms[0] = 0;
  }
  ...
  void Temperature::start_watching_bed() {
    if (degBed() < degTargetBed() - (mksCfg . watch_bed_temp_increase + 2 + 1)) {
      watch_target_bed_temp = degBed() + mksCfg . watch_bed_temp_increase;
      watch_bed_next_ms = HAL_GetTick() + (mksCfg . watch_bed_temp_period) * 1000UL;
    }
    else
      watch_bed_next_ms = 0;
  }
    ...
    Temperature::TRState Temperature::thermal_runaway_state_machine[1] = { TRInactive };
    millis_t Temperature::thermal_runaway_timer[1] = { 0 };
  ...
  void Temperature::thermal_runaway_protection(Temperature::TRState* state, millis_t* timer, float current, float target, int heater_id, int period_seconds, int hysteresis_degc) {
    ...
    static float tr_target_temperature[1 + 1] = { 0.0 };
    ...
    int heater_index = heater_id >= 0 ? heater_id : 1;
    ...
    switch (*state) {
      ...
      case TRStable:
        if (current >= tr_target_temperature[heater_index] - hysteresis_degc) {
          *timer = HAL_GetTick() + period_seconds * 1000UL;
          break;
        }
        else if (((long)(HAL_GetTick()-(*timer))<0)) break;
        *state = TRRunaway;
      case TRRunaway:
        _temp_error(heater_id, ("Thermal Runaway"), (MSG_THERMAL_RUNAWAY));
    }
  }
...
void Temperature::disable_all_heaters() {
  ...
  for (int8_t e = 0; e < 1; e ++) setTargetHotend(0, e);
  setTargetBed(0);
  ...
  print_job_timer.stop();
    ...
    { setTargetHotend(0, 0); soft_pwm[0] = 0; if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[16],gArrayGpioPin[16],(GPIO_PinState)GPIO_PIN_RESET); };
...
volatile bool Temperature::in_temp_isr = false;
  ...
  static uint8_t soft_pwm_0 = 0;
    static uint8_t soft_pwm_BED = 0;


    const uint8_t pwm_mask =
        0
    ;
    ...
    if (pwm_count_tmp >= 127) {
	  ...
	  if(soft_pwm_0 > pwm_mask) if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[16],gArrayGpioPin[16],(GPIO_PinState)GPIO_PIN_SET);
	  else if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[16],gArrayGpioPin[16],(GPIO_PinState)GPIO_PIN_RESET);

        soft_pwm_BED = (soft_pwm_BED & pwm_mask) + soft_pwm_bed;
        
		if(soft_pwm_BED > pwm_mask) if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[15],gArrayGpioPin[15],(GPIO_PinState)GPIO_PIN_SET);
		else if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[15],gArrayGpioPin[15],(GPIO_PinState)GPIO_PIN_RESET);

    }
    else {
      if (soft_pwm_0 <= pwm_count_tmp) if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[16],gArrayGpioPin[16],(GPIO_PinState)0);

        if (soft_pwm_BED <= pwm_count_tmp) if(!mksTmp . cfg_hardware_test_enable) HAL_GPIO_WritePin(gArrayGpioPort[15],gArrayGpioPin[15],(GPIO_PinState)0);
  ...
  switch (adc_sensor_state) {
    ...
    case SensorsReady: {
      ...
      const int extra_loops = 10 - (int)SensorsReady;
      ...
      case MeasureTemp_0:
		...
		raw_temp_value[0] += uhADCxConvertedValue[0]>>2;
        break;
      ...
      case MeasureTemp_BED:
		...
		raw_temp_bed_value += uhADCxConvertedValue[1]>>2;
        break;
  ...
  if (!adc_sensor_state && ++temp_count >= 16) { 
    ...
    memset(raw_temp_value,0,sizeof(raw_temp_value));
    ...
    for (uint8_t e = 0; e < (sizeof(temp_dir)/sizeof(*temp_dir)); e++) {
      const int16_t tdir = temp_dir[e], rawtemp = current_temperature_raw[e] * tdir;
      if (rawtemp > maxttemp_raw[e] * tdir && target_temperature[e] > 0) max_temp_error(e);
      if (rawtemp < minttemp_raw[e] * tdir && !(false) && target_temperature[e] > 0) {
            min_temp_error(e);
      }
    }
	...
	if (current_temperature_bed_raw <= bed_maxttemp_raw && target_temperature_bed > 0) max_temp_error(-1);
	if (bed_minttemp_raw <= current_temperature_bed_raw && target_temperature_bed > 0) min_temp_error(-1);
