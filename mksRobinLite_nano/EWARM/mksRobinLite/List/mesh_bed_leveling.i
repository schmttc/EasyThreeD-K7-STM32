...
void do_blocking_move_to_xy(const float &x, const float &y, const float &fr_mm_s=0.0);
  ...
  float mesh_bed_leveling::z_offset,
	mesh_bed_leveling::z_values[16][16],
	mesh_bed_leveling::index_to_xpos[16],
	mesh_bed_leveling::index_to_ypos[16];
	

  mesh_bed_leveling::mesh_bed_leveling() {
    for (uint8_t i = 0; i < mksCfg . grid_max_points_x; ++i)
      index_to_xpos[i] = (mksCfg . x_min_pos + mksCfg . mesh_inst) + i * ((((mksCfg . x_max_pos - (mksCfg . mesh_inst)) - ((mksCfg . x_min_pos + mksCfg . mesh_inst))) / (mksCfg . grid_max_points_x - 1)));
    for (uint8_t i = 0; i < mksCfg . grid_max_points_y; ++i)
      index_to_ypos[i] = (mksCfg . y_min_pos + mksCfg . mesh_inst) + i * ((((mksCfg . y_max_pos - (mksCfg . mesh_inst)) - ((mksCfg . y_min_pos + mksCfg . mesh_inst))) / (mksCfg . grid_max_points_y - 1)));
    reset();
  }
