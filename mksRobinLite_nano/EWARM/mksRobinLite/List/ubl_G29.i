...
int finish_incremental_LSF(struct linear_fit_data *);
  ...
  void gcode_G29_UBL() {
    ...
    if (ubl.eeprom_start < 0) {
      {serialprintPGM(("?You need to enable your EEPROM and initialize it" "\n"));};
      {serialprintPGM(("with M502, M500, M501 in that order.\n" "\n"));};
      return;
    }

    if (!code_seen('N') && axis_unhomed_error(true, true, true))  
      home_all_axes();
    ...
    if (code_seen('I')) {
      ...
      while (repetition_cnt--) {
        ...
        const mesh_index_pair location = find_closest_mesh_point_of_type(REAL, x_pos, y_pos, 0, 0, false);
        if (location.x_index < 0) {
          {serialprintPGM(("Entire Mesh invalidated.\n" "\n"));};
          break;            
        }
        ubl.z_values[location.x_index][location.y_index] = ((float) 0.NaN);
        cnt++;
      }
      {serialprintPGM(("Locations invalidated.\n" "\n"));};
    }

    if (code_seen('Q')) {
      const int test_pattern = code_has_value() ? code_value_int() : -1;
      if (!((test_pattern) >= (0) && (test_pattern) <= (2))) {
        {serialprintPGM(("Invalid test_pattern value. (0-2)\n" "\n"));};
        return;
      }
      {serialprintPGM(("Loading test_pattern values.\n" "\n"));};
      switch (test_pattern) {
        case 0:
          for (uint8_t x = 0; x < mksCfg . grid_max_points_x; x++) {   
            for (uint8_t y = 0; y < mksCfg . grid_max_points_y; y++) { 
              const float p1 = 0.5 * (mksCfg . grid_max_points_x) - x,
                          p2 = 0.5 * (mksCfg . grid_max_points_y) - y;
              ubl.z_values[x][y] += 2.0 * sqrt((pow(p1,2)+pow(p2,2)));
            }
          }
          break;
        case 1:
          for (uint8_t x = 0; x < mksCfg . grid_max_points_x; x++) {  
            ...
            ubl.z_values[x][x + (x < mksCfg . grid_max_points_y - 1) ? 1 : -1] += 9.999; 
          }
          break;
        case 2:
          
          for (uint8_t x = (mksCfg . grid_max_points_x) / 3; x < 2 * (mksCfg . grid_max_points_x) / 3; x++)   
            for (uint8_t y = (mksCfg . grid_max_points_y) / 3; y < 2 * (mksCfg . grid_max_points_y) / 3; y++) 
    ...
    if (code_seen('P')) {
		if (((phase_value) >= (0) && (phase_value) <= (1)) && ubl.state.eeprom_storage_slot == -1) {
		  ubl.state.eeprom_storage_slot = 0;
		  {serialprintPGM(("Default storage slot 0 selected.\n" "\n"));};
      }
      switch (phase_value) {
        case 0:
        ...
        case 1:
          ...
          if (!code_seen('C')) {
            ubl.invalidate();
            {serialprintPGM(("Mesh invalidated. Probing mesh.\n" "\n"));};
          }
          if (g29_verbose_level > 1) {
		  	
            
			{serialprintPGM(("Probing Mesh Points Closest to ("));};
			{if(!from_wifi_flag){customizedSerial . print(x_pos);}else{serial2 . print(x_pos);}};
            {if(!from_wifi_flag){customizedSerial . write(',');}else{serial2 . print(',');}};
            {if(!from_wifi_flag){customizedSerial . print(y_pos);}else{serial2 . print(y_pos);}};
            {serialprintPGM((").\n" "\n"));};
          }
          ubl.probe_entire_mesh(x_pos + mksCfg . x_probe_offset_from_extruder, y_pos + mksCfg . y_probe_offset_from_extruder,
        ...
        case 2: {
          ...
          {serialprintPGM(("Manually probing unreachable mesh locations.\n" "\n"));};
          do_blocking_move_to_z(mksCfg . z_clearance_between_probes);
          if (!x_flag && !y_flag) {
            ...
            x_pos = mksCfg . x_probe_offset_from_extruder > 0 ? (mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)) : (mksCfg . x_min_pos + mksCfg . ubl_mesh_inset);
            y_pos = mksCfg . y_probe_offset_from_extruder < 0 ? (mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)) : (mksCfg . y_min_pos + mksCfg . ubl_mesh_inset);
          }
        ...
        case 3: {
          ...
          if (c_flag) {
            while (repetition_cnt--) {
              const mesh_index_pair location = find_closest_mesh_point_of_type(INVALID, x_pos, y_pos, 0, 0, false);
    ...
    if (code_seen('T')) {
      ...
      float z1 = probe_pt(((mksCfg . ubl_probe_pt_1_x) + workspace_offset[X_AXIS]), ((mksCfg . ubl_probe_pt_1_y) + workspace_offset[Y_AXIS]), false, g29_verbose_level),
            z2 = probe_pt(((mksCfg . ubl_probe_pt_2_x) + workspace_offset[X_AXIS]), ((mksCfg . ubl_probe_pt_2_y) + workspace_offset[Y_AXIS]), false, g29_verbose_level),
            z3 = probe_pt(((mksCfg . ubl_probe_pt_3_x) + workspace_offset[X_AXIS]), ((mksCfg . ubl_probe_pt_3_y) + workspace_offset[Y_AXIS]), true, g29_verbose_level);

      
      

      ubl.save_ubl_active_state_and_disable();
      z1 -= ubl.get_z_correction(((mksCfg . ubl_probe_pt_1_x) + workspace_offset[X_AXIS]), ((mksCfg . ubl_probe_pt_1_y) + workspace_offset[Y_AXIS]))   ;
      z2 -= ubl.get_z_correction(((mksCfg . ubl_probe_pt_2_x) + workspace_offset[X_AXIS]), ((mksCfg . ubl_probe_pt_2_y) + workspace_offset[Y_AXIS]))   ;
      z3 -= ubl.get_z_correction(((mksCfg . ubl_probe_pt_3_x) + workspace_offset[X_AXIS]), ((mksCfg . ubl_probe_pt_3_y) + workspace_offset[Y_AXIS]))   ;

      do_blocking_move_to_xy(0.5 * ((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset))), 0.5 * ((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset))));
    ...
    if (code_seen('j')) g29_eeprom_dump();   
    ...
    if (code_seen('S')) {     
      ...
      if (storage_slot == -1) {                     
        {serialprintPGM(("G29 I 999" "\n"));};              
        for (uint8_t x = 0; x < mksCfg . grid_max_points_x; x++)
          for (uint8_t y = 0;  y < mksCfg . grid_max_points_y; y++)
            if (!isnan(ubl.z_values[x][y])) {
              {serial_echopair_P(("M421 I "),(x));};
              {serial_echopair_P((" J "),(y));};
              {serialprintPGM((" Z "));};
              {if(!from_wifi_flag){customizedSerial . print(ubl . z_values[x][y],6);}else{serial2 . print(ubl . z_values[x][y],6);}};
              {serial_echopair_P((" ; X "),(((ubl . mesh_index_to_xpos[x]) + workspace_offset[X_AXIS])));};
              {serial_echopair_P((", Y "),(((ubl . mesh_index_to_ypos[y]) + workspace_offset[Y_AXIS])));};
              {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
            }
        return;
      }
      ...
      if (!((storage_slot) >= (0) && (storage_slot) <= (j - 1)) || ubl.eeprom_start <= 0) {
        {serialprintPGM(("?EEPROM storage not available for use.\n" "\n"));};
        do{ {serial_echopair_P(("?Use 0 to "),(j - 1));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
        goto LEAVE;
      }
    ...
    if (code_seen('Z')) {
      if (code_has_value())
        ubl.state.z_offset = code_value_float();   
      else {
        ubl.save_ubl_active_state_and_disable();
        ...
        const millis_t nxt = HAL_GetTick() + 1500UL;
        while (ubl_lcd_clicked()) { 
          idle();
          if ((!((long)(HAL_GetTick()-(nxt))<0))) {
            {serialprintPGM(("\nZ-Offset Adjustment Stopped." "\n"));};
            do_blocking_move_to_z(mksCfg . z_clearance_deploy_probe);
  ...
  void unified_bed_leveling::find_mean_mesh_height() {
    ...
    for (uint8_t x = 0; x < mksCfg . grid_max_points_x; x++)
      for (uint8_t y = 0; y < mksCfg . grid_max_points_y; y++)
        if (!isnan(ubl.z_values[x][y]))
          sum_of_diff_squared += pow(ubl . z_values[x][y] - mean,2);

    do{ {serial_echopair_P(("# of samples: "),(n));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    {serialprintPGM(("Mean Mesh Height: "));};
    {if(!from_wifi_flag){customizedSerial . print(mean,6);}else{serial2 . print(mean,6);}};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

    const float sigma = sqrt(sum_of_diff_squared / (n + 1));
    {serialprintPGM(("Standard Deviation: "));};
    {if(!from_wifi_flag){customizedSerial . print(sigma,6);}else{serial2 . print(sigma,6);}};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
  ...
  void unified_bed_leveling::probe_entire_mesh(const float &lx, const float &ly, const bool do_ubl_mesh_map, const bool stow_probe, bool do_furthest) {
    ...
    do {
      ...
      location = find_closest_mesh_point_of_type(INVALID, lx, ly, 1, 0, do_furthest);
      if (location.x_index >= 0 && location.y_index >= 0) {
        ...
        if (!((rawx) >= (mksTmp . min_probe_x) && (rawx) <= (mksTmp . max_probe_x)) || !((rawy) >= (mksTmp . min_probe_y) && (rawy) <= (mksTmp . max_probe_y))) {
          (serialprintPGM(errormagic));
          {serialprintPGM(("Attempt to probe off the bed." "\n"));};
          ubl.has_control_of_lcd_panel = false;
          goto LEAVE;
        }
        const float measured_z = probe_pt(((rawx) + workspace_offset[X_AXIS]), ((rawy) + workspace_offset[Y_AXIS]), stow_probe, g29_verbose_level);
    ...
    do_blocking_move_to_xy(
      ((lx - (mksCfg . x_probe_offset_from_extruder)) < ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset)))?((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset)):(((lx - (mksCfg . x_probe_offset_from_extruder)) > ((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset))))? ((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset))) : (lx - (mksCfg . x_probe_offset_from_extruder))),
      ((ly - (mksCfg . y_probe_offset_from_extruder)) < ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset)))?((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset)):(((ly - (mksCfg . y_probe_offset_from_extruder)) > ((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset))))? ((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset))) : (ly - (mksCfg . y_probe_offset_from_extruder)))
    );
  }
  ...
  void unified_bed_leveling::tilt_mesh_based_on_3pts(const float &z1, const float &z2, const float &z3) {
    ...
    matrix_3x3 rotation;
    vector_3 v1 = vector_3( (mksCfg . ubl_probe_pt_1_x - mksCfg . ubl_probe_pt_2_x),
                            (mksCfg . ubl_probe_pt_1_y - mksCfg . ubl_probe_pt_2_y),
                            (z1 - z2) ),

             v2 = vector_3( (mksCfg . ubl_probe_pt_3_x - mksCfg . ubl_probe_pt_2_x),
                            (mksCfg . ubl_probe_pt_3_y - mksCfg . ubl_probe_pt_2_y),
                            (z3 - z2) ),
    ...
    if (g29_verbose_level > 2) {
      {serialprintPGM(("bed plane normal = ["));};
      {if(!from_wifi_flag){customizedSerial . print(normal . x,7);}else{serial2 . print(normal . x,7);}};
      {if(!from_wifi_flag){customizedSerial . write(',');}else{serial2 . print(',');}};
      {if(!from_wifi_flag){customizedSerial . print(normal . y,7);}else{serial2 . print(normal . y,7);}};
      {if(!from_wifi_flag){customizedSerial . write(',');}else{serial2 . print(',');}};
      {if(!from_wifi_flag){customizedSerial . print(normal . z,7);}else{serial2 . print(normal . z,7);}};
      {serialprintPGM(("]" "\n"));};
    ...
    float t = normal.x * (mksCfg . ubl_probe_pt_1_x) + normal.y * (mksCfg . ubl_probe_pt_1_y),
          d = t + normal.z * z1;

    if (g29_verbose_level>2) {
      {serialprintPGM(("D constant: "));};
      {if(!from_wifi_flag){customizedSerial . print(d,7);}else{serial2 . print(d,7);}};
      {serialprintPGM((" " "\n"));};
    }


    for (uint8_t i = 0; i < mksCfg . grid_max_points_x; i++) {
      for (uint8_t j = 0; j < mksCfg . grid_max_points_y; j++) {
        float x_tmp = ubl.mesh_index_to_xpos[i],
              y_tmp = ubl.mesh_index_to_ypos[j],
  ...
  float use_encoder_wheel_to_measure_point() {
    ...
    while (ubl_lcd_clicked()) HAL_Delay(50);  
    HAL_Delay(50);  
  ...
  static void say_and_take_a_measurement() {
    {serialprintPGM((" and take a measurement." "\n"));};
  }
  ...
  float measure_business_card_thickness(const float &in_height) {
    ...
    stepper.synchronize();
    {serialprintPGM(("Place shim under nozzle."));};
    say_and_take_a_measurement();

    const float z1 = use_encoder_wheel_to_measure_point();
    do_blocking_move_to_z(current_position[Z_AXIS] + 0);
    stepper.synchronize();

    {serialprintPGM(("Remove shim."));};
    say_and_take_a_measurement();
    ...
    if (g29_verbose_level > 1) {
      {serialprintPGM(("Business Card is: "));};
      {if(!from_wifi_flag){customizedSerial . print(abs(z1 - z2),6);}else{serial2 . print(abs(z1 - z2),6);}};
      {serialprintPGM(("mm thick." "\n"));};
    }
    ubl.has_control_of_lcd_panel = false;
  ...
  void manually_probe_remaining_mesh(const float &lx, const float &ly, const float &z_clearance, const float &card_thickness, const bool do_ubl_mesh_map) {
    ...
    mesh_index_pair location;
    do {
      location = find_closest_mesh_point_of_type(INVALID, lx, ly, 0, 0, false);
      ...
      if (!((rawx) >= ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset)) && (rawx) <= ((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)))) || !((rawy) >= ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset)) && (rawy) <= ((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset))))) {
        (serialprintPGM(errormagic));
        {serialprintPGM(("Attempt to probe off the bed." "\n"));};
        ubl.has_control_of_lcd_panel = false;
        goto LEAVE;
      ...
      const float xProbe = ((rawx) + workspace_offset[X_AXIS]),
                  yProbe = ((rawy) + workspace_offset[Y_AXIS]),
                  dx = xProbe - last_x,
                  dy = yProbe - last_y;
      ...
      const millis_t nxt = HAL_GetTick() + 1500L;
      while (ubl_lcd_clicked()) {     
        idle();
        if ((!((long)(HAL_GetTick()-(nxt))<0))) {
          {serialprintPGM(("\nMesh only partially populated." "\n"));};
      ...
      if (g29_verbose_level > 2) {
        {serialprintPGM(("Mesh Point Measured at: "));};
        {if(!from_wifi_flag){customizedSerial . print(ubl . z_values[location . x_index][location . y_index],6);}else{serial2 . print(ubl . z_values[location . x_index][location . y_index],6);}};
        {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
      }
    } while (location.x_index >= 0 && location.y_index >= 0);
  ...
  static void report_ubl_state() {
    say_ubl_name();
    {serialprintPGM(("System "));};
    if (!ubl.state.active) {serialprintPGM(("de"));};
    {serialprintPGM(("activated.\n" "\n"));};
  }
  ...
  bool g29_parameter_parsing() {
    ...
    if (repeat_flag) {
      repetition_cnt = code_has_value() ? code_value_int() : (mksCfg . grid_max_points_x) * (mksCfg . grid_max_points_y);
      if (repetition_cnt < 1) {
        {serialprintPGM(("?(R)epetition count invalid (1+).\n" "\n"));};
        return true;
      }
    }
    ...
    if (code_seen('P')) {
      ...
      if (!((phase_value) >= (0) && (phase_value) <= (6))) {
        {serialprintPGM(("?(P)hase value invalid (0-6).\n" "\n"));};
        err_flag = true;
      }
    }
    ...
    if (code_seen('J')) {
		...
		if (!((grid_size) >= (2) && (grid_size) <= (9))) {
		  {serialprintPGM(("?Invalid grid size (J) specified (2-9).\n" "\n"));};
		  err_flag = true;
		}
    }

    if (x_flag != y_flag) {
      {serialprintPGM(("Both X & Y locations must be specified.\n" "\n"));};
      err_flag = true;
    }

    if (!((((x_pos) - workspace_offset[X_AXIS])) >= (mksCfg . x_min_pos) && (((x_pos) - workspace_offset[X_AXIS])) <= (mksCfg . x_max_pos))) {
      {serialprintPGM(("Invalid X location specified.\n" "\n"));};
      err_flag = true;
    }

    if (!((((y_pos) - workspace_offset[Y_AXIS])) >= (mksCfg . y_min_pos) && (((y_pos) - workspace_offset[Y_AXIS])) <= (mksCfg . y_max_pos))) {
      {serialprintPGM(("Invalid Y location specified.\n" "\n"));};
      err_flag = true;
    }

    if (err_flag) return true;

    
    if (code_seen('A')) {
      if (code_seen('D')) {
        {serialprintPGM(("?Can't activate and deactivate at the same time.\n" "\n"));};
        return true;
      }
      ubl.state.active = 1;
      report_ubl_state();
	...
	if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))	
      ...
      if (code_seen('F') && code_has_value()) {
        ...
        if (!((fh) >= (0.0) && (fh) <= (100.0))) {
          {serialprintPGM(("?(F)ade height for Bed Level Correction not plausible.\n" "\n"));};
          return true;
        }
    ...
    if (!((map_type) >= (0) && (map_type) <= (1))) {
      {serialprintPGM(("Invalid map type.\n" "\n"));};
      return true;
    }
  ...
  void unified_bed_leveling::save_ubl_active_state_and_disable() {
    ...
    if (ubl_state_recursion_chk != 1) {
      {serialprintPGM(("save_ubl_active_state_and_disabled() called multiple times in a row." "\n"));};
  ...
  void unified_bed_leveling::restore_ubl_active_state_and_leave() {
    if (--ubl_state_recursion_chk) {
      {serialprintPGM(("restore_ubl_active_state_and_leave() called too many times." "\n"));};
  ...
  void g29_what_command() {
    const uint16_t k = (1536-1) - ubl.eeprom_start;

    say_ubl_name();
    {serialprintPGM(("System Version " "1.00" " "));};
    if (ubl.state.active)
    {  {if(!from_wifi_flag){customizedSerial . write('A');}else{serial2 . print('A');}};}
    else
    {  {serialprintPGM(("Ina"));};}
    {serialprintPGM(("ctive.\n" "\n"));};
    safe_delay(50);

    if (ubl.state.eeprom_storage_slot == -1)
    { {serialprintPGM(("No Mesh Loaded."));};}
    else {
      
	  {serialprintPGM(("Mesh "));};
	  {if(!from_wifi_flag){customizedSerial . print(ubl . state . eeprom_storage_slot);}else{serial2 . print(ubl . state . eeprom_storage_slot);}};
	  
      {serialprintPGM((" Loaded."));};
    }
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    safe_delay(50);

    do{ {serial_echopair_P(("UBL object count: "),((int)ubl_cnt));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

	if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))	
      do{ {serial_echopair_P(("planner.z_fade_height : "),(planner . z_fade_height));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    {serialprintPGM(("zprobe_zoffset: "));};
    {if(!from_wifi_flag){customizedSerial . print(zprobe_zoffset,7);}else{serial2 . print(zprobe_zoffset,7);}};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

    {serialprintPGM(("z_offset: "));};
    {if(!from_wifi_flag){customizedSerial . print(ubl . state . z_offset,7);}else{serial2 . print(ubl . state . z_offset,7);}};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    safe_delay(25);

    do{ {serial_echopair_P(("ubl.eeprom_start="),(hex_address(ubl . eeprom_start)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

    {serialprintPGM(("X-Axis Mesh Points at: "));};
    for (uint8_t i = 0; i < mksCfg . grid_max_points_x; i++) {
      {if(!from_wifi_flag){customizedSerial . print(((ubl . mesh_index_to_xpos[i]) + workspace_offset[X_AXIS]),1);}else{serial2 . print(((ubl . mesh_index_to_xpos[i]) + workspace_offset[X_AXIS]),1);}};
      {serialprintPGM(("  "));};
      safe_delay(50);
    }
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};

    {serialprintPGM(("Y-Axis Mesh Points at: "));};
    for (uint8_t i = 0; i < mksCfg . grid_max_points_y; i++) {
      {if(!from_wifi_flag){customizedSerial . print(((ubl . mesh_index_to_ypos[i]) + workspace_offset[Y_AXIS]),1);}else{serial2 . print(((ubl . mesh_index_to_ypos[i]) + workspace_offset[Y_AXIS]),1);}};
      {serialprintPGM(("  "));};
      safe_delay(50);
    }
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    ...
    do{ {serial_echopair_P(("ubl_state_at_invocation :"),(ubl_state_at_invocation));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    do{ {serial_echopair_P(("ubl_state_recursion_chk :"),(ubl_state_recursion_chk));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    safe_delay(50);
    do{ {serial_echopair_P(("Free EEPROM space starts at: "),(hex_address(ubl . eeprom_start)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);

    do{ {serial_echopair_P(("end of EEPROM              : "),(hex_address((1536-1))));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    safe_delay(50);

    do{ {serial_echopair_P(("sizeof(ubl) :  "),((int)sizeof(ubl)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    do{ {serial_echopair_P(("z_value[][] size: "),((int)sizeof(ubl . z_values)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    safe_delay(50);

    do{ {serial_echopair_P(("EEPROM free for UBL: "),(hex_address(k)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
    safe_delay(50);


	{serialprintPGM(("EEPROM can hold "));};
	{if(!from_wifi_flag){customizedSerial . print(k / sizeof(ubl . z_values));}else{serial2 . print(k / sizeof(ubl . z_values));}};
	
    {serialprintPGM((" meshes.\n" "\n"));};
    safe_delay(50);


	{serialprintPGM(("sizeof(ubl.state) : "));};
	{if(!from_wifi_flag){customizedSerial . print((int)sizeof(ubl . state));}else{serial2 . print((int)sizeof(ubl . state));}};


    
	{serialprintPGM(("\nGRID_MAX_POINTS_X  "));};
	{if(!from_wifi_flag){customizedSerial . print(mksCfg . grid_max_points_x);}else{serial2 . print(mksCfg . grid_max_points_x);}};
	
    
	{serialprintPGM(("\nGRID_MAX_POINTS_Y  "));};
	{if(!from_wifi_flag){customizedSerial . print(mksCfg . grid_max_points_y);}else{serial2 . print(mksCfg . grid_max_points_y);}};
	
    safe_delay(50);
    
	{serialprintPGM(("\nUBL_MESH_MIN_X         "));};
	{if(!from_wifi_flag){customizedSerial . print((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset));}else{serial2 . print((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset));}};
	
   
	{serialprintPGM(("\nUBL_MESH_MIN_Y         "));};
	{if(!from_wifi_flag){customizedSerial . print((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset));}else{serial2 . print((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset));}};
	
    safe_delay(50);
    
	{serialprintPGM(("\nUBL_MESH_MAX_X         "));};
	{if(!from_wifi_flag){customizedSerial . print((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)));}else{serial2 . print((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)));}};

   
	{serialprintPGM(("\nUBL_MESH_MAX_Y         "));};
	{if(!from_wifi_flag){customizedSerial . print((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)));}else{serial2 . print((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)));}};
	
    safe_delay(50);
    {serialprintPGM(("\nMESH_X_DIST        "));};
    {if(!from_wifi_flag){customizedSerial . print((float((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_x - 1)),6);}else{serial2 . print((float((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_x - 1)),6);}};
    {serialprintPGM(("\nMESH_Y_DIST        "));};
    {if(!from_wifi_flag){customizedSerial . print((float((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_y - 1)),6);}else{serial2 . print((float((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_y - 1)),6);}};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    safe_delay(50);

    if (!ubl.sanity_check()) {
      say_ubl_name();
      {serialprintPGM(("sanity checks passed." "\n"));};
    }
  }
  ...
  void g29_eeprom_dump() {
    ...
    {serialprintPGM(("EEPROM Dump:" "\n"));};
    for (uint16_t i = 0; i < (1536-1) + 1; i += 16) {
      if (!(i & 0x3)) idle();
      print_hex_word(i);
      {serialprintPGM((": "));};
      for (uint16_t j = 0; j < 16; j++) {
        ...
        AT24CXX_Read((uint16_t)kkkk,&cccc,1);
        print_hex_byte(cccc);
        {if(!from_wifi_flag){customizedSerial . print(' ');}else{serial2 . print(' ');}};
      }
      {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    }
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
  ...
  void g29_compare_current_mesh_to_stored_mesh() {
    ...
    float tmp_z_values[16][16];

    if (!code_has_value()) {
      {serialprintPGM(("?Mesh # required.\n" "\n"));};
      return;
    }
    storage_slot = code_value_int();

    int16_t j = ((1536-1) - ubl.eeprom_start) / sizeof(tmp_z_values);
    ...
    j = (1536-1) - (storage_slot + 1) * sizeof(tmp_z_values);
    
    int BufferSize=sizeof(tmp_z_values);	  
    uint16_t MemAddress =j;
    float* pBuffer = &tmp_z_values[0][0]; 
    AT24CXX_Read(MemAddress,(uint8_t*)pBuffer,BufferSize);

    {serial_echopair_P(("Subtracting Mesh "),(storage_slot));};
    do{ {serial_echopair_P((" loaded from EEPROM address "),(hex_address(j)));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0); 
  ...
  mesh_index_pair find_closest_mesh_point_of_type(const MeshPointType type, const float &lx, const float &ly, const bool probe_as_reference, unsigned int bits[16], const bool far_flag) {
    ...
    const float px = lx - (probe_as_reference == 1 ? mksCfg . x_probe_offset_from_extruder : 0),
                py = ly - (probe_as_reference == 1 ? mksCfg . y_probe_offset_from_extruder : 0);
    ...
    for (uint8_t i = 0; i < mksCfg . grid_max_points_x; i++) {
      for (uint8_t j = 0; j < mksCfg . grid_max_points_y; j++) {
        ...
        if ( (type == INVALID && isnan(ubl.z_values[i][j]))  
          ...
          const float rawx = ubl.mesh_index_to_xpos[i], 
                      rawy = ubl.mesh_index_to_ypos[j];

          
          

          if (probe_as_reference == 1 &&
            (!((rawx) >= (mksTmp . min_probe_x) && (rawx) <= (mksTmp . max_probe_x)) || !((rawy) >= (mksTmp . min_probe_y) && (rawy) <= (mksTmp . max_probe_y)))
          ) continue;

          
          

          const float mx = ((rawx) + workspace_offset[X_AXIS]), 
                      my = ((rawy) + workspace_offset[Y_AXIS]);

          float distance = sqrt((pow(px - mx,2)+pow(py - my,2))) + sqrt((pow(current_x - mx,2)+pow(current_y - my,2))) * 0.1;
          ...
          if (far_flag) {
            for (uint8_t k = 0; k < mksCfg . grid_max_points_x; k++) {
              for (uint8_t l = 0; l < mksCfg . grid_max_points_y; l++) {
                if (!isnan(ubl.z_values[k][l])) {
                  


 
                  distance += (i - k)* (i - k)* ((float((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_x - 1))) * .05
                            + (j - l) * (j - l) *((float((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_y - 1))) * .05;
  ...
  void fine_tune_mesh(const float &lx, const float &ly, const bool do_ubl_mesh_map) {
    ...
    do_blocking_move_to_xy(lx, ly);
    do {
      location = find_closest_mesh_point_of_type(SET_IN_BITMAP, lx, ly, 0, not_done, false);
      ...
      if (!((rawx) >= ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset)) && (rawx) <= ((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)))) || !((rawy) >= ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset)) && (rawy) <= ((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset))))) { 
        (serialprintPGM(errormagic));
        {serialprintPGM(("Attempt to edit off the bed." "\n"));}; 
        ubl.has_control_of_lcd_panel = false;
        goto FINE_TUNE_EXIT;
      }
      ...
      if (!isnan(new_z)) {  
        ...
        do_blocking_move_to_z(mksCfg . z_clearance_deploy_probe);    
        do_blocking_move_to_xy(((rawx) + workspace_offset[X_AXIS]), ((rawy) + workspace_offset[Y_AXIS]));

        round_off = (int32_t)(new_z * 1000.0);    
    ...
    {serialprintPGM(("Done Editing Mesh" "\n"));};
  }
  ...
  bool smart_fill_one(const uint8_t x, const uint8_t y, const int8_t xdir, const int8_t ydir) {
    ...
    if (isnan(ubl.z_values[x][y]) && !isnan(ubl.z_values[x1][y1]) && !isnan(ubl.z_values[x2][y2])) {
      if (ubl.z_values[x1][y1] < ubl.z_values[x2][y2])                  
        ubl.z_values[x][y] = ubl.z_values[x1][y1];                      
      else
        ubl.z_values[x][y] = 2.0 * ubl.z_values[x1][y1] - ubl.z_values[x2][y2];   
      return true;
    }
    return false;
  }
  ...
  void smart_fill_mesh() {
    const smart_fill_info info[] = {
      { 0, mksCfg . grid_max_points_x,      0, mksCfg . grid_max_points_y - 2,  false },  
      { 0, mksCfg . grid_max_points_x,      mksCfg . grid_max_points_y - 1, 0,  false },  
      { 0, mksCfg . grid_max_points_x - 2,  0, mksCfg . grid_max_points_y,      true  },  
      { mksCfg . grid_max_points_x - 1, 0,  0, mksCfg . grid_max_points_y,      true  }   
    };
  ...
  void unified_bed_leveling::tilt_mesh_based_on_probed_grid(const bool do_ubl_mesh_map) {
    const int16_t x_min = (((mksTmp . min_probe_x)>((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset)))?(mksTmp . min_probe_x):((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset))),
                      x_max = (((mksTmp . max_probe_x)<((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset))))?(mksTmp . max_probe_x):((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)))),
                      y_min = (((mksTmp . min_probe_y)>((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset)))?(mksTmp . min_probe_y):((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset))),
                      y_max = (((mksTmp . max_probe_y)<((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset))))?(mksTmp . max_probe_y):((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset))));
    ...
    for (uint8_t ix = 0; ix < grid_size; ix++) {
      ...
      for (int8_t iy = 0; iy < grid_size; iy++) {
        ...
        float measured_z = probe_pt(((x) + workspace_offset[X_AXIS]), ((y) + workspace_offset[Y_AXIS]), code_seen('E'), g29_verbose_level);

        measured_z -= ubl.get_z_correction(((x) + workspace_offset[X_AXIS]), ((y) + workspace_offset[Y_AXIS]))   ;
    ...
    if (finish_incremental_LSF(&lsf_results)) {
      {serialprintPGM(("Could not complete LSF!"));};
      return;
    }

    if (g29_verbose_level > 3) {
      {serialprintPGM(("LSF Results A="));};
      {if(!from_wifi_flag){customizedSerial . print(lsf_results . A,7);}else{serial2 . print(lsf_results . A,7);}};
      {serialprintPGM(("  B="));};
      {if(!from_wifi_flag){customizedSerial . print(lsf_results . B,7);}else{serial2 . print(lsf_results . B,7);}};
      {serialprintPGM(("  D="));};
      {if(!from_wifi_flag){customizedSerial . print(lsf_results . D,7);}else{serial2 . print(lsf_results . D,7);}};
      {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
    }
