...
void BSP_UartHwInit(BSP_UART_STACK *pUStack, uint32_t newBaudRate)
{
	...
	if(pUStack->id == 1)
	{
		pUart->handle.Instance = ((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800));
	}
	else if(pUStack->id == 2)
	{
		pUart->handle.Instance = ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400));
	}
	else if(pUStack->id == 3)
	{
		pUart->handle.Instance = ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4800));
	}
	else if(pUStack->id == 4)
	{
		pUart->handle.Instance = ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4C00));
	}
	else if(pUStack->id == 5)
	{
		pUart->handle.Instance = ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x5000));
	}
  
  
	  pUart->handle.Init.BaudRate = newBaudRate;
	  pUart->handle.Init.WordLength = ((uint32_t)0x00000000);
	  pUart->handle.Init.StopBits = ((uint32_t)0x00000000);
	  pUart->handle.Init.Parity = ((uint32_t)0x00000000);
	  pUart->handle.Init.Mode = ((uint32_t)((0x1U << (3U)) |(0x1U << (2U))));
	  pUart->handle.Init.HwFlowCtl = ((uint32_t)0x00000000);
	  pUart->handle.Init.OverSampling = ((uint32_t)0x00000000);

	  if(HAL_UART_DeInit(&pUart->handle) != HAL_OK)
	  {
	    BSP_MiscErrorHandler(1|(0x1000));
	  }  

	  if( HAL_UART_Init(&pUart->handle) != HAL_OK)
	  {
	    BSP_MiscErrorHandler(2|(0x1000));
	  }
}
...
void BSP_UartIfSendQueuedData(BSP_UART_STACK *pUStack)
{
	...
	while(USART_GetFlagStatus(pUart->handle.Instance, ((uint16_t)0x0080)) == RESET){}
      }
...
void BSP_UartFlushRx(BSP_UART_STACK *pUStack)
{
	...
	uint32_t tmp_flag = 0;  
	 ...
	 tmp_flag = (((&pUart->handle)->Instance ->SR & (((uint32_t)(0x1U << (5U))))) == (((uint32_t)(0x1U << (5U)))));
