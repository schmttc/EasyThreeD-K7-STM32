...
extern Temperature thermalManager;
  ...
  void un_retract_filament(float where[4]);
  void retract_filament(float where[4]);
  ...
  static float extrusion_multiplier = 1.0,
  ...
  void gcode_G26() {
    {serialprintPGM(("G26 command started.  Waiting for heater(s)." "\n"));};
    ...
    if (current_position[Z_AXIS] < mksCfg . z_clearance_between_probes) {
    ...
    memset(circle_flags,0,sizeof(circle_flags));
    memset(horizontal_mesh_line_flags,0,sizeof(horizontal_mesh_line_flags));
    memset(vertical_mesh_line_flags,0,sizeof(vertical_mesh_line_flags));
    ...
    for (i = 0; i <= 360 / 30; i++) {
      cos_table[i] = 5 * cos(((valid_trig_angle(i * 30.0))*3.14159265358979323846/180.0));
      sin_table[i] = 5 * sin(((valid_trig_angle(i * 30.0))*3.14159265358979323846/180.0));
    }

    do {

      if (ubl_lcd_clicked()) {              
          lcd_setstatuspgm(("Mesh Validation Stopped."), 99);
          lcd_quick_feedback();
        ...
        while (ubl_lcd_clicked()) {          
          idle();                            
          lcd_setstatuspgm(("Unpress Wheel"), 99);
        }
        goto LEAVE;
      }
      ...
      if (location.x_index >= 0 && location.y_index >= 0) {
		...
		if(mksCfg . machinetype == 2)	{
          if ((pow(circle_x,2)+pow(circle_y,2)) > pow(mksCfg . delta_printable_radius,2)) {
            (serialprintPGM(errormagic));
            {serialprintPGM(("Attempt to print outside of DELTA_PRINTABLE_RADIUS." "\n"));};
            goto LEAVE;
          }		}
       

        
        if (!((circle_x) >= (mksCfg . x_min_pos) && (circle_x) <= (mksCfg . x_max_pos)) || !((circle_y) >= (mksCfg . y_min_pos) && (circle_y) <= (mksCfg . y_max_pos))) {
          (serialprintPGM(errormagic));
          {serialprintPGM(("Attempt to print off the bed." "\n"));};
          goto LEAVE;
        }
        ...
        if (ubl.g26_debug_flag) {
          {serial_echopair_P(("   Doing circle at: (xi="),(xi));};
          {serial_echopair_P((", yi="),(yi));};
          {if(!from_wifi_flag){customizedSerial . write(')');}else{serial2 . print(')');}};
          {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
        }
        ...
        if (xi == 0) {       
          ...
          if (yi == 0)        
            start_angle = 0.0;
          else if (yi == mksCfg . grid_max_points_y - 1)
            end_angle = 0.0;
        }
        else if (xi == mksCfg . grid_max_points_x - 1) { 
          start_angle =  90.0;
          end_angle   = 270.0;
          if (yi == 0)                  
            end_angle = 180.0;
        ...
        for (tmp = start_angle; tmp < end_angle - 0.1; tmp += 30.0) {
		  ...
		  if(mksCfg . machinetype == 2)	{
            if ((pow(x,2)+pow(y,2)) > pow(mksCfg . delta_printable_radius,2))   
              continue;       }                               
          
			else 	{
		  	x  = ((x) < (mksCfg . x_min_pos + 1))?(mksCfg . x_min_pos + 1):(((x) > (mksCfg . x_max_pos - 1))? (mksCfg . x_max_pos - 1) : (x)); 
            y  = ((y) < (mksCfg . y_min_pos + 1))?(mksCfg . y_min_pos + 1):(((y) > (mksCfg . y_max_pos - 1))? (mksCfg . y_max_pos - 1) : (y));
            xe = ((xe) < (mksCfg . x_min_pos + 1))?(mksCfg . x_min_pos + 1):(((xe) > (mksCfg . x_max_pos - 1))? (mksCfg . x_max_pos - 1) : (xe));
            ye = ((ye) < (mksCfg . y_min_pos + 1))?(mksCfg . y_min_pos + 1):(((ye) > (mksCfg . y_max_pos - 1))? (mksCfg . y_max_pos - 1) : (ye));	}
          ...
          print_line_from_here_to_there(((x) + workspace_offset[X_AXIS]), ((y) + workspace_offset[Y_AXIS]), layer_height, ((xe) + workspace_offset[X_AXIS]), ((ye) + workspace_offset[Y_AXIS]), layer_height);
    ...
    LEAVE:
    lcd_reset_alert_level();
    lcd_setstatuspgm(("Leaving G26"));

    retract_filament(destination);
    destination[Z_AXIS] = mksCfg . z_clearance_between_probes;
    ...
    for (uint8_t i = 0; i < mksCfg . grid_max_points_x; i++) {
      for (uint8_t j = 0; j < mksCfg . grid_max_points_y; j++) {
        if (!is_bit_set(circle_flags, i, j)) {
          const float mx = ubl.mesh_index_to_xpos[i],  
                      my = ubl.mesh_index_to_ypos[j];

          
          float f = sqrt((pow(X - mx,2)+pow(Y - my,2)));

          
          
          
          
          f += sqrt((pow(x_pos - mx,2)+pow(y_pos - my,2))) / 15.0;

          
          if (random_deviation > 1.0)
          {
            int16_t randData = rand()% (int)random_deviation;
            f += randData;
          }
        ...
        if (i < mksCfg . grid_max_points_x) { 
          ...
          if (is_bit_set(circle_flags, i, j) && is_bit_set(circle_flags, i + 1, j)) { 
            if (!is_bit_set(horizontal_mesh_line_flags, i, j)) {
              ...
              sx = ubl.mesh_index_to_xpos[  i  ] + (5 - (3)); 
              ex = ubl.mesh_index_to_xpos[i + 1] - (5 - (3)); 

              sx = ((sx) < (mksCfg . x_min_pos + 1))?(mksCfg . x_min_pos + 1):(((sx) > (mksCfg . x_max_pos - 1))? (mksCfg . x_max_pos - 1) : (sx));
              sy = ey = ((ubl . mesh_index_to_ypos[j]) < (mksCfg . y_min_pos + 1))?(mksCfg . y_min_pos + 1):(((ubl . mesh_index_to_ypos[j]) > (mksCfg . y_max_pos - 1))? (mksCfg . y_max_pos - 1) : (ubl . mesh_index_to_ypos[j]));
              ex = ((ex) < (mksCfg . x_min_pos + 1))?(mksCfg . x_min_pos + 1):(((ex) > (mksCfg . x_max_pos - 1))? (mksCfg . x_max_pos - 1) : (ex));

              if (ubl.g26_debug_flag) {
                {serial_echopair_P((" Connecting with horizontal line (sx="),(sx));};
                {serial_echopair_P((", sy="),(sy));};
                {serial_echopair_P((") -> (ex="),(ex));};
                {serial_echopair_P((", ey="),(ey));};
                {if(!from_wifi_flag){customizedSerial . write(')');}else{serial2 . print(')');}};
                {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
                
              }

              print_line_from_here_to_there(((sx) + workspace_offset[X_AXIS]), ((sy) + workspace_offset[Y_AXIS]), layer_height, ((ex) + workspace_offset[X_AXIS]), ((ey) + workspace_offset[Y_AXIS]), layer_height);
              bit_set(horizontal_mesh_line_flags, i, j);   
            }
          }

          if (j < mksCfg . grid_max_points_y) { 
            ...
            if (is_bit_set(circle_flags, i, j) && is_bit_set(circle_flags, i, j + 1)) { 
              if (!is_bit_set( vertical_mesh_line_flags, i, j)) {
                ...
                sy = ubl.mesh_index_to_ypos[  j  ] + (5 - (3)); 
                ey = ubl.mesh_index_to_ypos[j + 1] - (5 - (3)); 

                sx = ex = ((ubl . mesh_index_to_xpos[i]) < (mksCfg . x_min_pos + 1))?(mksCfg . x_min_pos + 1):(((ubl . mesh_index_to_xpos[i]) > (mksCfg . x_max_pos - 1))? (mksCfg . x_max_pos - 1) : (ubl . mesh_index_to_xpos[i]));
                sy = ((sy) < (mksCfg . y_min_pos + 1))?(mksCfg . y_min_pos + 1):(((sy) > (mksCfg . y_max_pos - 1))? (mksCfg . y_max_pos - 1) : (sy));
                ey = ((ey) < (mksCfg . y_min_pos + 1))?(mksCfg . y_min_pos + 1):(((ey) > (mksCfg . y_max_pos - 1))? (mksCfg . y_max_pos - 1) : (ey));

                if (ubl.g26_debug_flag) {
                  {serial_echopair_P((" Connecting with vertical line (sx="),(sx));};
                  {serial_echopair_P((", sy="),(sy));};
                  {serial_echopair_P((") -> (ex="),(ex));};
                  {serial_echopair_P((", ey="),(ey));};
                  {if(!from_wifi_flag){customizedSerial . write(')');}else{serial2 . print(')');}};
  ...
  void move_to(const float &x, const float &y, const float &z, const float &e_delta) {
    ...
    feed_value = has_xy_component ? ((((planner . max_feedrate_mm_s[X_AXIS])<(planner . max_feedrate_mm_s[Y_AXIS]))?(planner . max_feedrate_mm_s[X_AXIS]):(planner . max_feedrate_mm_s[Y_AXIS]))) / 10.0 : planner.max_feedrate_mm_s[E_AXIS] / 1.5;

    if (ubl.g26_debug_flag) do{ {serial_echopair_P(("in move_to() feed_value for XY:"),(feed_value));}; {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}}; }while(0);
  ...
  void print_line_from_here_to_there(const float &sx, const float &sy, const float &sz, const float &ex, const float &ey, const float &ez) {
    const float dx_s = current_position[X_AXIS] - sx,   
                ...
                dist_start = (pow(dx_s,2)+pow(dy_s,2)),        
                                                        
                dx_e = current_position[X_AXIS] - ex,   
                dy_e = current_position[Y_AXIS] - ey,
                dist_end = (pow(dx_e,2)+pow(dy_e,2)),

                line_length = sqrt((pow(ex - sx,2)+pow(ey - sy,2)));

    
    
    if (dist_end < dist_start && (5) < abs(line_length)) {
  ...
  bool parse_G26_parameters() {
    ...
    if (code_seen('B')) {
      ...
      if (!((bed_temp) >= (15) && (bed_temp) <= (140))) {
        {serialprintPGM(("?Specified bed temperature not plausible." "\n"));};
        return true;
      }
    }
    ...
    if (code_seen('L')) {
      ...
      if (!((layer_height) >= (0.0) && (layer_height) <= (2.0))) {
        {serialprintPGM(("?Specified layer height not plausible." "\n"));};
        return true;
      }
    }
    ...
    if (code_seen('Q')) {
      if (code_has_value()) {
        ...
        if (!((retraction_multiplier) >= (0.05) && (retraction_multiplier) <= (15.0))) {
          {serialprintPGM(("?Specified Retraction Multiplier not plausible." "\n"));};
          return true;
        }
      }
      else {
        {serialprintPGM(("?Retraction Multiplier must be specified." "\n"));};
        return true;
      }
    }
    ...
    if (code_seen('N') || code_seen('n')) {
      ...
      if (!((nozzle) >= (0.1) && (nozzle) <= (1.0))) {
        {serialprintPGM(("?Specified nozzle size not plausible." "\n"));};
        return true;
      }
    }
    ...
    if (code_seen('P')) {
      if (!code_has_value())
        ...
        if (!((prime_length) >= (0.0) && (prime_length) <= (25.0))) {
          {serialprintPGM(("?Specified prime length not plausible." "\n"));};
          return true;
        }
      }
    }
    ...
    if (code_seen('F')) {
      ...
      if (!((filament_diameter) >= (1.0) && (filament_diameter) <= (4.0))) {
        {serialprintPGM(("?Specified filament size not plausible." "\n"));};
        return true;
      }
    }
    extrusion_multiplier *= pow(1.75,2) / pow(filament_diameter,2);         
                                                                      
                                                                      

    extrusion_multiplier *= filament_diameter * pow(nozzle,2) / pow(0.3,2); 

    if (code_seen('H')) {
      hotend_temp = code_value_temp_abs();
      if (!((hotend_temp) >= (165) && (hotend_temp) <= (280))) {
        {serialprintPGM(("?Specified nozzle temperature not plausible." "\n"));};
        return true;
      }
    }
    ...
    if (code_seen('X')) {
      ...
      if (!((x_pos) >= (mksCfg . x_min_pos) && (x_pos) <= (mksCfg . x_max_pos))) {
        {serialprintPGM(("?Specified X coordinate not plausible." "\n"));};
        return true;
      }
    }
    else
    ...
    if (code_seen('Y')) {
      ...
      if (!((y_pos) >= (mksCfg . y_min_pos) && (y_pos) <= (mksCfg . y_max_pos))) {
        {serialprintPGM(("?Specified Y coordinate not plausible." "\n"));};
        return true;
      }
    }
  ...
  bool turn_on_heaters() {
   ...
   if(mksCfg . has_temp_bed) {
        if (bed_temp > 25) {
          lcd_setstatuspgm(("G26 Heating Bed."), 99);
          lcd_quick_feedback();
          ubl.has_control_of_lcd_panel = true;
          thermalManager.setTargetBed(bed_temp);
          next = HAL_GetTick() + 5000UL;
          while (abs(thermalManager.degBed() - bed_temp) > 3) {
            if (ubl_lcd_clicked()) return exit_from_g26();
            if (((long)(HAL_GetTick()-(next))<0)) {
              next = HAL_GetTick() + 5000UL;
              print_heaterstates();
              HAL_Delay(2000);      
            }
        ...
        lcd_setstatuspgm(("G26 Heating Nozzle."), 99);
        lcd_quick_feedback();
   }
  ...
  bool prime_nozzle() {
    ...
    if (prime_flag == -1) {  
      ...
      lcd_setstatuspgm(("User-Controlled Prime"), 99);
      chirp_at_user();
      ...
      while (!ubl_lcd_clicked()) {
        ...
        destination[E_AXIS] += 0.25;
          Total_Prime += 0.25;
          if (Total_Prime >= 300) return true;
      ...
      while (ubl_lcd_clicked()) idle();           
        ...
        lcd_setstatuspgm(("Done Priming"), 99);
        lcd_quick_feedback();
      ...
      ubl.has_control_of_lcd_panel = false;
        ...
        lcd_setstatuspgm(("Fixed Length Prime."), 99);
        lcd_quick_feedback();
      set_destination_to_current();
