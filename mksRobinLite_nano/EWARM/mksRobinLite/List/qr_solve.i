 ...
 
{
  ...
  for (int j = 0; j < n; j++) {
    for (int i = 0; i < m; i++) {
      do{ if (value < r8_abs(a[i + j * m])) value = r8_abs(a[i + j * m]); }while(0);
    }
  }
  return value;
}
 ...
 
{
  ...
  if (kr != 0) {
    job = 110;
    info = dqrsl(a, lda, m, kr, qraux, b, rsd, rsd, x, rsd, rsd, job); ((void)(info));
  }
 ...
 
{
	...
	float *a_qr,*qraux,*r,tol;
	a_qr = (float *)malloc(n*m*sizeof(float));
	qraux = (float *)malloc(n*sizeof(float));
	r = (float *)malloc(m*sizeof(float));
	int ind, itask, *jpvt, kr, lda;
  ...
  ind = dqrls(a_qr, lda, m, n, tol, &kr, b, x, r, jpvt, qraux, itask); ((void)(ind));
  free(a_qr);
  free(qraux);
  free(r);
  free(jpvt);
