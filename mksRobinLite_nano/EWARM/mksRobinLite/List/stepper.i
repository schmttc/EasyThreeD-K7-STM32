...
extern uint8_t cmd_id_bak;

Stepper stepper; 



block_t* Stepper::current_block = 0;  
...
volatile long Stepper::count_position[4] = { 0 };
volatile signed char Stepper::count_direction[4] = { 1, 1, 1, 1 };
...
void Stepper::set_directions() {
    ...
    if (motor_direction(X_AXIS)) { HAL_GPIO_WritePin(gArrayGpioPort[2],gArrayGpioPin[2],(GPIO_PinState)mksCfg . invert_x_dir); count_direction[X_AXIS] = -1; } else { HAL_GPIO_WritePin(gArrayGpioPort[2],gArrayGpioPin[2],(GPIO_PinState)!mksCfg . invert_x_dir); count_direction[X_AXIS] = 1; }; 
    if (motor_direction(Y_AXIS)) { HAL_GPIO_WritePin(gArrayGpioPort[5],gArrayGpioPin[5],(GPIO_PinState)mksCfg . invert_y_dir); count_direction[Y_AXIS] = -1; } else { HAL_GPIO_WritePin(gArrayGpioPort[5],gArrayGpioPin[5],(GPIO_PinState)!mksCfg . invert_y_dir); count_direction[Y_AXIS] = 1; }; 
    if (motor_direction(Z_AXIS)) { HAL_GPIO_WritePin(gArrayGpioPort[8],gArrayGpioPin[8],(GPIO_PinState)mksCfg . invert_z_dir); count_direction[Z_AXIS] = -1; } else { HAL_GPIO_WritePin(gArrayGpioPort[8],gArrayGpioPin[8],(GPIO_PinState)!mksCfg . invert_z_dir); count_direction[Z_AXIS] = 1; }; 

    if (motor_direction(E_AXIS)) {
      HAL_GPIO_WritePin(gArrayGpioPort[12],gArrayGpioPin[12],(GPIO_PinState)mksCfg . invert_e0_dir);
...
void Stepper::StepperHandler()	{
  ...
  uint16_t ocr_val;
    ...
    if (step_remaining && (endstops . enabled || endstops . z_probe_enabled)) {   
      endstops.update();
      if (step_remaining > 3000) {
        step_remaining -= 3000;
        ocr_val = 3000;
      }
      else {
      ...
      do{htim2 . Instance ->ARR = ocr_val; if(htim2 . Instance ->CNT>(ocr_val-16)) htim2 . Instance ->CNT=ocr_val-16;}while(0);

      do{ if (htim2 . Instance ->ARR < htim2 . Instance ->CNT + 16) htim2 . Instance ->ARR = htim2 . Instance ->CNT + 16; }while(0);

      do { __asm volatile("cpsie i"); if (thermalManager . in_temp_isr) HAL_TIM_Base_Stop_IT(&htim4); else HAL_TIM_Base_Start_IT(&htim4); HAL_TIM_Base_Start_IT(&htim2); } while(0); 
      return;
    }
  ...
  if (cleaning_buffer_counter) {
    ...
    planner.discard_current_block();
      if (!cleaning_buffer_counter && (true)) enqueue_and_echo_commands_P(("M84 X Y Z E"));
    do{htim2 . Instance ->ARR = 200; if(htim2 . Instance ->CNT>(200-16)) htim2 . Instance ->CNT=200-16;}while(0); 
    do { __asm volatile("cpsie i"); if (thermalManager . in_temp_isr) HAL_TIM_Base_Stop_IT(&htim4); else HAL_TIM_Base_Start_IT(&htim4); HAL_TIM_Base_Start_IT(&htim2); } while(0); 
    return;
  }
  ...
  if (!current_block) {
    ...
    if (current_block) {
      ...
      do{htim2 . Instance ->ARR = 2000; if(htim2 . Instance ->CNT>(2000-16)) htim2 . Instance ->CNT=2000-16;}while(0); 
      do { __asm volatile("cpsie i"); if (thermalManager . in_temp_isr) HAL_TIM_Base_Stop_IT(&htim4); else HAL_TIM_Base_Start_IT(&htim4); HAL_TIM_Base_Start_IT(&htim2); } while(0); 
      return;
    }
  }
  ...
  cmd_id_bak =current_block->cmd_id;
  ...
  for (uint8_t i = step_loops; i--;) {
      ...
      counter_X += current_block->steps[X_AXIS]; if (counter_X > 0) { HAL_GPIO_WritePin(gArrayGpioPort[3],gArrayGpioPin[3],(GPIO_PinState)!false); };
      counter_Y += current_block->steps[Y_AXIS]; if (counter_Y > 0) { HAL_GPIO_WritePin(gArrayGpioPort[6],gArrayGpioPin[6],(GPIO_PinState)!false); };
      counter_Z += current_block->steps[Z_AXIS]; if (counter_Z > 0) { HAL_GPIO_WritePin(gArrayGpioPort[9],gArrayGpioPin[9],(GPIO_PinState)!false); };
    
        counter_E += current_block->steps[E_AXIS]; if (counter_E > 0) { HAL_GPIO_WritePin(gArrayGpioPort[13],gArrayGpioPin[13],(GPIO_PinState)!false); };

    

      if (counter_X > 0) { counter_X -= current_block->step_event_count; count_position[X_AXIS] += count_direction[X_AXIS]; HAL_GPIO_WritePin(gArrayGpioPort[3],gArrayGpioPin[3],(GPIO_PinState)false); };
      if (counter_Y > 0) { counter_Y -= current_block->step_event_count; count_position[Y_AXIS] += count_direction[Y_AXIS]; HAL_GPIO_WritePin(gArrayGpioPort[6],gArrayGpioPin[6],(GPIO_PinState)false); };
      if (counter_Z > 0) { counter_Z -= current_block->step_event_count; count_position[Z_AXIS] += count_direction[Z_AXIS]; HAL_GPIO_WritePin(gArrayGpioPort[9],gArrayGpioPin[9],(GPIO_PinState)false); };

        if (counter_E > 0) { counter_E -= current_block->step_event_count; count_position[E_AXIS] += count_direction[E_AXIS]; HAL_GPIO_WritePin(gArrayGpioPort[13],gArrayGpioPin[13],(GPIO_PinState)false); };
  ...
  if (step_events_completed <= (uint32_t)current_block->accelerate_until) {
    ...
    acc_step_rate = (uint16_t)((((uint64_t)acceleration_time * (uint64_t)current_block->acceleration_rate)>> 24)& 0XFFFF);;
    acc_step_rate += current_block->initial_rate;

    
    do{ if (acc_step_rate > current_block->nominal_rate) acc_step_rate = current_block->nominal_rate; }while(0);

    
    const uint16_t timer = calc_timer(acc_step_rate);

    do { (ocr_val = (uint16_t)timer); if ((endstops . enabled || endstops . z_probe_enabled) && timer > 3000) { const uint16_t remainder = (uint16_t)timer % (3000); ocr_val = (remainder < 1000) ? 3000 + remainder : 3000; step_remaining = (uint16_t)timer - ocr_val; } } while(0);  
    do{htim2 . Instance ->ARR = ocr_val; if(htim2 . Instance ->CNT>(ocr_val-16)) htim2 . Instance ->CNT=ocr_val-16;}while(0);
    ...
    uint16_t step_rate;
    step_rate = (uint16_t)((((uint64_t)deceleration_time * (uint64_t)current_block->acceleration_rate)>> 24)& 0XFFFF);;

    if (step_rate < acc_step_rate) { 
      step_rate = acc_step_rate - step_rate;
      do{ if (step_rate < current_block->final_rate) step_rate = current_block->final_rate; }while(0);
    }
    else
    ...
    do { (ocr_val = (uint16_t)OCR1A_nominal); if ((endstops . enabled || endstops . z_probe_enabled) && OCR1A_nominal > 3000) { const uint16_t remainder = (uint16_t)OCR1A_nominal % (3000); ocr_val = (remainder < 1000) ? 3000 + remainder : 3000; step_remaining = (uint16_t)OCR1A_nominal - ocr_val; } } while(0);  
    do{htim2 . Instance ->ARR = ocr_val; if(htim2 . Instance ->CNT>(ocr_val-16)) htim2 . Instance ->CNT=ocr_val-16;}while(0);
...
void Stepper::init() {
    ...
    ;
    if (!0) HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)GPIO_PIN_SET);
    ;
    if (!0) HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)GPIO_PIN_SET);
    ;
    if (!0) HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)GPIO_PIN_SET);
    ;
    if (!0) HAL_GPIO_WritePin(gArrayGpioPort[30],gArrayGpioPin[30],(GPIO_PinState)GPIO_PIN_SET);
  ...
  endstops.init();
    ...
    ; HAL_GPIO_WritePin(gArrayGpioPort[3],gArrayGpioPin[3],(GPIO_PinState)false); do{ HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)!0); axis_known_position[X_AXIS] = false; }while(0);

    ; HAL_GPIO_WritePin(gArrayGpioPort[6],gArrayGpioPin[6],(GPIO_PinState)false); do{ HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)!0); axis_known_position[Y_AXIS] = false; }while(0);

    ; HAL_GPIO_WritePin(gArrayGpioPort[9],gArrayGpioPin[9],(GPIO_PinState)false); do{ HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)!0); axis_known_position[Z_AXIS] = false; }while(0);

    ; HAL_GPIO_WritePin(gArrayGpioPort[13],gArrayGpioPin[13],(GPIO_PinState)false); HAL_GPIO_WritePin(gArrayGpioPort[30],gArrayGpioPin[30],(GPIO_PinState)!0);
...
float Stepper::get_axis_position_mm(AxisEnum axis) {
  ...
  if(mksCfg . machinetype & ((4 | 8) | (16 | 32) | (64 | 128))) {
    ...
    if (axis == mksTmp . core_axis_1 || axis == mksTmp . core_axis_2) {
      __asm volatile("cpsid i");;
      
      
      axis_steps = 0.5f * (
        axis == mksTmp . core_axis_2 ? CORESIGN(count_position[mksTmp . core_axis_1] - count_position[mksTmp . core_axis_2])
                            : count_position[mksTmp . core_axis_1] + count_position[mksTmp . core_axis_2]
      );
      __asm volatile("cpsie i");;
    }
    else
...
void Stepper::report_positions() {
  ...
  __asm volatile("cpsie i");;
  	...
  	if(mksCfg . machinetype&(256 | 512) || mksCfg . machinetype&(16 | 32) || mksCfg . machinetype&(256 | 512))
    { {serialprintPGM((" Count A:"));};}
  
  else
  {{serialprintPGM((" Count X:"));};}

  {if(!from_wifi_flag){customizedSerial . print(xpos);}else{serial2 . print(xpos);}};

 
  if(mksCfg . machinetype&(4 | 8) || mksCfg . machinetype&(64 | 128) || mksCfg . machinetype&(256 | 512))
  { {serialprintPGM((" B:"));};}
  else
  { {serialprintPGM((" Y:"));};}
  
  {if(!from_wifi_flag){customizedSerial . print(ypos);}else{serial2 . print(ypos);}};
  ...
  {if(!from_wifi_flag){customizedSerial . print(zpos);}else{serial2 . print(zpos);}};
