...
extern uint8_t cmd_queue_index_r_bak;
...
block_t Planner::block_buffer[16];
volatile uint8_t Planner::block_buffer_head = 0,           
                 Planner::block_buffer_tail = 0;

float Planner::max_feedrate_mm_s[4], 
      Planner::axis_steps_per_mm[4],
      Planner::steps_to_mm[4];

 
unsigned long Planner::max_acceleration_steps_per_s2[4],
         	  Planner::max_acceleration_mm_per_s2[4]; 
...
long Planner::position[4] = { 0 };

uint32_t Planner::cutoff_long;

float Planner::previous_speed[4],
      Planner::previous_nominal_speed;

  uint8_t Planner::g_uc_extruder_last_move[1] = { 0 };
...
void Planner::calculate_trapezoid_for_block(block_t* const block, const float &entry_factor, const float &exit_factor) {
  ...
  do{ if (initial_rate < 120) initial_rate = 120; }while(0);
  do{ if (final_rate < 120) final_rate = 120; }while(0);
  ...
  if (plateau_steps < 0) {
    ...
    do{ if (accelerate_steps < 0) accelerate_steps = 0; }while(0); 
    accelerate_steps = ((((uint32_t)accelerate_steps)<(block->step_event_count))?((uint32_t)accelerate_steps):(block->step_event_count));
    plateau_steps = 0;
  }
  ...
  __asm volatile("cpsid i");;  
  if (!(((block->flag)&(1<<(BLOCK_BIT_BUSY)))!=0)) { 
...
void Planner::reverse_pass_kernel(block_t* const current, const block_t *next) {
  ...
  if (current->entry_speed != max_entry_speed) {
    ...
    current->entry_speed = ((((current->flag)&(1<<(BLOCK_BIT_NOMINAL_LENGTH)))!=0) || max_entry_speed <= next->entry_speed)
      ? max_entry_speed
      : (((max_entry_speed)<(max_allowable_speed(-current->acceleration, next->entry_speed, current->millimeters)))?(max_entry_speed):(max_allowable_speed(-current->acceleration, next->entry_speed, current->millimeters)));
    (current->flag |= (1<<(BLOCK_BIT_RECALCULATE)));
  }
}
...
void Planner::reverse_pass() {
  ...
  if (movesplanned() > 3) {
    ...
    uint8_t b = ((block_buffer_head - 3)&(16-1));
    while (b != tail) {
      if (block[0] && (((block[0]->flag)&(1<<(BLOCK_BIT_START_FROM_FULL_HALT)))!=0)) break;
...
void Planner::forward_pass_kernel(const block_t* previous, block_t* const current) {
  ...
  if (!(((previous->flag)&(1<<(BLOCK_BIT_NOMINAL_LENGTH)))!=0)) {
    if (previous->entry_speed < current->entry_speed) {
      float entry_speed = (((current->entry_speed)<(max_allowable_speed(-previous->acceleration, previous->entry_speed, previous->millimeters)))?(current->entry_speed):(max_allowable_speed(-previous->acceleration, previous->entry_speed, previous->millimeters)));
...
void Planner::recalculate_trapezoids() {
  ...
  while (block_index != block_buffer_head) {
    ...
    if (current) {
      ...
      if ((((current->flag)&(1<<(BLOCK_BIT_RECALCULATE)))!=0) || (((next->flag)&(1<<(BLOCK_BIT_RECALCULATE)))!=0)) {
  ...
  if (next) {
    ...
    calculate_trapezoid_for_block(next, next->entry_speed / nom, (0.05) / nom);
    (next->flag &= ~(1<<(BLOCK_BIT_RECALCULATE)));
  }
}
  ...
  void Planner::getHighESpeed() {
    ...
    for (uint8_t b = block_buffer_tail; b != block_buffer_head; b = next_block_index(b)) {
      ...
      if (block->steps[X_AXIS] || block->steps[Y_AXIS] || block->steps[Z_AXIS]) {
        ...
        do{ if (high < se) high = se; }while(0);
      }
    }

    float t = autotemp_min + high * autotemp_factor;
    t = ((t) < (autotemp_min))?(autotemp_min):(((t) > (autotemp_max))? (autotemp_max) : (t));
    if (t < oldt) t = t * (1 - (0.98)) + oldt * (0.98);
    oldt = t;
...
void Planner::check_axes_activity() {
  unsigned char axis_active[4] = { 0 },
                tail_fan_speed[1];

    for (uint8_t i = 0; i < 1; i++) tail_fan_speed[i] = fanSpeeds[i];


  if (blocks_queued()) {

      for (uint8_t i = 0; i < 1; i++) tail_fan_speed[i] = block_buffer[block_buffer_tail].fan_speed[i];
    ...
    for (uint8_t b = block_buffer_tail; b != block_buffer_head; b = next_block_index(b)) {
      ...
      for (uint8_t i=X_AXIS; i<=E_AXIS; i++) if (block->steps[i]) axis_active[i]++;
    }
  }
  ...
  void Planner::apply_leveling(float &lx, float &ly, float &lz) {
	...
	if(mksCfg . bed_leveling_method&((4 | 2) | (4 | 8) | 16))
		{
	...
	if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))	
		{
      if (z_fade_height) {
        const float raw_lz = ((lz) - workspace_offset[Z_AXIS]);
		...
		if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))	
			{
		  		lz += mbl.get_z(((lx) - workspace_offset[X_AXIS]), ((ly) - workspace_offset[Y_AXIS]) , z_fade_factor);
			}
			else
				{
				lz += mbl.get_z_mks(((lx) - workspace_offset[X_AXIS]), ((ly) - workspace_offset[Y_AXIS]));
				}
		}
		}
		else if(mksCfg . bed_leveling_method&(4 | 2))
			{
			float dx = ((lx) - workspace_offset[X_AXIS]) - (0),
				  dy = ((ly) - workspace_offset[Y_AXIS]) - (0),
				  dz = ((lz) - workspace_offset[Z_AXIS]);
			
			apply_rotation_xyz(bed_level_matrix, dx, dy, dz);
			
			lx = ((dx + 0) + workspace_offset[X_AXIS]);
			ly = ((dy + 0) + workspace_offset[Y_AXIS]);
			lz = ((dz) + workspace_offset[Z_AXIS]);
			}
  ...
  void Planner::unapply_leveling(float logical[3]) {
	...
	if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))
      if (z_fade_height && ((logical[Z_AXIS]) - workspace_offset[Z_AXIS]) >= z_fade_height) return;
		...
		if (mbl.active()) {
                      if((mksCfg . machinetype != 2) && (mksCfg . bed_leveling_method&( 16 )))	
                      {
						const float c = mbl.get_z(((logical[X_AXIS]) - workspace_offset[X_AXIS]), ((logical[Y_AXIS]) - workspace_offset[Y_AXIS]), 1.0);
						logical[Z_AXIS] = (z_fade_height * (((logical[Z_AXIS]) - workspace_offset[Z_AXIS]) - c)) / (z_fade_height - c);
                      }
					  else
					  	{
						logical[Z_AXIS] -= mbl.get_z_mks(((logical[X_AXIS]) - workspace_offset[X_AXIS]), ((logical[Y_AXIS]) - workspace_offset[Y_AXIS]));
					  	}
		}
		...
		float dx = ((logical[X_AXIS]) - workspace_offset[X_AXIS]) - (0),
			  dy = ((logical[Y_AXIS]) - workspace_offset[Y_AXIS]) - (0),
			  dz = ((logical[Z_AXIS]) - workspace_offset[Z_AXIS]);
		
		apply_rotation_xyz(inverse, dx, dy, dz);
		
		logical[X_AXIS] = ((dx + 0) + workspace_offset[X_AXIS]);
		logical[Y_AXIS] = ((dy + 0) + workspace_offset[Y_AXIS]);
		logical[Z_AXIS] = ((dz) + workspace_offset[Z_AXIS]);
...
void Planner::_buffer_line(const float &a, const float &b, const float &c, const float &e, float fr_mm_s, const uint8_t extruder) {
  ...
  long de = target[E_AXIS] - position[E_AXIS];
    ...
    if (de) {
      if (thermalManager.tooColdToExtrude(extruder)) {
        ...
        if (labs(de) > (int32_t)axis_steps_per_mm[E_AXIS] * (300)) { 
          position[E_AXIS] = target[E_AXIS]; 
          de = 0; 
          {serialprintPGM(echomagic);};
  ...
  uint8_t dm = 0;
   ...
   if(mksCfg . machinetype & (4 | 8))
	   ...
	   if (db < 0) (dm |= (1<<(Y_HEAD))); 			   
	   if (dc < 0) (dm |= (1<<(Z_AXIS)));
	   if (da + db < 0) (dm |= (1<<(A_AXIS)));		   
	   if (CORESIGN(da - db) < 0) (dm |= (1<<(B_AXIS))); 
   	}
		...
		if (db < 0) (dm |= (1<<(Y_AXIS)));
		if (dc < 0) (dm |= (1<<(Z_HEAD)));				
		if (da + dc < 0) (dm |= (1<<(A_AXIS)));			
		if (CORESIGN(da - dc) < 0) (dm |= (1<<(C_AXIS))); 
		}
		...
		if (db < 0) (dm |= (1<<(Y_HEAD)));				
		if (dc < 0) (dm |= (1<<(Z_HEAD)));				
		if (db + dc < 0) (dm |= (1<<(B_AXIS)));			
		if (CORESIGN(db - dc) < 0) (dm |= (1<<(C_AXIS))); 
		}
	else
		{
		if (da < 0) (dm |= (1<<(X_AXIS)));
  ...
  block->cmd_id = cmd_queue_index_r_bak;
  ...
  block->step_event_count = (((((((((block->steps[X_AXIS])>(block->steps[Y_AXIS]))?(block->steps[X_AXIS]):(block->steps[Y_AXIS])))>(block->steps[Z_AXIS]))?((((block->steps[X_AXIS])>(block->steps[Y_AXIS]))?(block->steps[X_AXIS]):(block->steps[Y_AXIS]))):(block->steps[Z_AXIS])))>(esteps))?(((((((block->steps[X_AXIS])>(block->steps[Y_AXIS]))?(block->steps[X_AXIS]):(block->steps[Y_AXIS])))>(block->steps[Z_AXIS]))?((((block->steps[X_AXIS])>(block->steps[Y_AXIS]))?(block->steps[X_AXIS]):(block->steps[Y_AXIS]))):(block->steps[Z_AXIS]))):(esteps));

  
  if (block->step_event_count < 6) return;

  

    for (uint8_t i = 0; i < 1; i++) block->fan_speed[i] = fanSpeeds[i];
  ...
  if(mksCfg . machinetype & (4 | 8))
	  ...
	  if (block->steps[A_AXIS] || block->steps[B_AXIS]) {
		...
		if (block->steps[Z_AXIS]) HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)0);
  	}
  else if(mksCfg . machinetype & (16 | 32))
  	{
	  ...
	  if (block->steps[Y_AXIS]) HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)0);
  	}
  else if(mksCfg . machinetype & (64 | 128))
  	{
	  ...
	  if (block->steps[X_AXIS]) HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)0);
  	}
	else
	{
		if (block->steps[X_AXIS]) HAL_GPIO_WritePin(gArrayGpioPort[14],gArrayGpioPin[14],(GPIO_PinState)0);
  ...
  if (esteps) {
      ...
      switch(extruder) {
        case 0:
          ...
          g_uc_extruder_last_move[0] = (16) * 2;
        break;
      }
  }

  if (esteps)
    do{ if (fr_mm_s < min_feedrate_mm_s) fr_mm_s = min_feedrate_mm_s; }while(0);
  else
    do{ if (fr_mm_s < min_travel_feedrate_mm_s) fr_mm_s = min_travel_feedrate_mm_s; }while(0);
  ...
  if (block->steps[X_AXIS] < 6 && block->steps[Y_AXIS] < 6 && block->steps[Z_AXIS] < 6) {
	...
	if(mksCfg . machinetype & (4 | 8))
		block->millimeters = sqrt(pow(delta_mm[X_HEAD],2) + pow(delta_mm[Y_HEAD],2) + pow(delta_mm[Z_AXIS],2));
	else if	(mksCfg . machinetype & (16 | 32))
		block->millimeters = sqrt(pow(delta_mm[X_HEAD],2) + pow(delta_mm[Y_AXIS],2) + pow(delta_mm[Z_HEAD],2));
	else if (mksCfg . machinetype & (64 | 128))
		block->millimeters = sqrt(pow(delta_mm[X_AXIS],2) + pow(delta_mm[Y_HEAD],2) + pow(delta_mm[Z_HEAD],2));
	else	
  ...
  if(!(mksCfg . machinetype & (2 | (256 | 512))))
  {    
    if (((moves_queued) >= (2) && (moves_queued) <= ((16) / 2 - 1))) {
      if (segment_time < min_segment_time) {
        
         
        
        inverse_mm_s = 1000000.0 / (segment_time + lround((float)2 * (min_segment_time - segment_time) / moves_queued));
  ...
  float current_speed[4], speed_factor = 1.0; 
  for (uint8_t i=X_AXIS; i<=E_AXIS; i++) {
    const float cs = fabs(current_speed[i] = delta_mm[i] * inverse_mm_s);
    if (cs > max_feedrate_mm_s[i]) do{ if (speed_factor > max_feedrate_mm_s[i] / cs) speed_factor = max_feedrate_mm_s[i] / cs; }while(0);
  }
  ...
  if (speed_factor < 1.0) {
    for (uint8_t i=X_AXIS; i<=E_AXIS; i++) current_speed[i] *= speed_factor;
  ...
  if (!block->steps[X_AXIS] && !block->steps[Y_AXIS] && !block->steps[Z_AXIS]) {
    ...
    if (block->step_event_count <= cutoff_long) {
      do{ if (block->steps[X_AXIS] && max_acceleration_steps_per_s2[X_AXIS+0] < accel) { const uint32_t comp = max_acceleration_steps_per_s2[X_AXIS+0] * block->step_event_count; if (accel * block->steps[X_AXIS] > comp) accel = comp / block->steps[X_AXIS]; } }while(0);
      do{ if (block->steps[Y_AXIS] && max_acceleration_steps_per_s2[Y_AXIS+0] < accel) { const uint32_t comp = max_acceleration_steps_per_s2[Y_AXIS+0] * block->step_event_count; if (accel * block->steps[Y_AXIS] > comp) accel = comp / block->steps[Y_AXIS]; } }while(0);
      do{ if (block->steps[Z_AXIS] && max_acceleration_steps_per_s2[Z_AXIS+0] < accel) { const uint32_t comp = max_acceleration_steps_per_s2[Z_AXIS+0] * block->step_event_count; if (accel * block->steps[Z_AXIS] > comp) accel = comp / block->steps[Z_AXIS]; } }while(0);
      do{ if (block->steps[E_AXIS] && max_acceleration_steps_per_s2[E_AXIS+0] < accel) { const uint32_t comp = max_acceleration_steps_per_s2[E_AXIS+0] * block->step_event_count; if (accel * block->steps[E_AXIS] > comp) accel = comp / block->steps[E_AXIS]; } }while(0);
    }
    else {
      do{ if (block->steps[X_AXIS] && max_acceleration_steps_per_s2[X_AXIS+0] < accel) { const float comp = (float)max_acceleration_steps_per_s2[X_AXIS+0] * (float)block->step_event_count; if ((float)accel * (float)block->steps[X_AXIS] > comp) accel = comp / (float)block->steps[X_AXIS]; } }while(0);
      do{ if (block->steps[Y_AXIS] && max_acceleration_steps_per_s2[Y_AXIS+0] < accel) { const float comp = (float)max_acceleration_steps_per_s2[Y_AXIS+0] * (float)block->step_event_count; if ((float)accel * (float)block->steps[Y_AXIS] > comp) accel = comp / (float)block->steps[Y_AXIS]; } }while(0);
      do{ if (block->steps[Z_AXIS] && max_acceleration_steps_per_s2[Z_AXIS+0] < accel) { const float comp = (float)max_acceleration_steps_per_s2[Z_AXIS+0] * (float)block->step_event_count; if ((float)accel * (float)block->steps[Z_AXIS] > comp) accel = comp / (float)block->steps[Z_AXIS]; } }while(0);
      do{ if (block->steps[E_AXIS] && max_acceleration_steps_per_s2[E_AXIS+0] < accel) { const float comp = (float)max_acceleration_steps_per_s2[E_AXIS+0] * (float)block->step_event_count; if ((float)accel * (float)block->steps[E_AXIS] > comp) accel = comp / (float)block->steps[E_AXIS]; } }while(0);
    }
  }
  ...
  block->acceleration_rate = (long)(accel * 8.388608); 
  ...
  if (moves_queued > 1 && previous_nominal_speed > 0.0001) {
    ...
    for (uint8_t axis=X_AXIS; axis<=E_AXIS; axis++) {
      ...
      const float jerk = (v_exit > v_entry)
          ? 
            ( (v_entry > 0.f || v_exit < 0.f) ? (v_exit - v_entry) : (((v_exit)>(-v_entry))?(v_exit):(-v_entry)) )
          : 
            ( (v_entry < 0.f || v_exit > 0.f) ? (v_entry - v_exit) : (((-v_exit)>(v_entry))?(-v_exit):(v_entry)) );
    ...
    if (previous_safe_speed > vmax_junction_threshold && safe_speed > vmax_junction_threshold) {
      ...
      (block->flag |= (1<<(BLOCK_BIT_START_FROM_FULL_HALT)));
  ...
  const float v_allowable = max_allowable_speed(-block->acceleration, 0.05, block->millimeters);
  ...
  memcpy(previous_speed,current_speed,(((sizeof(previous_speed))<(sizeof(current_speed)))?(sizeof(previous_speed)):(sizeof(current_speed))));
  ...
  memcpy(position,target,(((sizeof(position))<(sizeof(target)))?(sizeof(position)):(sizeof(target))));
...
void Planner::set_position_mm_kinematic(const float position[4]) {
  ...
  float lpos[4];
  if(mksCfg . bed_leveling_method&((4 | 2) | (4 | 8) | 32))	{
  	lpos[0] = position[X_AXIS];
  	lpos[1] = position[Y_AXIS];
  	lpos[2] = position[Z_AXIS];
	apply_leveling(lpos);
  	}
  else
  	{
...
void Planner::reset_acceleration_rates() {
  ...
  for (uint8_t i=X_AXIS; i<4; i++) {
    ...
    if (true) do{ if (highest_rate < max_acceleration_steps_per_s2[i]) highest_rate = max_acceleration_steps_per_s2[i]; }while(0);
  }
...
void Planner::refresh_positioning() {
  for (uint8_t i=X_AXIS; i<4; i++) steps_to_mm[i] = 1.0 / axis_steps_per_mm[i];
