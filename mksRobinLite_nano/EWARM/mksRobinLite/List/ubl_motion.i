...
{  {if(!from_wifi_flag){customizedSerial . print(destination[X_AXIS],6);}else{serial2 . print(destination[X_AXIS],6);}};}
  }
  ...
  void debug_current_and_destination(const char *title) {
    ...
    const float dx = current_position[X_AXIS] - destination[X_AXIS],
                ...
                xy_dist = sqrt((pow(dx,2)+pow(dy,2)));
    ...
    if (xy_dist == 0.0) {
      ...
      {if(!from_wifi_flag){customizedSerial . print(fpmm,6);}else{serial2 . print(fpmm,6);}};
    }

    {serialprintPGM(("    current=( "));};
    {if(!from_wifi_flag){customizedSerial . print(current_position[X_AXIS],6);}else{serial2 . print(current_position[X_AXIS],6);}};
    {serialprintPGM((", "));};
    {if(!from_wifi_flag){customizedSerial . print(current_position[Y_AXIS],6);}else{serial2 . print(current_position[Y_AXIS],6);}};
    {serialprintPGM((", "));};
    {if(!from_wifi_flag){customizedSerial . print(current_position[Z_AXIS],6);}else{serial2 . print(current_position[Z_AXIS],6);}};
    {serialprintPGM((", "));};
    {if(!from_wifi_flag){customizedSerial . print(current_position[E_AXIS],6);}else{serial2 . print(current_position[E_AXIS],6);}};
    {serialprintPGM((" )   destination=( "));};
    ...
    {serialprintPGM((" )   "));};
    {if(!from_wifi_flag){customizedSerial . print(title);}else{serial2 . print(title);}};
    {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
  ...
  void ubl_line_to_destination(const float &feed_rate, uint8_t extruder) {
    ...
    const int cell_start_xi = ubl.get_cell_index_x(((start[X_AXIS]) - workspace_offset[X_AXIS])),
              cell_start_yi = ubl.get_cell_index_y(((start[Y_AXIS]) - workspace_offset[Y_AXIS])),
              cell_dest_xi  = ubl.get_cell_index_x(((end[X_AXIS]) - workspace_offset[X_AXIS])),
              cell_dest_yi  = ubl.get_cell_index_y(((end[Y_AXIS]) - workspace_offset[Y_AXIS]));

    if (ubl.g26_debug_flag) {
      {serial_echopair_P((" ubl_line_to_destination(xe="),(end[X_AXIS]));};
      {serial_echopair_P((", ye="),(end[Y_AXIS]));};
      {serial_echopair_P((", ze="),(end[Z_AXIS]));};
      {serial_echopair_P((", ee="),(end[E_AXIS]));};
      {if(!from_wifi_flag){customizedSerial . write(')');}else{serial2 . print(')');}};
      {if(!from_wifi_flag){customizedSerial . write('\n');}else{serial2 . print('\n');}};
      debug_current_and_destination(("Start of ubl_line_to_destination()"));
    }
    ...
    if (cell_start_xi == cell_dest_xi && cell_start_yi == cell_dest_yi) { 
      ...
      if (!((cell_dest_xi) >= (0) && (cell_dest_xi) <= (mksCfg . grid_max_points_x - 1)) || !((cell_dest_yi) >= (0) && (cell_dest_yi) <= (mksCfg . grid_max_points_y - 1))) {
        ...
        if (ubl.g26_debug_flag)
          debug_current_and_destination(("out of bounds in ubl_line_to_destination()"));
      ...
      const float xratio = (((end[X_AXIS]) - workspace_offset[X_AXIS]) - ubl.mesh_index_to_xpos[cell_dest_xi]) * (1.0 / ((float((mksCfg . x_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . x_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_x - 1)))),
      ...
      const float yratio = (((end[Y_AXIS]) - workspace_offset[Y_AXIS]) - ubl.mesh_index_to_ypos[cell_dest_yi]) * (1.0 / ((float((mksCfg . y_max_pos - (mksCfg . ubl_mesh_inset)) - ((mksCfg . y_min_pos + mksCfg . ubl_mesh_inset))) / float(mksCfg . grid_max_points_y - 1))));
      ...
      if (ubl.g26_debug_flag)
        debug_current_and_destination(("FINAL_MOVE in ubl_line_to_destination()"));
    ...
    if (dxi == 0) {       
      ...
      while (current_yi != cell_dest_yi + down_flag) {
        current_yi += dyi;
        const float next_mesh_line_y = ((ubl . mesh_index_to_ypos[current_yi]) + workspace_offset[Y_AXIS]);
        ...
        const float y = ((ubl . mesh_index_to_ypos[current_yi]) + workspace_offset[Y_AXIS]);
      ...
      if (ubl.g26_debug_flag)
        debug_current_and_destination(("vertical move done in ubl_line_to_destination()"));
    ...
    if (dyi == 0) {             
      ...
      while (current_xi != cell_dest_xi + left_flag) {
        current_xi += dxi;
        const float next_mesh_line_x = ((ubl . mesh_index_to_xpos[current_xi]) + workspace_offset[X_AXIS]),
        ...
        const float x = ((ubl . mesh_index_to_xpos[current_xi]) + workspace_offset[X_AXIS]);
      ...
      if (ubl.g26_debug_flag)
        debug_current_and_destination(("horizontal move done in ubl_line_to_destination()"));
    ...
    while (xi_cnt > 0 || yi_cnt > 0) {
      ...
      const float next_mesh_line_x = ((ubl . mesh_index_to_xpos[current_xi + dxi]) + workspace_offset[X_AXIS]),
                  next_mesh_line_y = ((ubl . mesh_index_to_ypos[current_yi + dyi]) + workspace_offset[Y_AXIS]),
                  y = m * next_mesh_line_x + c,   
    ...
    if (ubl.g26_debug_flag)
      debug_current_and_destination(("generic move done in ubl_line_to_destination()"));
