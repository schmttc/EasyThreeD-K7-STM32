...
void PrintCounter::initStats() {
  ...
  uint8_t eppr_w=0x16;
  AT24CXX_Write((uint16_t ) this->address,&eppr_w,1); 
}
...
void PrintCounter::loadStats() {
        ...
        uint8_t eppr_r;
        AT24CXX_Read((uint16_t )this->address,&eppr_r,1);
        if(eppr_r != 0x16) this->initStats();
	else 
	{
	  int BufferSize=sizeof(printStatistics);	  
	  uint16_t MemAddress =this->address + sizeof(uint8_t);
	  uint16_t* pBuffer = &(this->data.totalPrints); 
...
void PrintCounter::showStats() {
  ...
  {serialprintPGM(("Stats: "));};

  {serialprintPGM(("Prints: "));};
  {if(!from_wifi_flag){customizedSerial . print(this->data . totalPrints);}else{serial2 . print(this->data . totalPrints);}};

  {serialprintPGM((", Finished: "));};
  {if(!from_wifi_flag){customizedSerial . print(this->data . finishedPrints);}else{serial2 . print(this->data . finishedPrints);}};

  {serialprintPGM((", Failed: "));}; 
  {if(!from_wifi_flag){customizedSerial . print(this->data . totalPrints - this->data . finishedPrints - ((this->isRunning() || this->isPaused()) ? 1 : 0));}else{serial2 . print(this->data . totalPrints - this->data . finishedPrints - ((this->isRunning() || this->isPaused()) ? 1 : 0));}};
  ...
  {serialprintPGM(("Total time: "));};
  {if(!from_wifi_flag){customizedSerial . print(buffer);}else{serial2 . print(buffer);}};


  elapsed = this->data.longestPrint;
  elapsed.toString(buffer);

  {serialprintPGM((", Longest job: "));};
  {if(!from_wifi_flag){customizedSerial . print(buffer);}else{serial2 . print(buffer);}};
  ...
  {serialprintPGM(("Filament used: "));};
  {if(!from_wifi_flag){customizedSerial . print(this->data . filamentUsed / 1000);}else{serial2 . print(this->data . filamentUsed / 1000);}};
  {serialprintPGM(("m"));};
...
void PrintCounter::tick() {
  ...
  static uint32_t update_last = HAL_GetTick(),
                  eeprom_last = HAL_GetTick();
