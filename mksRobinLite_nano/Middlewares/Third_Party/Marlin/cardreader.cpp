...
void CardReader::initsd() {
  ...
  //#define SPI_SPEED SPI_FULL_SPEED
	#define SPI_SPEED SPI_SD_INIT_RATE
  #endif
...
int CardReader::openFile(char* tName, bool read, bool push_current/*=false*/) {
  ...
  if (isFileOpen()) { //replacing current file by new file, or subfile call
	...
	lastOpen = false;
	  ...
	  //SERIAL_ECHOLNPAIR("\" pos", sdpos); 	/*--mks--*/
	  SERIAL_ECHOPGM("\" pos");
	  SERIAL_ECHOLN(sdpos);
  ...
  if (read) {
    if (file.open(curDir, fname, O_READ)) {
      ...
      //SERIAL_PROTOCOLLNPAIR(MSG_SD_SIZE, filesize);	/*--mks--*/
      sdpos = 0;

      SERIAL_PROTOCOLLNPGM(MSG_SD_FILE_SELECTED);
      getfilename(0, fname);
      lcd_setstatus(longFilename[0] ? longFilename : fname);
		if(strcmp(fname,MKS_PFT_NAME) != 0)
			{
			...
			//else
			//	strcpy(mksReprint.filename,fname);	
			}
    }
    else {
    ...
    if (!file.open(curDir, fname, O_CREAT | O_APPEND | O_WRITE | O_TRUNC)) {
      ...
      saving = true;
      //SERIAL_PROTOCOLLNPAIR(MSG_SD_WRITE_TO_FILE, name);	//mks marsk
      if(strcmp(fname,MKS_PFT_NAME) != 0)
        lcd_setstatus(fname);
    }
  }
