...
//  #if ENABLED(MESH_BED_LEVELING) && ENABLED(LCD_BED_LEVELING)
  ...
  // Manual Moves
  //const float manual_feedrate_mm_m[] = MANUAL_FEEDRATE;	//mks
...
//  #if ENABLED(PIDTEMP)	/*--mks cfg end--*/
#if 1
...
void lcd_Init_MINI12864() 	/*--mks--*/
{
	u8g.u8gInit_MINI12864();
}

void lcd_Init_LCD12864() 	/*--mks--*/
{
	u8g.u8gInit_LCD12864();
}

void lcd_Init_OLED12864() 	/*--mks--*/
{
  ...
  void lcd_quick_feedback() {
    ...
    lcd_buzz(LCD_FEEDBACK_FREQUENCY_DURATION_MS, LCD_FEEDBACK_FREQUENCY_HZ);	/*--mks--*/
    #if ENABLED(LCD_USE_I2C_BUZZER)
      delay(10);
    #elif PIN_EXISTS(BEEPER)
	...
	MENU_ITEM(submenu, MSG_PREPARE, lcd_prepare_menu);
      ...
      if (card.cardOK) {
        if (card.isFileOpen()) {
          ...
          if (card.sdprinting)
          //if (card.sdprinting || planner.blocks_queued())
            MENU_ITEM(function, MSG_PAUSE_PRINT, lcd_sdcard_pause);
          else
...
#ifdef USE_MKS_WIFI 
		MENU_ITEM(submenu, MSG_WIFI_INFO, lcd_wifi_info);
#endif
...
//    #if ENABLED(MESH_BED_LEVELING) && ENABLED(LCD_BED_LEVELING)
   ...
   // #endif
    ...
    //
		...
		TIM1->CCR1 = fanSpeeds[0]*10000/255;	/*--mks--*/
      #endif
      #if HAS_FAN1
...
/*
    ...
    ;
*/
	 uint8_t total_probe_points  = (GRID_MAX_POINTS_X) * (GRID_MAX_POINTS_Y);
...
*/	
 ...
 if(card.sdprinting == false)
    MENU_ITEM(submenu, MSG_IN_OUT_FILAMENT, lcd_filament_change_header);
    ...
    //
	  ...
	  if(Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN)
	  	{
		  if (!endstops.z_probe_enabled && (READ(Z_MIN_PIN) != Z_MIN_ENDSTOP_INVERTING))
			MENU_ITEM(gcode, MSG_BLTOUCH_RESET, PSTR("M280 P" STRINGIFY(Z_ENDSTOP_SERVO_NR) " S" STRINGIFY(BLTOUCH_RESET)));
	  	}
	  else if(Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MAX_ENDSTOP_PIN
	  	{
		  if (!endstops.z_probe_enabled && (READ(Z_MAX_PIN) != Z_MIN_PROBE_ENDSTOP_INVERTING))
			MENU_ITEM(gcode, MSG_BLTOUCH_RESET, PSTR("M280 P" STRINGIFY(Z_ENDSTOP_SERVO_NR) " S" STRINGIFY(BLTOUCH_RESET)));
	  	}
    #endif
...
#define IN_OUT_FILAMENT() do { \
      if (_menuLineNr == _thisItemNr) { \
        if (lcdDrawUpdate) { \
          lcd_implementation_drawmenu_static(_lcdLineNr, PSTR(MSG_IN_OUT_TEMP), false, false); \
...
void in_filament_move(){
	  START_MENU();
	  IN_OUT_FILAMENT();
	  STATIC_ITEM(MSG_IN_FILAMENTING, true, false);
	  END_MENU();
}
void out_filament_move(){
	  START_MENU();
	  IN_OUT_FILAMENT();
	  STATIC_ITEM(MSG_OUT_FILAMENTING, true, false);
	  END_MENU();
}
void in_out_heating(){
	  START_MENU();
	  if(currentScreen != lcd_status_screen){
	  IN_OUT_FILAMENT();
	  STATIC_ITEM(MSG_HEATING, true, false);
	  STATIC_ITEM(MSG_HEATING_WAIT, true, false);
	  	}
	  END_MENU();
}
void AUTO_IN_FILAMENT() {
   if(in_filament_flag == true){
	    if(abs(thermalManager.current_temperature[active_extruder] -thermalManager.degTargetHotend(active_extruder))<1){
			...
			enqueue_and_echo_commands_P(PSTR("G1 E100 F100"));
...
void AUTO_OUT_FILAMENT(){
  if(out_filament_flag == true){
			...
			mks_lcd_move_get_e_amount_move_flag = true;
			enqueue_and_echo_commands_P(PSTR("G91"));
			enqueue_and_echo_commands_P(PSTR("G1 E10 F100"));
			enqueue_and_echo_commands_P(PSTR("G1 E-110 F200"));
		        enqueue_and_echo_commands_P(PSTR("G90"));
	        }
			else in_out_heating();
        } 
...
void lcd_filament_change_header(){
  ...
  MENU_BACK(MSG_PREPARE);
  MENU_ITEM(submenu, MSG_IN_FILAMENT, mks_in_filament);
  MENU_ITEM(submenu, MSG_OUT_FILAMENT, mks_out_filament);
  MENU_ITEM(function, MSG_FILAMENT_MOVE_10MM_STOP, mks_lcd_move_get_e_amount_stop);
  END_MENU();
}
  void lcd_move_menu() {
    START_MENU();
    ...
    //if (_MOVE_XYZ_ALLOWED) {	//mks_delta
...
#endif
	if(PIDTEMP == 0 && PIDTEMPBED == 0)  sprintf_P(cmd, PSTR("M303 U1 E%i S%i"), e,autotune_temp[e]);		
	if(PIDTEMP == 0 && PIDTEMPBED == 1)  sprintf_P(cmd, PSTR("M303 U1 E%i S%i"), e,autotune_temp_bed);		
	if(PIDTEMP == 1 && PIDTEMPBED == 0)  sprintf_P(cmd, PSTR("M303 U1 E%i S%i"), e,autotune_temp[e]);		
	if(PIDTEMP == 1 && PIDTEMPBED == 1)  sprintf_P(cmd, PSTR("M303 U1 E%i S%i"), e,e < 0 ? autotune_temp_bed : autotune_temp[e]);		
...
#include "thermistornames.h"
		...
		STATIC_ITEM("Max Temp: ", false, false, buffer); 
#if TEMP_SENSOR_1 != 0
  #undef THERMISTOR_ID
  ...
  #include "thermistornames.h"
    ...
    void lcd_info_board_menu() {
      ...
      STATIC_ITEM(MSG_INFO_BAUDRATE ": " STRINGIFY(BAUDRATE), false); // Baud: 250000
      STATIC_ITEM(MSG_INFO_PROTOCOL ": " PROTOCOL_VERSION, false);    // Protocol: 1.0
      #if POWER_SUPPLY == 0
        STATIC_ITEM(MSG_INFO_PSU ": Generic", false);
      #elif POWER_SUPPLY == 1
        STATIC_ITEM(MSG_INFO_PSU ": ATX", false);  // Power Supply: ATX
      #elif POWER_SUPPLY == 2
        STATIC_ITEM(MSG_INFO_PSU ": XBox", false); // Power Supply: XBox
      #endif
      END_SCREEN();
    }
    ...
    void lcd_info_printer_menu() {
	  ...
	  switch(MACHINETPYE)
		  {
		  case DELTA:
			   STATIC_ITEM("Deltabot", true,true); 
			  break;
		  case COREXY:
		  case COREYX:
		  case COREXZ:
		  ...
		  case COREYZ:
		  case COREZY:
			  STATIC_ITEM("Core Printer", true,true); 
			  break;
		  case MORGAN_SCARA:
		  case MAKERARM_SCARA:
			  STATIC_ITEM("Scara Printer", true,true); 
			  break;
		  default: 
		  		STATIC_ITEM("3D Printer", true,true); 
		  	break;		  
		  }
	  STATIC_ITEM("Version: " SHORT_LITE_VERSION, false);
	 
#ifdef USE_MKS_WIFI	
	  if(wifi_firm_ver[0] != 0)
	  {
	  	 memset(tempBuf, 0, sizeof(tempBuf));
		sprintf_P(tempBuf, "WIFI: %s", (const char *)wifi_firm_ver);
		STATIC_ITEM(tempBuf, false);
		
	  }
	  else
#endif            
	  	STATIC_ITEM("WIFI: ", false);
	  	
      STATIC_ITEM(MSG_INFO_EXTRUDERS ": " STRINGIFY(EXTRUDERS), false); // Extruders: 2
      END_SCREEN();
    }
    ...
    void lcd_info_menu() {
      ...
      END_MENU();
	   ...
	   /**
		 *
		 * "About Wifi's ip/apname and so on 
		 *
		 */
#ifdef USE_MKS_WIFI      
		void lcd_wifi_info() {
		...
		MENU_ITEM(submenu, MSG_CLOUD_INFO, lcd_cloud_service_menu); 	   // Printer Info >
	
		memset(tempBuf, 0, sizeof(tempBuf));
		sprintf_P(tempBuf, "IP:%s", (const char *)ipPara.ip_addr);
		STATIC_ITEM((const char *)tempBuf,false);
	
		memset(tempBuf, 0, sizeof(tempBuf));
		sprintf_P(tempBuf, "wifi:%s", (const char *)wifiPara.ap_name);
		STATIC_ITEM((const char *)tempBuf,false);
	
		if(wifiPara.mode == 0x01)
		{
			memset(tempBuf, 0, sizeof(tempBuf));
			sprintf_P(tempBuf, "key:%s", (const char *)wifiPara.keyCode);
			STATIC_ITEM((const char *)tempBuf,false);
		}
	
		memset(tempBuf, 0, sizeof(tempBuf));
		if(wifiPara.mode == 0x01)
			//strcpy(tempBuf, "STATE: AP");
			strcpy(tempBuf, MSG_STATE_AP);
		else
			//strcpy(tempBuf, "STATE: STA");
	   ...
	   /**
		...
		void lcd_cloud_service_menu()
		{
			...
			START_MENU();
			MENU_BACK(MSG_WIFI_INFO);
	
			if((cloud_para.state == 0x12) && (cloud_para.unbinding_flag == 0))
			{
				MENU_ITEM(function, MSG_CLOUD_UNBIND_USER, cloud_unbind); 
			}
			
			STATIC_ITEM((const char *)cloud_para.id);
			...
			{
				STATIC_ITEM((const char *)"Unbinding");
			}
			else
			{
				if(cloud_para.state == 0x10)
				{
					STATIC_ITEM(MSG_CLOUD_NOT_CONNECTED);
				}
				else if(cloud_para.state == 0x11)
				{
					STATIC_ITEM(MSG_CLOUD_NOT_BINDED);
				}
				else if(cloud_para.state == 0x12)
				{
					STATIC_ITEM(MSG_CLOUD_IS_BINDED);
				}
				else
				{
					STATIC_ITEM(MSG_CLOUD_DISABLED);
				}
			}
		//	u8g.drawFrame(0,  16, 127,	30);
   ...
   */
    ...
    #define HOTEND_STATUS_ITEM() do { \
      if (_menuLineNr == _thisItemNr) { \
        if (lcdDrawUpdate) { \
          lcd_implementation_drawmenu_static(_lcdLineNr, PSTR(MSG_FILAMENT_CHANGE_NOZZLE), false, false); \
          lcd_implementation_hotend_status(_lcdLineNr); \
        } \
        if (_skipStatic && encoderLine <= _thisItemNr) { \
          encoderPosition += ENCODER_STEPS_PER_MENU_ITEM; \
    ...
    void lcd_filament_change_wait_for_nozzles_to_heat() {
      ...
      #ifndef MSG_FILAMENT_CHANGE_HEATING_2
   ...
   */
    ...
    inline void handle_reprapworld_keypad() {
      ...
      if (!REPRAPWORLD_KEYPAD_PRESSED) {
		...
		if(MACHINETPYE != DELTA && Z_HOME_DIR == -1)
...
*/    
  ...
  if (ELAPSED(ms, next_lcd_update_ms)
    ...
    next_lcd_update_ms = ms + LCD_UPDATE_INTERVAL;
      ...
      if (encoderPastThreshold || lcd_clicked) {
        if (encoderPastThreshold) {
	     ...
	     return_to_status_ms1 = ms + MKSLCD_TIMEOUT_TO_STATUS;
        lcdDrawUpdate = LCDVIEW_KEEP_REDRAWING;
      }
    ...
    #endif
    {
      ...
      else if (currentScreen != mks_in_filament && currentScreen != mks_out_filament && currentScreen != in_filament_move && currentScreen != out_filament_move&& ELAPSED(ms, return_to_status_ms)){
          lcd_return_to_status();
      }
	  else if((currentScreen == mks_in_filament || currentScreen == mks_out_filament || currentScreen == in_filament_move || currentScreen == out_filament_move) && ELAPSED(ms, return_to_status_ms1)){
              lcd_return_to_status();
	      in_filament_move_flag = false;
	      out_filament_move_flag = false;
	  }
	...
	//step1: 運行了擠出機
	  ...
	  //step2: 擠出機停止運行開始計時
	   ...
	   //step3: 停止擠出機運行后多少間隔停止加熱頭
	   if(mks_lcd_move_get_e_amount_move_flag2 == true && millis()-mksms1 >5*60*1000){
...
void lcd_setstatuspgm(const char * const message, const uint8_t level) {
  ...
  //strncpy_P(lcd_status_message, message, 3 * (LCD_WIDTH));	/*--mks--*/
  strncpy(lcd_status_message, message, 3 * (LCD_WIDTH));
