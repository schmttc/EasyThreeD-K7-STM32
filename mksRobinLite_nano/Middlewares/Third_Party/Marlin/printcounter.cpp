...
void PrintCounter::initStats() {
  ...
  this->saveStats();
  //eeprom_write_byte((uint8_t *) this->address, 0x16);	/*--mks--*/
  uint8_t eppr_w=0x16;
...
void PrintCounter::loadStats() {
  ...
  */ /*--mks--*/
	...
	{
	  int BufferSize=sizeof(printStatistics);	  //sizeof(printStatistics) = 24 ???
	  uint16_t MemAddress =this->address + sizeof(uint8_t);
	  uint16_t* pBuffer = &(this->data.totalPrints); //???：＜data?：♂11|━?|━：2：＜???：oy?Y
