...
/*   
  ...
  #endif
*/
#define CORE_IS_XY (COREXY | COREYX)
#define CORE_IS_XZ (COREXZ | COREZX)
#define CORE_IS_YZ (COREYZ | COREZY)
#define IS_CORE (CORE_IS_XY | CORE_IS_XZ | CORE_IS_YZ)

#define CORE_AXIS_1 mksTmp.core_axis_1
#define CORE_AXIS_2 mksTmp.core_axis_2
#define NORMAL_AXIS mksTmp.normal_axis

static long CORESIGN(const long n) 
{
	if(MACHINETPYE&COREYX || MACHINETPYE&COREZX || MACHINETPYE&COREZY)
		return (-n);
	else
		return n;
}
...
*/
  ...
  #define IS_SCARA (MORGAN_SCARA | MAKERARM_SCARA)
  #define IS_KINEMATIC (DELTA | IS_SCARA)
  #define IS_CARTESIAN  Cartesian
...
#define Y_HOME_POS 0
   ...
   */
//  #if defined(DELTA) && !defined(DELTA_HEIGHT)
#if 0	//mks_delta
...
//#define HOMING_Z_WITH_PROBE (HAS_BED_PROBE  && ENABLED(Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN))
#define HOMING_Z_WITH_PROBE (HAS_BED_PROBE )
...
#define HAS_PID_HEATING (PIDTEMP || PIDTEMPBED)	
...
#define IS_Z2_OR_PROBE(A,M) ( \
	 (ENABLED(Z_DUAL_ENDSTOPS) && Z2_USE_ENDSTOP == _##A##M##_) \
  ||  Z_MIN_PROBE_PIN == A##_##M##_PIN ) )
#endif
  /**
   * Set ENDSTOPPULLUPS for active endstop switches
   */
  #if ENABLED(ENDSTOPPULLUPS)   //mks
...
#define HAS_Z_MIN_PROBE_PIN (PIN_EXISTS(Z_MIN_PROBE))
  ...
  #define HAS_TEMP_BED mksCfg.has_temp_bed
  ...
  //#define HAS_THERMALLY_PROTECTED_BED (ENABLED(THERMAL_PROTECTION_BED) && HAS_TEMP_BED && HAS_HEATER_BED)
	#define HAS_THERMALLY_PROTECTED_BED (ENABLED(THERMAL_PROTECTION_BED) && HAS_HEATER_BED)

 // #define WATCH_HOTENDS (ENABLED(THERMAL_PROTECTION_HOTENDS) && WATCH_TEMP_PERIOD > 0)
//  #define WATCH_THE_BED (HAS_THERMALLY_PROTECTED_BED && WATCH_BED_TEMP_PERIOD > 0)
	#define WATCH_HOTENDS (ENABLED(THERMAL_PROTECTION_HOTENDS) )
	#define WATCH_THE_BED (HAS_THERMALLY_PROTECTED_BED )
  ...
  #define WRITE_HEATER_0P(v) WRITE(HEATER_0_PIN, v)
    ...
    #define WRITE_HEATER_0(v) if(!mksTmp.cfg_hardware_test_enable) WRITE_HEATER_0P(v)
  #endif
  #if HAS_HEATER_BED
    //#define WRITE_HEATER_BED(v) WRITE(HEATER_BED_PIN, v)
	#define WRITE_HEATER_BED(v) if(!mksTmp.cfg_hardware_test_enable) WRITE(HEATER_BED_PIN, v)
  #endif
...
//  #if defined(DELTA)	//mks_dela
  ...
  #define ABL_PLANAR (AUTO_BED_LEVELING_LINEAR | AUTO_BED_LEVELING_3POINT)
  /*--mks cfg-- AUTO_BED_LEVELING_BILINEAR */
  //#define ABL_GRID   (ENABLED(AUTO_BED_LEVELING_LINEAR) || ENABLED(AUTO_BED_LEVELING_BILINEAR))
  //#define HAS_ABL    (ABL_PLANAR || ABL_GRID || ENABLED(AUTO_BED_LEVELING_UBL))
  //#define ABL_GRID 1
  #define ABL_GRID   (AUTO_BED_LEVELING_LINEAR | AUTO_BED_LEVELING_BILINEAR)
  //#define HAS_ABL 1
  #define HAS_ABL    (ABL_PLANAR | ABL_GRID | AUTO_BED_LEVELING_UBL)
  /*--mks cfg-- MESH_BED_LEVELING */
  //#define HAS_LEVELING          (HAS_ABL || ENABLED(MESH_BED_LEVELING))
  //#define HAS_LEVELING	1
  #define HAS_LEVELING          (HAS_ABL | MESH_BED_LEVELING)
  ...
  #define PLANNER_LEVELING      (ABL_PLANAR | ABL_GRID | MESH_BED_LEVELING)
...
/* 
  ...
  #endif
*/
#define MIN_PROBE_X mksTmp.min_probe_x
#define MAX_PROBE_X mksTmp.max_probe_x
#define MIN_PROBE_Y mksTmp.min_probe_y
#define MAX_PROBE_Y mksTmp.max_probe_y
...
//  #define HAS_HOME_OFFSET (DISABLED(NO_WORKSPACE_OFFSETS) || ENABLED(DELTA))	//mks_delta
#define HAS_HOME_OFFSET 1	 

  // Either offset yields extra calculations on all moves
  //#define HAS_WORKSPACE_OFFSET (HAS_POSITION_SHIFT || HAS_HOME_OFFSET)	//mks_delta
#define HAS_WORKSPACE_OFFSET 1  
  // M206 doesn't apply to DELTA
// #define HAS_M206_COMMAND (HAS_HOME_OFFSET && DISABLED(DELTA))			//mks_delta
