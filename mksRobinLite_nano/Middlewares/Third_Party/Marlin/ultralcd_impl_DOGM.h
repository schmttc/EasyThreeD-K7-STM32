...
#define FONT_SPECIAL_NAME Marlin_symbols
  ...
  #elif ENABLED(DISPLAY_CHARSET_ISO10646_CN)    /*--mks cfg--LCD_LANGUAGE*/
...
// LCD selection
  ...
  #ifdef MKS_OLED_096          //SCK        MOSI              CS           A0
    U8GLIB_SSD1306_128X64 u8g(LCD_PINS_D4, LCD_PINS_ENABLE, LCD_PINS_RS, LCD_PINS_D5);
  #else
...
#ifdef ENABLED(MKS_OLED_096) && ENABLED(LCD_SCREEN_ROT_180)
  #define mks_offset 0 
#else
  #define mks_offset 0
#endif
...
static void lcd_implementation_init() {
    ...
    #elif ENABLED(LITE_MAYI_LOGO) || ENABLED(LITE_PLT_LOGO)
      constexpr uint8_t offy = LOGO_START_Y;
    #else
    ...
    if (show_bootscreen) {
      u8g.firstPage();  //mks - show logo begin
      do {
    ...
    show_bootscreen = false;    //mks - show logo end
...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_UNDER(STATUS_SCREENHEIGHT + 1)) {
    ...
    u8g.drawBitmapP(MKS_BMP_START_X, MKS_BMP_START_Y, STATUS_SCREENBYTEWIDTH, STATUS_SCREENHEIGHT,
      #if HAS_FAN0
  ...
  if (PAGE_UNDER(28)) {
    // Extruders
    HOTEND_LOOP() _draw_heater_status(MKS_TEMP_START_X + e * 25, e);//5
