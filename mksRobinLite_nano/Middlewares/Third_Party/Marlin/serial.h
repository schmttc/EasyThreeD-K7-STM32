...
#ifdef USE_MKS_WIFI
#define SERIAL_CHAR(x) {if(!from_wifi_flag){MYSERIAL.write(x);}else{WIFICOM.print(x);}}
#define SERIAL_EOL SERIAL_CHAR('\n')

#define SERIAL_PROTOCOLCHAR(x)              SERIAL_CHAR(x)
#define SERIAL_PROTOCOL(x)               {if(!from_wifi_flag){MYSERIAL.print(x);}else{WIFICOM.print(x);}}
#define SERIAL_PROTOCOL_F(x,y)          {if(!from_wifi_flag){MYSERIAL.print(x,y);}else{WIFICOM.print(x,y);}}
#define SERIAL_PROTOCOLPGM(x)               {serialprintPGM(PSTR(x));}
#define SERIAL_PROTOCOLLN(x)               {do{ if(!from_wifi_flag){MYSERIAL.print(x);}else{WIFICOM.print(x);} SERIAL_EOL; }while(0);}
