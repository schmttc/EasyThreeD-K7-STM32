...
#else
  int freeMemory() { return 0; }  //No free ram management
#endif //!SDSUPPORT
...
static bool drain_queued_commands_P() {
  if (queued_commands_P != NULL) {
    size_t i = 0;
    char c, cmd[30];
    strncpy(cmd, queued_commands_P, sizeof(cmd) - 1);
    // strncpy_P(cmd, queued_commands_P, sizeof(cmd) - 1);   -- BDI
    cmd[sizeof(cmd) - 1] = '\0';
...
void setup_killpin() {
  #if HAS_KILL          //RobinLite::HAS_KILL == 0   
    SET_INPUT(KILL_PIN);
    WRITE(KILL_PIN, HIGH);
  #endif
}
...
void suicide() {
#if defined(STM_3DPRINT)		// Marlin4ST
	serialprintPGM("3DPrinter board resetting !");
	NVIC_SystemReset();
#elif defined(MKS_ROBIN)		// MarlinMks
  #if HAS_SUICIDE
...
#if ENABLED(FILAMENT_RUNOUT_SENSOR) //RobinLite::FILAMENT_RUNOUT_SENSOR==0
	setup_filrunoutpin();
#endif
...
#if HAS_STEPPER_RESET   //RobinLite::HAS_STEPPER_RESET == 0
    disableStepperDrivers();    
#endif

#if !defined(DEBUG_ONGOING)
  #if defined(WITH_RPI_DETECTION) //RobinLite::WITH_RPI_DETECTION==0
    BSP_RPiGpioInit();
    BSP_RPiWaitUntilReady();
  #elif defined(WAIT_FOR_RPI)   //RobinLite::WAIT_FOR_RPI==0
...
void loop() {
  ...
  #if ENABLED(SDSUPPORT)
    p_card->checkautostart(false);
  #endif

#if !defined(NO_WIFI)
  if ((commands_in_queue) && (cmdForWifi==false)) {
#else
...
#define XYZ_CONSTS_FROM_CONFIG(type, array, CONFIG) \
  ...
  // { return pgm_read_any(&array##_P[axis]); }   // BDI
...
#endif							//#endif Marlin4ST / MarlinMks
    ...
    // Home Z last if homing towards the bed
      ...
      if (home_all_axis || homeZ) {
          ...
          if (home_all_axis) {
             ...
             */
            destination[X_AXIS] = round((float)Z_SAFE_HOMING_X_POINT - (X_PROBE_OFFSET_FROM_EXTRUDER));
            destination[Y_AXIS] = round((float)Z_SAFE_HOMING_Y_POINT - (Y_PROBE_OFFSET_FROM_EXTRUDER));
...
#elif defined(STM32F103xB)
      int abl2 = sq(auto_bed_leveling_grid_points);
        double eqnAMatrix[AUTO_BED_LEVELING_GRID_POINTS*AUTO_BED_LEVELING_GRID_POINTS * 3], // "A" matrix of the linear system of equations
               eqnBVector[AUTO_BED_LEVELING_GRID_POINTS*AUTO_BED_LEVELING_GRID_POINTS],     // "B" vector of Z points
               mean = 0.0;
        int8_t indexIntoAB[AUTO_BED_LEVELING_GRID_POINTS][AUTO_BED_LEVELING_GRID_POINTS];
#endif	 
...
inline void gcode_M17() {
  ...
  inline void gcode_M26() {
    if (p_card->cardOK && code_seen('S'))
      p_card->setIndex(code_value_long());
  }
  ...
  inline void gcode_M30() {
    if (p_card->cardOK) {
      p_card->closefile();
      p_card->removeFile(current_command_args);
    }
  }
...
inline void gcode_M31() {
  ...
  inline void gcode_M32() {
    ...
    if (p_card->cardOK) {
      p_card->openFile(namestartpos, true, call_procedure);
    ...
    inline void gcode_M33() {
      p_card->printLongPath(current_command_args);
    }
...
inline void gcode_M42() {
  ...
  // analogWrite(pin_number, pin_status);   // BDI
